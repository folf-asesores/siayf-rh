...
public class ConfiguracionModuloAccionService implements Serializable{
	...
	private void guardarDetalleConfiguracion(Integer configuracion, List<AccionDTO> acciones){
		if(!acciones.isEmpty()){
		//primero buscara si ya hay detalles de la configuracion de ser asi los borrara y guardara la nueva lista de acciones.
		List<DetalleConfiguracionModuloAccionEntity> detallesExistentes = new ArrayList<>();
		
		detallesExistentes = detalleConfiguracionDao.obtenerDetallesPorIdConfiguracion(configuracion);
		...
		for(AccionDTO dtos :acciones){
			System.out.println("accion" + dtos.getId_accion());
			DetalleConfiguracionModuloAccionEntity detalle = new DetalleConfiguracionModuloAccionEntity();
			detalle.setId_accion(accionesRepository.obtenerPorId(dtos.getId_accion()));
			detalle.setIdConfiguracionModuloAccion(configuracion);
	...
	public void eliminar(Integer id){
		...
		//primero buscara si ya hay detalles de la configuracion de ser asi los borrara y guardara la nueva lista de acciones.
				...
				detallesExistentes = detalleConfiguracionDao.obtenerDetallesPorIdConfiguracion(entity.getId_configuracion_modulo_accion());
				if(detallesExistentes!=null){
					for(DetalleConfiguracionModuloAccionEntity detalleDelet: detallesExistentes){
						detalleConfiguracionDao.eliminarPorId(detalleDelet.getId_detalle_configuracion_modulo_accion());
					}
	...
	private List<AccionDTO> obtenerAccionesPorConfiguracion(Integer idConfiguracion){
		...
		List<DetalleConfiguracionModuloAccionEntity> detalles = detalleConfiguracionDao.obtenerDetallesPorIdConfiguracion(idConfiguracion);
