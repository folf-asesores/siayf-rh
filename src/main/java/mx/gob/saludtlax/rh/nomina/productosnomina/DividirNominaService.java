...
public class DividirNominaService implements Serializable {
    ...
    private static final String OBTENER_PRODUCTO_NOMINA = "SELECT NEW mx.gob.saludtlax.rh.nomina.productosnomina.DividirNominaDTO("
            + " nominaEmpleado.idNominaEmpleado," + " productoNomina.idProductoNomina," + " productoNomina.nombreProducto," + " nominaEmpleado.consecutivo,"
            + " empleado.idEmpleado," + " empleado.rfc," + " empleado.nombreCompleto," + " unidadResponsable.idUnidadResponsable,"
            + " unidadResponsable.descripcion," + " rama.idRamaPuesto," + " rama.nombreRamaPuesto," + " nominaEmpleado.percepciones,"
            + " nominaEmpleado.deducciones," + " nominaEmpleado.neto)" + " FROM NominaEmpleadoEntity AS nominaEmpleado"
            + " JOIN nominaEmpleado.idProductoNomina AS productoNomina" + " JOIN nominaEmpleado.idEmpleado AS empleado"
            + " JOIN nominaEmpleado.idUnidadResponsable AS unidadResponsable" + " JOIN nominaEmpleado.idPuestoGeneral AS puesto" + " JOIN puesto.idRama AS rama"
            + " WHERE productoNomina.idProductoNomina = :idProductoNomina";
    private static final String OBTENER_ID_NOMINA_EMPLEADOS = "SELECT nominaEmpleado.idNominaEmpleado" + " FROM NominaEmpleadoEntity AS nominaEmpleado"
            + " JOIN nominaEmpleado.idProductoNomina AS productoNomina" + " JOIN nominaEmpleado.idEmpleado AS empleado"
            + " JOIN nominaEmpleado.idUnidadResponsable AS unidadResponsable" + " JOIN nominaEmpleado.idPuestoGeneral AS puesto" + " JOIN puesto.idRama AS rama"
            + " WHERE productoNomina.idProductoNomina = :idProductoNomina" + " AND empleado.rfc IN :rfc"
            + " AND unidadResponsable.descripcion LIKE :unidadResponsable" + " AND rama.nombreRamaPuesto LIKE :rama";
    private static final String ACTUALIZAR_PRODUCTO_NOMINA = "UPDATE nomina_empleado" + " INNER JOIN empleados "
            + " ON empleados.id_empleado = nomina_empleado.id_empleado" + " SET nomina_empleado.id_producto_nomina = :idProductoNominaNuevo"
            + " WHERE nomina_empleado.id_nomina_empleado > 0" + " AND nomina_empleado.id_producto_nomina = :idProductoNominaViejo"
