...
public class EvaluadorService {
	...
	public BigDecimal evaluarFormula(ConceptoNominaFederalesDTO conceptoFederal, NominaEmpleadoEntity nominaEmpleado) {
		TabuladorDTO tabulador = tabuladorService.obtenerTabuladorPorPuesto(
				nominaEmpleado.getIdPuestoGeneral().getIdPuestoGeneral(), FechaUtil.ejercicioActual());
		...
		if (tabulador == null || formula == null) {
			...
			if (formula.contains("C30") || formula.contains("CA1")) {
				conceptos30 = conceptoNominaService.obtenerConceptosPorConfiguracionPresupuestal(
						nominaEmpleado.getIdConfiguracionPresupuestal().getId());
				...
				if (conceptos30 != null) {
					...
					for (ConceptoNominaFederalesDTO conceptoNominaFederales : conceptos30) {
						if (conceptoNominaFederales.getClave().contains("30BR")
								|| conceptoNominaFederales.getClave().contains("30MR")
						...
						if (conceptoNominaFederales.getClave().contains("A100")
								|| conceptoNominaFederales.getClave().contains("A200")
								|| conceptoNominaFederales.getClave().contains("A300")
								|| conceptoNominaFederales.getClave().contains("A400")
...
//						System.out.println("Formula concepto a1: " + formula);
			...
			formula = formula.replace("EX0700",
					(tabulador.getSueldoBrutoMensual() == null ? "0.0" : tabulador.getSueldoBrutoMensual() + ""));
			formula = formula.replace("EX4200", (tabulador.getAsignacionBrutaMensual() == null ? "0.0"
					: tabulador.getAsignacionBrutaMensual() + ""));
			formula = formula.replace("EX55AG",
					(tabulador.getAgaBrutaMensual() == null ? "0.0" : tabulador.getAgaBrutaMensual() + ""));
