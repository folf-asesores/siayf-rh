...
public class TimbradoProductoNominaService implements Serializable {
	...
	public List<ProductoNominaDTO> listadoNominasPendientesPorTimbrar() {
		...
		List<ProductoNominaDTO> listadoProductoNominaDTO = new ArrayList<ProductoNominaDTO>();
	...
	public TimbradoResult timbrarProductoNominina(int idProductoNomina) {
		...
		List<DatosCFDINomina> listadoDatosCFDINomina = new ArrayList<DatosCFDINomina>();
		...
		for (NominaEmpleadoEntity nominaEmpleadoEntity : listadoNominaEmpleadoEntity) {
			...
			datosCFDINomina
					.setAntiguedad(calcularAnyosAntiguedad(nominaEmpleadoEntity.getIdEmpleado().getFechaIngreso()));
			...
			if (nominaEmpleadoEntity.getIdConfiguracionPresupuestal().getPuesto().getPuestoTimbre() != null) {
				datosCFDINomina
						.setPuesto(nominaEmpleadoEntity.getIdConfiguracionPresupuestal().getPuesto().getPuestoTimbre());

			} else {
				datosCFDINomina
						.setPuesto(nominaEmpleadoEntity.getIdConfiguracionPresupuestal().getPuesto().getPuesto());
			}

			datosCFDINomina.setTipoContrato(
					nominaEmpleadoEntity.getIdConfiguracionPresupuestal().getTipoContratacion().getTipoContratacion());
			// Falta Jornada
			...
			DireccionEntity direccionEmpleadoEntity = direccionRepository
					.consultarDireccionEmpleadoPorId(nominaEmpleadoEntity.getIdEmpleado().getIdEmpleado());
			...
			List<DeduccionCFDI> listadoDeducionesCFDI = new ArrayList<DeduccionCFDI>();
			...
			List<PercepcionCFDI> listadoPercepcionesCFDI = new ArrayList<PercepcionCFDI>();
			...
			if (datosCFDINominaTimbrar.getCurp().length() > 0) {
				...
				if (datosCFDITimbrado.getIdComprobante() != null) {
					...
					NominaEmpleadoEntity nominaEmpleado = nominaEmpleadoRepository
							.obtenerPorId(new Integer(datosCFDINominaTimbrar.getFolio()));
