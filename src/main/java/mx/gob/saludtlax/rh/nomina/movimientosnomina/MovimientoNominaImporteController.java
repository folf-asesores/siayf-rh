...
public class MovimientoNominaImporteController implements Serializable {
	...
	public void incio() {
		...
		tipoMovimientoSeleccionado = tipoMovimientosNominaEJB
				.obtenerTipoMovimientoPorClave(beanMovimientos.getView().getClaveMovimiento());
		...
		configuracionMovimiento = movimientoFijoService
				.obtenerConfiguracionesPorTipoMovimiento(tipoMovimientoSeleccionado.getIdTimpoMovimiento());
			...
			if (!configuracionMovimiento.getListConceptoNomina().isEmpty()) {
				...
				for (ConceptoNominaFederalesDTO concepto : configuracionMovimiento.getListConceptoNomina()) {
					...
					if (concepto.getTipo().compareTo(1) == 0) {
						System.out.println("concepto1: " + concepto.getTipo() + concepto.getDescripcion() + " "
								+ concepto.getFormula());
					...
					if (concepto.getTipo().compareTo(2) == 0) {
						...
						System.out.println("concepto2: " + concepto.getTipo() + concepto.getDescripcion() + " "
								+ concepto.getFormula());
						Evaluador evaluador = new Evaluador(concepto.getFormula());
	...
	public void agregarMovimiento() {
		MovimientosController beanMovimientos = (MovimientosController) JSFUtils.getManagedBean("movimientos");
		System.out
				.println("Empleado seleccionado" + beanMovimientos.getView().getEmpleadoSeleccionado().getIdEmpleado());
