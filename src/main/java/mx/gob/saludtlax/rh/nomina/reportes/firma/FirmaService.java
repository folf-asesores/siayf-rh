    ...
    private FirmaDTO convertir(List<FirmaPojo> datosBrutos) {
        if (datosBrutos != null && !datosBrutos.isEmpty()) {
            ...
            for (FirmaPojo firmaPojo : datosBrutos) {
                ...
                if (!unidadesResponsables
                        .containsKey(numeroUnidadResponsable)) {
                    ...
                    firmasEmpleados.put(firmaEmpleado.getFiliacion(),
                            firmaEmpleado);

                    Map<Integer, ProgramaDTO> programas = new TreeMap<>();
                    Integer idPrograma = firmaPojo.getIdPrograma();
                    ProgramaDTO programa = new ProgramaDTO.Builder(idPrograma,
                            firmaPojo.getPrograma(),
                            firmaPojo.getInicioPeriodo(),
                            firmaPojo.getFinPeriodo())
                                    .setFirmasEmpleados(firmasEmpleados)
                                    .construirProgramaDTO();
                    programas.put(idPrograma, programa);

                    UnidadResponsableDTO unidadResponsable = new UnidadResponsableDTO.Builder(
                            firmaPojo.getClave(), firmaPojo.getDescripcion())
                    ...
                    UnidadResponsableDTO unidadResponsable = unidadesResponsables
                            .get(numeroUnidadResponsable);
                    Map<Integer, ProgramaDTO> programas = unidadResponsable
                            .getProgramas();
                    Integer idPrograma = firmaPojo.getIdPrograma();
                    ...
                    if (!programas.containsKey(idPrograma)) {
                        ...
                        ProgramaDTO programa = new ProgramaDTO.Builder(
                                idPrograma, firmaPojo.getPrograma(),
                                firmaPojo.getInicioPeriodo(),
                                firmaPojo.getFinPeriodo())
                                        .setFirmasEmpleados(firmasEmpleados)
                                        .construirProgramaDTO();
                        ...
                        Map<String, FirmaEmpleadoDTO> firmasEmpleados = programa
                                .getFirmasEmpleados();
                        FirmaEmpleadoDTO firmaEmpleado = new FirmaEmpleadoDTO.Builder()
                                .setFiliacion(firmaPojo.getFiliacion())
                                .setNombre(firmaPojo.getNombre())
            ...
            FirmaDTO firma = new FirmaDTO.Builder(
                    firmaPojo.getIdProductoNomina(), firmaPojo.getFechaPago(),
