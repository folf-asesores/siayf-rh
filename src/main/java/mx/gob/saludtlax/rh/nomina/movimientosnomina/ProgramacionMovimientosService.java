...
public class ProgramacionMovimientosService implements Serializable {
    ...
    public void crear(ProgramarMovimientoDTO dto) {
        ...
        entity.setIdTipoMovimiento(tipoMovimientoRepository
        ...
        if (dto.getListaDetalles() != null
                && !dto.getListaDetalles().isEmpty()) {
            for (DetalleProgramacionMovimientoDTO dtoDetalle : dto
                    .getListaDetalles()) {
                ...
                if (dtoDetalle.getIdPuesto() != null) {
                    newDetalle.setIdPuesto(puestoGeneralRepository
                            .obtenerPorId(dtoDetalle.getIdPuesto()));
                }
                if (dtoDetalle.getIdTipoContratacion() != null) {
                    newDetalle.setIdTipoContratacion(contratacionRepository
                            .obtenerPorId(dtoDetalle.getIdTipoContratacion()));
                }
    ...
    public void modificar(ProgramarMovimientoDTO dto) {
        ...
        if (dto.getListaDetalles() != null
            ...
            listaold = detalleProgramacionMovimientoRepository
                    .obtenerRegistrosPorProgramacionMovimimento(
                            dto.getIdProgramacionMovimiento());
            for (DetalleProgramacionMovimientoEntity entityOld : listaold) {
                detalleProgramacionMovimientoRepository
                        .eliminar(detalleProgramacionMovimientoRepository
                                .obtenerPorId(entityOld
                                        .getIdDetalleProgramacionMovimiento()));
            }
    ...
    public void eliminar(ProgramarMovimientoDTO dto) {
        ...
        programarMovimientoRepository
                .eliminarPorId(dto.getIdProgramacionMovimiento());
    }

    public List<ProgramarMovimientoDTO> obtenerListaRegistros(
            Integer idTipoMovimieto) {
        List<ProgramarMovimientoDTO> dtos = new ArrayList<>();
        List<ProgramarMovimientoEntity> entitys = new ArrayList<>();
        entitys = programarMovimientoRepository
                .obtenerRegistrosPorTipoMovimiento(idTipoMovimieto);

        if (entitys != null && !entitys.isEmpty()) {
            for (ProgramarMovimientoEntity en : entitys) {
                ProgramarMovimientoDTO progdto = new ProgramarMovimientoDTO();
                progdto.setIdProgramacionMovimiento(
                        en.getIdProgramacionMovimiento());
                progdto.setIdTipoMovimiento(
                        en.getIdTipoMovimiento().getIdMovimientoNomina());
                progdto.setImporte(en.getImporte());
                progdto.setMovimiento(en.getIdTipoMovimiento().getClave() + " "
                        + en.getIdTipoMovimiento().getDescripcion());
                ...
                listaDetalles = listaDetallesPorProgramacionMovimiento(
                        progdto.getIdProgramacionMovimiento());
    ...
    public List<DetalleProgramacionMovimientoDTO> listaDetalles() {
        ...
        if (!lista.isEmpty()) {
            ...
            for (DetalleProgramacionMovimientoEntity entity : lista) {
                ...
                dto.setIdDetalleProgramacionMovimiento(
                        entity.getIdDetalleProgramacionMovimiento());

                if (entity.getIdPuesto() != null) {
                    PuestoGeneralEntity puestoEnt = new PuestoGeneralEntity();
                    puestoEnt = puestoGeneralRepository.obtenerPorId(
                            entity.getIdPuesto().getIdPuestoGeneral());
                    dto.setDescripcionPuesto(puestoEnt.getCodigo() + " "
                            + puestoEnt.getPuesto());
                ...
                if (entity.getIdTipoContratacion() != null) {
                    ...
                    tipocE = contratacionRepository.obtenerPorId(
                            entity.getIdTipoContratacion().getId());
                    dto.setIdTipoContratacion(tipocE.getId());
                    dto.setDescripcionTipoContratacion(
                            tipocE.getTipoContratacion());
                }

                dto.setIdProgramacionMovimiento(
                        entity.getIdProgramacionMovimiento()
                                .getIdProgramacionMovimiento());
    ...
    public List<DetalleProgramacionMovimientoDTO> listaDetallesPorProgramacionMovimiento(
            Integer idProgramacionMov) {
        List<DetalleProgramacionMovimientoEntity> lista = new ArrayList<>();
        lista = detalleProgramacionMovimientoRepository
                .obtenerRegistrosPorProgramacionMovimimento(idProgramacionMov);
        List<DetalleProgramacionMovimientoDTO> listadtos = new ArrayList<>();
        if (!lista.isEmpty()) {
