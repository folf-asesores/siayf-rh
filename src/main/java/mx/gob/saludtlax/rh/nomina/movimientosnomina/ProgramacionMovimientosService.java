...
public class ProgramacionMovimientosService implements Serializable {
        ...
        if (dto.getListaDetalles() != null && !dto.getListaDetalles().isEmpty()) {
            for (DetalleProgramacionMovimientoDTO dtoDetalle : dto.getListaDetalles()) {
                ...
                if (dtoDetalle.getIdTipoContratacion() != null) {
                    newDetalle.setIdTipoContratacion(contratacionRepository.obtenerPorId(dtoDetalle.getIdTipoContratacion()));
                }
    ...
    public void modificar(ProgramarMovimientoDTO dto) {
        ProgramarMovimientoEntity entity = programarMovimientoRepository.obtenerPorId(dto.getIdProgramacionMovimiento());
        ...
        if (dto.getListaDetalles() != null && !dto.getListaDetalles().isEmpty()) {
            ...
            listaold = detalleProgramacionMovimientoRepository.obtenerRegistrosPorProgramacionMovimimento(dto.getIdProgramacionMovimiento());
            for (DetalleProgramacionMovimientoEntity entityOld : listaold) {
                detalleProgramacionMovimientoRepository
                        .eliminar(detalleProgramacionMovimientoRepository.obtenerPorId(entityOld.getIdDetalleProgramacionMovimiento()));
            }
    ...
    public List<ProgramarMovimientoDTO> obtenerListaRegistros(Integer idTipoMovimieto) {
        ...
        if (entitys != null && !entitys.isEmpty()) {
            for (ProgramarMovimientoEntity en : entitys) {
                ...
                progdto.setMovimiento(en.getIdTipoMovimiento().getClave() + " " + en.getIdTipoMovimiento().getDescripcion());
