...
public class MovimientosContratosService {
    ...
    public boolean esTipoDeContratos(Integer idEmpleado) {
        ...
        Query query = session.createSQLQuery(
                " SELECT " + " tc.area_responsable " + " FROM tipos_contratacion AS tc " + " INNER JOIN configuraciones_presupuestales_empleados AS cpe "
                        + " ON cpe.id_tipo_contratacion = tc.id_tipo_contratacion " + " WHERE " + " cpe.id_empleado = :idEmpleado ")
    ...
    public List<MovimientoContratosDTO> obtenerMovimientosNominaEmpleadoLista(Integer idEmpleadoSeleccionado) {
        Session session = entityManager.unwrap(Session.class);
        Query query = session.createSQLQuery("CALL usp_movimientos_contratos_empleado(:idEmpleado)").setParameter("idEmpleado", idEmpleadoSeleccionado);
        ...
        if (!result.isEmpty()) {
            for (MovimientoContratosDTO mov : result) {
                ...
                listEntitys = entityManager.createQuery("Select d From DetalleMovimientosCotratosEntity AS d WHERE d.idMovimientoContrato=:idMovimiento",
                        DetalleMovimientosCotratosEntity.class).setParameter("idMovimiento", mov.getIdMovimientoContratos()).getResultList();
    ...
    public List<InfoMovimientoDTO> obtenerInfoMovimientos(Integer idNominaEmpleado) {
        ...
        if (!movimientos.isEmpty()) {
            for (MovimientosNominaContratosEntity ent : movimientos) {
                ...
                Session session = entityManager.unwrap(Session.class);
                Query query = session.createSQLQuery(" SELECT " + " dmc.id_detalle_movimiento AS idDetalleMovimiento, " + " MIN(dmc.numero_pago) AS numeroPago "
                        + " FROM detalle_movimientos_contratos AS dmc " + " WHERE " + " dmc.id_movimiento_contrato = :idMovimiento " + " AND "
                        + " dmc.id_estatus_detalle = 1 ").setParameter("idMovimiento", ent.getIdMovimiento());
    ...
    public void eliminarMovimiento(MovimientoContratosDTO dto) {
        ...
        List<DetalleMovimientosCotratosEntity> detalles = detalleMovimientosContratosRepository
                .obtenerDetalleMovimientosPorMovimiento(dto.getIdMovimientoContratos());
        if (!detalles.isEmpty()) {
