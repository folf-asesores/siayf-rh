...
public class MovimientosContratosService {
    ...
    @Inject
    private FaltaContadaRepository faltaContadaRepository;
    @Inject
    ...
    public List<MovimientoContratosDTO> obtenerMovimientosNominaEmpleadoLista(
        ...
        Query query = session
                .createSQLQuery(
                        "CALL usp_movimientos_contratos_empleado(:idEmpleado)")
                .setParameter("idEmpleado", idEmpleadoSeleccionado);
        query.setResultTransformer(
                Transformers.aliasToBean(MovimientoContratosDTO.class));
        @SuppressWarnings("unchecked")
        List<MovimientoContratosDTO> result = query.list();
        ...
        if (!result.isEmpty()) {
            for (MovimientoContratosDTO mov : result) {
                ...
                listEntitys = entityManager.createQuery(
                        ...
                        .setParameter("idMovimiento",
                                mov.getIdMovimientoContratos())
                        .getResultList();
                if (!listEntitys.isEmpty()) {
                    ...
                    for (DetalleMovimientosCotratosEntity detalleEnty : listEntitys) {
                        ...
                        dto.setIdMovimientoContrato(
                                detalleEnty.getIdMovimientoContrato());
                        dto.setIdDetalleMovimiento(
                                detalleEnty.getIdDetalleMovimiento());
    ...
    public MovimientosNominaContratosEntity crear(MovimientoContratosDTO dto) {
        try {
            ...
            ConceptoNominaContratosEntity conceptoNominaContrato = conceptoNominaContratosRepository
                    .obtenerPorId(dto.getIdConceptoContratos());
    ...
    public List<DetalleMovimientoContratoDTO> listaDetallesMovimientosPorMovimiento(
            Integer idMovimiento) {
        try {
            return detalleMovimientosContratosRepository
                    .obtenerDetallesPorMovimiento(idMovimiento);
    ...
    public List<InfoMovimientoDTO> obtenerInfoMovimientos(
        ...
        if (!movimientos.isEmpty()) {
            for (MovimientosNominaContratosEntity ent : movimientos) {
                System.out.println(
                        "ent.getIdMovimiento():: " + ent.getIdMovimiento());
                ...
                Query query = session.createSQLQuery(" SELECT "
                        ...
                        + " AND " + " dmc.id_estatus_detalle = 1 ")
                        .setParameter("idMovimiento", ent.getIdMovimiento());
                query.setResultTransformer(Transformers
                        .aliasToBean(DetalleMovimientoContratoDTO.class));
                DetalleMovimientoContratoDTO detalleMovimiento = (DetalleMovimientoContratoDTO) query
                        .uniqueResult();
                System.out.println("detalleMovimiento:: "
                        + detalleMovimiento.getIdDetalleMovimiento());
                DetalleMovimientosCotratosEntity detalle = null;
                if (detalleMovimiento != null
                        && detalleMovimiento.getIdDetalleMovimiento() != null) {
                    detalle = entityManager.find(
                            DetalleMovimientosCotratosEntity.class,
                            detalleMovimiento.getIdDetalleMovimiento());
                }

                infoDto.setIdConcepto(
                        ent.getConceptoNominaContrato().getIdConceptoNomina());
                ...
                infoDto.setTipoConcepto(
                        ent.getConceptoNominaContrato().getTipo());
                if (detalle != null) {
    ...
    public void eliminarMovimiento(MovimientoContratosDTO dto) {
        MovimientosNominaContratosEntity entity = movimientosNominaContratosRepository
                .obtenerPorId(dto.getIdMovimientoContratos());
        List<DetalleMovimientosCotratosEntity> detalles = detalleMovimientosContratosRepository
                .obtenerDetalleMovimientosPorMovimiento(
                        dto.getIdMovimientoContratos());
        if (!detalles.isEmpty()) {
            for (DetalleMovimientosCotratosEntity detalle : detalles) {
                detalleMovimientosContratosRepository.eliminar(detalle);
            }
        }
        List<FaltaContadaEntity> faltaContadaList = faltaContadaRepository
                .obtenerPorMovimientoContrato(entity);
    ...
    public void actualizarMovimientoContratos(MovimientoContratosDTO dto) {
        ...
        for (FaltaContadaDTO faltaContadaDTO : dto.getFaltaContadaList()) {
            if (!faltaContadaRepository.faltaEstaContada(entity,
                    faltaContadaDTO.getFechaFalta())) {
                FaltaContadaEntity faltaContadaEntity = new FaltaContadaEntity();
                faltaContadaEntity.setEmpleado(empleadoEntity);
                faltaContadaEntity
                        .setFechaFalta(faltaContadaDTO.getFechaFalta());
        ...
        for (FaltaContadaEntity faltaContadaEntity : faltaContadaList) {
            Boolean sigue = false;
            for (FaltaContadaDTO faltaContadaDTO : dto.getFaltaContadaList()) {
                if (faltaContadaEntity.getFechaFalta()
                        .equals(faltaContadaDTO.getFechaFalta())) {
                    sigue = true;
                    break;
                }
            }
    ...
    public MovimientoContratosDTO obtenerMovimientoContrato(
        ...
        MovimientosNominaContratosEntity entity = movimientosNominaContratosRepository
                .obtenerPorId(movimientoContratos.getIdMovimientoContratos());
        List<FaltaContadaEntity> faltaContadaList = faltaContadaRepository
                .obtenerPorMovimientoContrato(entity);
        ...
        if (!faltaContadaList.isEmpty()) {
            for (FaltaContadaEntity faltaContadaEntity : faltaContadaList) {
                ...
                faltaContadaDTO
                        .setFechaFalta(faltaContadaEntity.getFechaFalta());
                faltaContadaDTO.setIdFaltaContada(
                        faltaContadaEntity.getIdFaltaContada());
                if (faltaContadaEntity.getNominaEmpleado() != null) {
                    faltaContadaDTO.setIdNominaEmpleado(faltaContadaEntity
                            .getNominaEmpleado().getIdNominaEmpleado());
                }
                faltaContadaDTO.setIdMovimientoEventual(faltaContadaEntity
                        .getMovimientoNominaEventual().getIdMovimiento());
