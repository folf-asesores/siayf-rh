...
public class ProductosNominaService {
    ...
    @Inject
    private EstatusNominasEmpleadoRepository estatusNominaRepository;
    @Inject
    private PeriodoCalendariosRepository periodoCalendariosRepository;
    @Inject
    private FuenteFinanciamientoRepository fuenteFinanciamientoDAO;
    @Inject
    private SubFuenteFinanciamientoTempRepository subfuenteFinanciamientoTempRepository;
    @Inject
    private TipoNominaRepository tipoNominaRepository;
    @Inject
    private TipoContratacionRepository tipoContratacionRepository;
    @Inject
    private EstatusProductoNominaRepository estatusProductoNominaRepository;
    @Inject
    ...
    public List<ProductoNominaListaDTO> filtrarProductoNomina(ProductoNominaFiltroDTO filtro) {
        ...
        Query query = session
                .createSQLQuery("CALL usp_filtrar_productos_nomina(:ejercicioFiscal, :idSubfuenteFinanciamiento," + " :idTipoNomina, :idArea, :idEstatus) ")
                .setParameter("ejercicioFiscal", ejercicioFiscal).setParameter("idSubfuenteFinanciamiento", idSubfuenteFinanciamiento)
                .setParameter("idTipoNomina", idTipoNomina).setParameter("idArea", filtro.getIdArea()).setParameter("idEstatus", idEstatus);
        query.setResultTransformer(Transformers.aliasToBean(ProductoNominaListaDTO.class));
        @SuppressWarnings("unchecked")
        List<ProductoNominaListaDTO> result = query.list();
        return result;
    }
    ...
    public ProductoNominaDTO obtenerProductoNomina(Integer idProductoNomina) {
        Session session = entityManager.unwrap(Session.class);
        Query query = session
                .createSQLQuery(" SELECT " + " pdn.id_producto_nomina 				AS idProductoNomina, "
                        + " pdn.nombre_producto 				AS nombreProducto, " + " pdn.id_ejercicio_fiscal 			AS idEjercicioFiscal, "
                        + " pdn.id_tipo_periodo 				AS idTipoPeriodo, " + " pdn.id_periodo_calendario 			AS idPeriodoCalendario, "
                        + " pdn.id_fuente_financiamiento 		AS idFuenteFinanciamiento, "
                        + " pdn.id_subfuente_financiamiento 	AS idSubfuenteFinanciamiento, " + " pdn.id_tipo_nomina 					AS idTipoNomina, "
                        + " pdn.id_estatus_producto_nomina 		AS idEstatusProductoNomina, " + " pdn.id_banco                		AS idBanco, "
                        + " pdn.id_cuenta_bancaria          	AS idCuentaBancaria, " + " pdn.numero_inicio_cheques           AS numeroInicioCheques, "
                        + " pdn.id_usuario 						AS idUsuario, " + " pdn.ejercicio_fiscal 				AS ejercicioFiscal, "
                        + " pdn.numero_periodo 					AS numeroPeriodo, " + " pdn.inicio_periodo 					AS inicioPeriodo, "
                        + " pdn.fin_periodo 					AS finPeriodo, " + " pdn.inicio_rango_faltas 			AS inicioRangoFaltas, "
                        + " pdn.fin_rango_faltas 				AS finRangoFaltas, " + " pdn.fecha_pago 						AS fechaPago, "
                        + " pdn.id_tipo_contratacion 			AS idTipoContratacion, " + " tn.descripcion 						AS tipoNomina, "
                        + " spn.estatus 						AS estatusProductoNomina, " + " tc.tipo_contratacion 				AS tipoContratacion, "
                        + " tc.area_responsable 				AS idArea, " + " tp.tipo_periodo 					AS tipoPeriodo,"
                        + " pdn.dias_prima_vacasional           AS diasPrimaVacasional, " + " pdn.dias_aguinaldo                  AS diasAguinaldo, "
                        + " pdn.dias_exento_prima_vacasional    AS diasExentoPrimaVacasional, " + " pdn.dias_exento_aguinaldo           AS diasExentoAguinaldo "
                        + " FROM productos_nomina 				AS pdn " + " INNER JOIN tipos_nominas 			AS tn "
                        + "  ON tn.id_tipo_nomina = pdn.id_tipo_nomina " + " INNER JOIN estatus_productos_nomina AS spn "
                        + "  ON spn.id_estatus_producto_nomina = pdn.id_estatus_producto_nomina " + " INNER JOIN tipos_contratacion 		AS tc "
                        + "  ON tc.id_tipo_contratacion = pdn.id_tipo_contratacion " + " INNER JOIN tipos_periodos 			AS tp "
                        + "  ON tp.id_tipo_periodo = pdn.id_tipo_periodo " + " WHERE " + " pdn.id_producto_nomina = :idProductoNomina ")
        ...
        @SuppressWarnings("unchecked")
        List<ProductoNominaDTO> result = query.list();
    ...
    public void actualizarProductoNomina(ProductoNominaDTO dto) {
        try {
            ...
            if (dto.getIdSubfuenteFinanciamiento() != null && dto.getIdSubfuenteFinanciamiento() > 0) {
                ...
                entity.setSubfuenteFinanciamiento(subfuenteFinanciamientoTempRepository.obtenerPorId(dto.getIdSubfuenteFinanciamiento()));
            }
            ...
            entity.setEjercicioFiscal(ejercicioFiscalRepository.obtenerEjercioPorEjercicionFiscal(dto.getEjercicioFiscal(), dto.getIdTipoPeriodo()));
            ...
            entity.setEstatusProductoNomina(estatusProductoNominaRepository.obtenerPorId(dto.getIdEstatusProductoNomina()));
    ...
    public void abrirProductoNomina(ProductoNominaDTO productoNomina) {
        Session session = entityManager.unwrap(Session.class);
        Query query = session.createSQLQuery("CALL usp_generar_estructura_nomina(:idProductoNomina) ").setParameter("idProductoNomina",
    ...
    public List<EstatusProductoNominaDTO> obtenerEstatusProductoNominaLista() {
        Session session = entityManager.unwrap(Session.class);
        Query query = session
                .createSQLQuery(" SELECT " + " id_estatus_producto_nomina AS idEstatusConceptoNomina, " + " estatus " + " FROM estatus_productos_nomina ");
        query.setResultTransformer(Transformers.aliasToBean(EstatusProductoNominaDTO.class));
        @SuppressWarnings("unchecked")
        List<EstatusProductoNominaDTO> result = query.list();
        return result;
    }

    public List<TipoContratacionDTO> listaTipoContratacionListPorArea(Integer tipoArea) {
        Session session = entityManager.unwrap(Session.class);
        Query query = session.createSQLQuery(" SELECT " + " id_tipo_contratacion AS idTipoContratacion, " + " tipo_contratacion AS tipoContratacion "
                + " FROM tipos_contratacion " + " WHERE " + " area_responsable = :tipoArea ").setParameter("tipoArea", tipoArea);
        query.setResultTransformer(Transformers.aliasToBean(TipoContratacionDTO.class));
        @SuppressWarnings("unchecked")
        List<TipoContratacionDTO> result = query.list();
        return result;
    }

    public void aplicarConsecutivosProductoNomina(ProductoNominaDTO productoNomina) {
        Session session = entityManager.unwrap(Session.class);
        Query query = session.createSQLQuery("CALL usp_actualizar_consecutivo_nomina_empleado(:idProductoNomina) ").setParameter("idProductoNomina",
                productoNomina.getIdProductoNomina());
        query.executeUpdate();
    }
    ...
    public List<ProductoNominaDTO> obtenerProductoNominaLista(List<NominaEmpleadoDTO> nominaEmpleadoLista) {
        ...
        for (NominaEmpleadoDTO nominaEmpleado : nominaEmpleadoLista) {
            Query query = session.createSQLQuery(" SELECT " + " pdn.id_producto_nomina AS idProductoNomina, " + " pdn.nombre_producto AS nombreProducto, "
                    + " pdn.id_ejercicio_fiscal AS idEjercicioFiscal, " + " pdn.id_tipo_periodo AS idTipoPeriodo, "
                    + " pdn.id_periodo_calendario AS idPeriodoCalendario, " + " pdn.id_fuente_financiamiento AS idFuenteFinanciamiento, "
                    + " pdn.id_subfuente_financiamiento AS idSubfuenteFinanciamiento, " + " pdn.id_tipo_nomina AS idTipoNomina, "
                    + " pdn.id_estatus_producto_nomina AS idEstatusProductoNomina, " + " pdn.id_usuario AS idUsuario, "
                    + " pdn.ejercicio_fiscal AS ejercicioFiscal, " + " pdn.numero_periodo AS numeroPeriodo, " + " pdn.inicio_periodo AS inicioPeriodo, "
                    + " pdn.fin_periodo AS finPeriodo, " + " pdn.inicio_rango_faltas AS inicioRangoFaltas, " + " pdn.fin_rango_faltas AS finRangoFaltas, "
                    + " pdn.fecha_pago AS fechaPago, " + " pdn.id_tipo_contratacion AS idTipoContratacion " + " FROM productos_nomina AS pdn " + " WHERE "
                    + " pdn.id_producto_nomina = :idEmpleado ").setParameter("idEmpleado", nominaEmpleado.getIdProductoNomina());
            query.setResultTransformer(Transformers.aliasToBean(ProductoNominaDTO.class));
            @SuppressWarnings("unchecked")
            List<ProductoNominaDTO> resultTem = query.list();
    ...
    public List<CuentaBancariaDTO> obtenerCuentaBancariaList(ProductoNominaDTO productoNomina) {
        Session session = entityManager.unwrap(Session.class);
        Query query = session.createSQLQuery("" + " SELECT " + " id_cuenta_bancaria AS idCuentaBancaria, " + " banco AS banco, "
                + " numero_cuenta AS numeroCuenta, " + " descripcion AS descripcion, " + " fuente_financiamiento AS fuenteFinanciamiento, "
                + " ejercicio_fiscal AS ejercicioFiscal, " + " clave_cuenta AS claveCuenta " + " FROM cuentas_bancarias ");
    ...
    public List<BancoDTO> obtenerBancoList() {
        Session session = entityManager.unwrap(Session.class);
        Query query = session.createSQLQuery("" + " SELECT " + " id_banco_sat AS idBanco, " + " clave AS clave, " + " nombre_corto AS nombreCorto, "
                + " razon_social AS razonSocial " + " FROM bancos_sat ");
        query.setResultTransformer(Transformers.aliasToBean(BancoDTO.class));
        @SuppressWarnings("unchecked")
        List<BancoDTO> result = query.list();
        return result;
    }

    public void eliminarProductoNomina(ProductoNominaDTO productoNomina) {
        Session session = entityManager.unwrap(Session.class);
        Query query = session.createSQLQuery(" SELECT " + " ne.id_nomina_empleado AS idNominaEmpleado " + " FROM nomina_empleado AS ne " + " WHERE "
                + " ne.id_producto_nomina = :idProductoNomina ").setParameter("idProductoNomina", productoNomina.getIdProductoNomina());
        query.setResultTransformer(Transformers.aliasToBean(NominaEmpleadoDTO.class));

        @SuppressWarnings("unchecked")
        List<NominaEmpleadoDTO> nominaEmpleadoList = query.list();
