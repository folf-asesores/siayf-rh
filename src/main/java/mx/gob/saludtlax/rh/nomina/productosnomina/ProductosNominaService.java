...
public class ProductosNominaService {
    ...
    private static final Logger LOGGER = Logger
            .getLogger(ProductosNominaService.class);
    ...
    public ProductoNominaDTO crearProductoNomina(ProductoNominaDTO datos) {
        try {
            ...
            entity.setTipoContratacion(tipoContratacionRepository
                    .obtenerPorId(datos.getIdTipoContratacion()));
            entity.setEjercicioFiscal(
                    ejercicioFiscalRepository.obtenerEjercioPorEjercicionFiscal(
                            datos.getEjercicioFiscal(),
                            datos.getIdTipoPeriodo()));
            entity.setAnyoEjercicioFiscal(datos.getEjercicioFiscal());
            entity.setTipoPeriodo(tipoPeriodoRepository
                    .obtenerPorId(datos.getIdTipoPeriodo()));
            entity.setPeriodoCalendario(periodoCalendariosRepository
                    .obtenerPorId(datos.getIdPeriodoCalendario()));
            ...
            entity.setEstatusProductoNomina(estatusProductoNominaRepository
                    .obtenerPorId(datos.getIdEstatusProductoNomina()));
            entity.setFechaPago(datos.getFechaPago());
            entity.setTipoNomina(
                    tipoNominaRepository.obtenerPorId(datos.getIdTipoNomina()));
            entity.setFechaPago(datos.getFechaPago());
            entity.setUsuario(
                    usuarioRepository.obtenerPorId(datos.getIdUsuario()));
    ...
    public List<ProductoNominaListaDTO> filtrarProductoNomina(
        ...
        Integer idTipoNomina = (filtro.getIdTipoNomina() != null
                ? filtro.getIdTipoNomina() : 0);
        Integer idEstatus = (filtro.getIdEstatus() != null
                ? filtro.getIdEstatus() : 0);
        Integer idSubfuenteFinanciamiento = (filtro
                .getIdSubfuenteFinanciamiento() != null
                        ? filtro.getIdSubfuenteFinanciamiento() : 0);
        Integer ejercicioFiscal = (filtro.getEjercicioFiscal() != null
                && filtro.getEjercicioFiscal() > 0 ? filtro.getEjercicioFiscal()
                        : 0);

        Query query = session.createSQLQuery(
                "CALL usp_filtrar_productos_nomina(:ejercicioFiscal, :idSubfuenteFinanciamiento,"
                        + " :idTipoNomina, :idArea, :idEstatus) ")
                .setParameter("ejercicioFiscal", ejercicioFiscal)
                .setParameter("idSubfuenteFinanciamiento",
        ...
        query.setResultTransformer(
                Transformers.aliasToBean(ProductoNominaListaDTO.class));
    ...
    public ProductoNominaDTO obtenerProductoNomina(Integer idProductoNomina) {
        ...
        Query query = session.createSQLQuery(" SELECT "
                ...
                + "  ON tp.id_tipo_periodo = pdn.id_tipo_periodo " + " WHERE "
    ...
    public void actualizarProductoNomina(ProductoNominaDTO dto) {
        try {
            ProductoNominaEntity entity = productoNominaRepository
                    .obtenerPorId(dto.getIdProductoNomina());

            entity.setNombreProducto(dto.getNombreProducto());
            entity.setTipoContratacion(tipoContratacionRepository
                    .obtenerPorId(dto.getIdTipoContratacion()));
            if (dto.getIdSubfuenteFinanciamiento() != null
                    && dto.getIdSubfuenteFinanciamiento() > 0) {
                entity.setCambiarSubfuenteFinanciamiento(
                        dto.getCambiarFuenteFinanciamiento());
                entity.setSubfuenteFinanciamiento(
                        subfuenteFinanciamientoTempRepository.obtenerPorId(
                                dto.getIdSubfuenteFinanciamiento()));
            }
            if (dto.getIdFuenteFinanciamiento() != null
                    && dto.getIdFuenteFinanciamiento() > 0) {
                entity.setFuenteFinanciamiento(fuenteFinanciamientoDAO
            ...
            if (dto.getIdBanco() != null && dto.getIdBanco() > 0) {
                entity.setBanco(
                        bancoSatRepository.obtenerPorId(dto.getIdBanco()));
            }
            if (dto.getIdCuentaBancaria() != null
                    && dto.getIdCuentaBancaria() > 0) {
                entity.setCuentaBancaria(cuentasBancariasRepository
                        .obtenerPorId(dto.getIdCuentaBancaria()));
            }

            entity.setEjercicioFiscal(
                    ejercicioFiscalRepository.obtenerEjercioPorEjercicionFiscal(
                            dto.getEjercicioFiscal(), dto.getIdTipoPeriodo()));
            entity.setAnyoEjercicioFiscal(dto.getEjercicioFiscal());
            entity.setTipoPeriodo(
                    tipoPeriodoRepository.obtenerPorId(dto.getIdTipoPeriodo()));
            entity.setPeriodoCalendario(periodoCalendariosRepository
                    .obtenerPorId(dto.getIdPeriodoCalendario()));
            ...
            entity.setEstatusProductoNomina(estatusProductoNominaRepository
                    .obtenerPorId(dto.getIdEstatusProductoNomina()));
            entity.setTipoNomina(
                    tipoNominaRepository.obtenerPorId(dto.getIdTipoNomina()));
            entity.setFechaPago(dto.getFechaPago());
            entity.setUsuario(
                    usuarioRepository.obtenerPorId(dto.getIdUsuario()));

            entity.setDiasAguinaldo(dto.getDiasAguinaldo());
            entity.setDiasExentoAguinaldo(dto.getDiasExentoAguinaldo());
            entity.setDiasExentoPrimaVacasional(
                    dto.getDiasExentoPrimaVacasional());
    ...
    public void abrirProductoNomina(ProductoNominaDTO productoNomina) {
        Session session = entityManager.unwrap(Session.class);
        Query query = session.createSQLQuery(
                "CALL usp_generar_estructura_nomina(:idProductoNomina) ")
    ...
    @TransactionAttribute(TransactionAttributeType.REQUIRES_NEW)
    public void abrirProductoNominaSuplencia(Integer idProductoNomina,
            QuincenasSuplenciasEntity q) {

        EstatusNominasEmpleadoEntity estatusNomina = estatusNominaRepository
                .obtenerPorId(1);
        ProductoNominaEntity productoNomina = productoNominaRepository
                .obtenerPorId(idProductoNomina);
        TipoContratacionEntity tipoContratacion = tipoContratacionRepository
                .obtenerPorId(EnumTipoContratacion.SUPLENCIA);
        TiposNombramientosEntity tipoNombramiento = tiposNombramientoRepository
        ...
        nomina.setIdCentroResponsabilidad(
                q.getSuplente().getCentroResponsabilidad());
        ...
        nomina.setNumeroIdPersonal(
                q.getSuplente().getEmpleado().getNumeroEmpleado());
    ...
    public List<EstatusProductoNominaDTO> obtenerEstatusProductoNominaLista() {
        Session session = entityManager.unwrap(Session.class);
        Query query = session.createSQLQuery(" SELECT "
                + " id_estatus_producto_nomina AS idEstatusConceptoNomina, "
                + " estatus " + " FROM estatus_productos_nomina ");
        query.setResultTransformer(
                Transformers.aliasToBean(EstatusProductoNominaDTO.class));
        @SuppressWarnings("unchecked")
        List<EstatusProductoNominaDTO> result = query.list();
        return result;
    }

    public List<TipoContratacionDTO> listaTipoContratacionListPorArea(
            Integer tipoArea) {
        Session session = entityManager.unwrap(Session.class);
        Query query = session
                .createSQLQuery(" SELECT "
                        + " id_tipo_contratacion AS idTipoContratacion, "
                        + " tipo_contratacion AS tipoContratacion "
                        + " FROM tipos_contratacion " + " WHERE "
                        + " area_responsable = :tipoArea ")
                .setParameter("tipoArea", tipoArea);
        query.setResultTransformer(
                Transformers.aliasToBean(TipoContratacionDTO.class));
        @SuppressWarnings("unchecked")
        List<TipoContratacionDTO> result = query.list();
        return result;
    }

    public void aplicarConsecutivosProductoNomina(
            ProductoNominaDTO productoNomina) {
        Session session = entityManager.unwrap(Session.class);
        Query query = session.createSQLQuery(
                "CALL usp_actualizar_consecutivo_nomina_empleado(:idProductoNomina) ")
                .setParameter("idProductoNomina",
                        productoNomina.getIdProductoNomina());
        query.executeUpdate();
    }
    ...
    public List<ProductoNominaDTO> obtenerProductoNominaLista(
            List<NominaEmpleadoDTO> nominaEmpleadoLista) {
        Session session = entityManager.unwrap(Session.class);
        ...
        for (NominaEmpleadoDTO nominaEmpleado : nominaEmpleadoLista) {
            Query query = session.createSQLQuery(" SELECT "
                    ...
                    + " pdn.fecha_pago AS fechaPago, "
                    + " pdn.id_tipo_contratacion AS idTipoContratacion "
                    + " FROM productos_nomina AS pdn " + " WHERE "
                    + " pdn.id_producto_nomina = :idEmpleado ").setParameter(
                            "idEmpleado", nominaEmpleado.getIdProductoNomina());
            query.setResultTransformer(
                    Transformers.aliasToBean(ProductoNominaDTO.class));
            @SuppressWarnings("unchecked")
            ...
            productoNominaDTO
                    .setIdNominaEmpleado(nominaEmpleado.getIdNominaEmpleado());
    ...
    public void validarProductoNomina(ProductoNominaDTO productoNomina) {
        ...
        EstatusProductoNominaEntity estatusProductoNominaEntity = estatusProductoNominaRepository
                .obtenerPorId(5);
    ...
    public void aplicarChequesProductoNomina(ProductoNominaDTO productoNomina) {
        Session session = entityManager.unwrap(Session.class);
        Query query = session.createSQLQuery(
                "CALL usp_actualizar_numero_cheque_nomina_empleado(:idProductoNomina,:numeroInicioCheques) ")
                .setParameter("idProductoNomina",
                        productoNomina.getIdProductoNomina())
                .setParameter("numeroInicioCheques",
                        productoNomina.getNumeroInicioCheques());
        query.executeUpdate();
    }
    ...
    public List<BancoDTO> obtenerBancoList() {
        Session session = entityManager.unwrap(Session.class);
        Query query = session.createSQLQuery("" + " SELECT "
                + " id_banco_sat AS idBanco, " + " clave AS clave, "
    ...
    public void eliminarProductoNomina(ProductoNominaDTO productoNomina) {
        ...
        // producto_nomina
        productoNominaRepository
                .eliminarPorId(productoNomina.getIdProductoNomina());
    }
    ...
    public void eliminarNominaEmpleado(Integer idNominaEmpleado) {
        ...
        for (ConceptosNominaEmpleadosEntity conceptosNominaEmpleadosEntity : conceptosNominaEmpleadosEntities) {
            conceptosNominaEmpleadosRepository
                    .eliminar(conceptosNominaEmpleadosEntity);
        }
