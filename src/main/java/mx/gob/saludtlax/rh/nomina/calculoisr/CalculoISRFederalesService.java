...
public class CalculoISRFederalesService {
	...
	@Inject
	ConceptosNominaEspecialRepository conceptosNominaEspecialRepository;
	...
	public IsrDTO calcularIsrEmpleado(ConfiguracionIsrDTO configuracionIsr) {
		...
		if (configuracionIsr.getBaseGravable() == null
				|| configuracionIsr.getBaseGravable().compareTo(BigDecimal.ZERO) < 1) {
			...
			isr.setPercepcionEfectiva(
					configuracionIsr.getBaseGravable().subtract(impuestoRetener).subtract(subsidioEntregar));
		}
		return isr;
	}
	
	private void calculoISRCompensaciones(){
	...
	private TarifaIsrDTO obtenerTarifaIsr(ConfiguracionIsrDTO baseGravable) {
		...
		Query query = session
				.createSQLQuery("SELECT " + " limite_inferior AS limiteInferior, "
						+ " porcentaje_aplicable AS porcentajeAplicable, " + " cuota_fija AS cuotaFija "
						+ " FROM tarifas_retenciones " + " WHERE " + " id_tipo_periodo = :idTipoPeriodo " + " AND "
		...
		List<TarifaIsrDTO> result = (List<TarifaIsrDTO>) query.list();
	...
	public ConfiguracionIsrDTO obtenerConfiguracioIsr(Integer idEmpleado) {
		Session session = entityManager.unwrap(Session.class);
		Query query = session
				.createSQLQuery("SELECT sueldo AS baseGravable, " + " id_tipo_periodo AS idTipoPeriodo"
						+ " FROM configuraciones_presupuestales_empleados " + " WHERE id_empleado = :idEmpleado")
				.setParameter("idEmpleado", idEmpleado);
		query.setResultTransformer(Transformers.aliasToBean(ConfiguracionIsrDTO.class));
		@SuppressWarnings("unchecked")

		List<ConfiguracionIsrDTO> result = (List<ConfiguracionIsrDTO>) query.list();
		...
		if (configuracionIsrDTO.getBaseGravable() == null) {
			query = session
					.createSQLQuery("SELECT " + " sueldo_bruto_mensual " + " FROM tabuladores "
	...
	private BigDecimal obtenerSubsidio(ConfiguracionIsrDTO configuracionIsr) {
		Session session = entityManager.unwrap(Session.class);
		Query query = session
				.createSQLQuery("SELECT " + " subsidio " + " FROM tablas_subsidios " + " WHERE "
						+ " id_tipo_periodo = :idTipoPeriodo " + " AND " + " ( :baseGravable BETWEEN "
						+ "	limite_inferior AND limite_superior) ")
				.setParameter("idTipoPeriodo", configuracionIsr.getIdTipoPeriodo())
				.setParameter("baseGravable", configuracionIsr.getBaseGravable());
		@SuppressWarnings("unchecked")
