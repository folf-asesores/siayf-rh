...
public class CalculoAguinaldoService implements Serializable {
    ...
    public AguinaldoResult calcular(AguinaldoParams aguinaldoParams) {
        ...
        BigDecimal salarioDario = salarioMensual.divide(new BigDecimal("30"),
                4);
        // Ahora sabemos cualto le corresponde de aguinaldo completo
        BigDecimal aguinaldoCompleto = salarioDario
                .multiply(aguinaldoParams.getDiasPagar());
        // POr ultimo tenemos cuando le corresponde por dia trabajado de
        // aguinaldo.
        BigDecimal aguinaldoPorDia = aguinaldoCompleto
                .divide(new BigDecimal(TOTAL_DIAS_PAGAR_AGUINALDO), 4);

        BigDecimal aguinaldo = null;
        BigDecimal diasPagarAguinaldo = null;
        BigDecimal importeExcentoAguinaldo = calcularImporteExcento(
                aguinaldoParams.getDiasExentoPagar());
        ...
        if (aguinaldoParams.isCalculoFiniquito()) {
            if (aguinaldoParams.getFechaCalculo() == null) {
                throw new ReglaNegocioException(
                        "Debe enviar la fecha con la que realzara el calculo",
                        ReglaNegocioCodigoError.CONFIGURACION_NO_EXISTE);
            }
            diasPagarAguinaldo = new BigDecimal(
                    calcularDiasPagarFiniquito(aguinaldoParams.getIdEmpleado(),
                            aguinaldoParams.getFechaCalculo()));
        } else {
            diasPagarAguinaldo = new BigDecimal(calcularDiasPagarAnual(
                    aguinaldoParams.getIdEmpleado(),
                    FechaUtil.anioFecha(aguinaldoParams.getFechaCalculo())));
        }
        ...
        if (aguinaldo.compareTo(importeExcentoAguinaldo) > 0) {
            ...
            aguinaldoResult
                    .setGravado(aguinaldo.subtract(importeExcentoAguinaldo));
        } else {
    ...
    private Integer calcularDiasPagarAnual(Integer idEmpleado, Integer anio) {
        BigInteger diasDescontar = (BigInteger) entityManager
                .createNativeQuery(
                        "CALL usp_dias_descontar_anyo (:idEmpleado, :anio);")
                .setParameter("idEmpleado", idEmpleado)
                .setParameter("anio", anio).getSingleResult();
        Integer diasAguinaldo = TOTAL_DIAS_PAGAR_AGUINALDO
                - diasDescontar.intValue();
        return diasAguinaldo;
    }

    private Integer calcularDiasPagarFiniquito(Integer idEmpleado, Date fecha) {

        BigInteger diasTrabajados = (BigInteger) entityManager
                .createNativeQuery(
                        "CALL ups_total_dias_trabajados_aguinaldo (:fecha,:idEmpleado);")
