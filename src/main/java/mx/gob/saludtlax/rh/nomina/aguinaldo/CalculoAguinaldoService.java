...
//		}
		...
		if (aguinaldoParams.isCalculoFiniquito()) {
			if (aguinaldoParams.getFechaCalculo() == null) {
				throw new ReglaNegocioException("Debe enviar la fecha con la que realzara el calculo",
						ReglaNegocioCodigoError.CONFIGURACION_NO_EXISTE);
			}
			diasPagarAguinaldo = new BigDecimal(
					calcularDiasPagarFiniquito(aguinaldoParams.getIdEmpleado(), aguinaldoParams.getFechaCalculo()));
		} else {
			diasPagarAguinaldo = new BigDecimal(calcularDiasPagarAnual(aguinaldoParams.getIdEmpleado(), FechaUtil.anioFecha(aguinaldoParams.getFechaCalculo())));
		}
	...
	private Integer calcularDiasPagarAnual(Integer idEmpleado, Integer anio) {
		BigInteger diasDescontar = (BigInteger) entityManager
				.createNativeQuery("CALL usp_dias_descontar_anyo (:idEmpleado, :anio);")
	...
	private Integer calcularDiasPagarFiniquito(Integer idEmpleado, Date fecha) {
		...
		BigInteger diasTrabajados = (BigInteger) entityManager
				.createNativeQuery("CALL ups_total_dias_trabajados_aguinaldo (:fecha,:idEmpleado);")
