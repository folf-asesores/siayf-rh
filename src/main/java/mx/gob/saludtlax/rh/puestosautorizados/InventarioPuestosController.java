...
public class InventarioPuestosController implements Serializable {
	...
	@PostConstruct
	public void inicio() {
		this.view.setInventarios(vacante.consultarInventariosVacantes());
		this.view.setMostrarInventario(true);
	}
	...
	public void descargarContratoProyeccion(Integer idTipoContratacion) {
		try {
			...
			String[] parametros = { "ID_USUARIO", String.valueOf(usuario.getIdUsuario()), "REPORTE_NOMBRE",
					"detalle_empleado", "TIPO_REPORTE", "xlsx", "ID_TIPO_CONTRATACION",
			...
			if (bytes != null) {
				JSFUtils.descargarArchivo(bytes, CadenaUtil.converterSpace("Detalles_Empleados"),
