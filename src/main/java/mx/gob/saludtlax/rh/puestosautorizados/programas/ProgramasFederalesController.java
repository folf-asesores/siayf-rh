...
public class ProgramasFederalesController implements Serializable {
        ...
        if (view.getNuevoPrograma().getIdDependencia() != 0) {
            view.setListaUnidadesResponsables(SelectItemsUtil.listaCatalogos(
                    catalogo.listaUnidadesResponsablesPorDependencia(
                            view.getNuevoPrograma().getIdDependencia())));
        }
    ...
    public void obtenerUnidadesResponsablesEdicionPrograma() {
        ...
        if (view.getEdicionPrograma().getIdDependencia() != null
            ...
            view.setListaUnidadesResponsables(SelectItemsUtil.listaCatalogos(
                    catalogo.listaUnidadesResponsablesPorDependencia(
                            view.getEdicionPrograma().getIdDependencia())));
        }
    ...
    public void registrarDetallePrograma() {
        try {
            ...
            view.getDetallePrograma()
                    .setIdTipoDetalle(EnumTipoDetallePrograma.PARTIDA);
            programa.crearDetallePrograma(view.getDetallePrograma());
            view.setDetalles(programa.consultarDetallesProgramas(
                    view.getDetallePrograma().getIdPrograma()));
    ...
    public void onRowEdit(RowEditEvent event) {
        FacesMessage msg = new FacesMessage("Car Edited",
                ((DetalleProgramaDTO) event.getObject()).getIdDetalle() + "");
    ...
    public void calcularTotal() {
        ...
        if (ValidacionUtil
                .esNumeroPositivo(view.getDetallePrograma().getNumeroPersonas())
                && ValidacionUtil.esNumeroPositivo(
                        view.getDetallePrograma().getCostoUnitario())) {

            BigDecimal numero = new BigDecimal(
                    view.getDetallePrograma().getNumeroPersonas());
            BigDecimal total = numero
                    .multiply(view.getDetallePrograma().getCostoUnitario());
    ...
    public void seleccionarEdicionPrograma(Integer idPrograma) {
        ...
        view.setEdicionPrograma(
                programa.obtenerDetalleProgramaPorId(idPrograma));
