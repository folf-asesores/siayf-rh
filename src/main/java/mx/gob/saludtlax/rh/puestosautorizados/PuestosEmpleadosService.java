...
public class PuestosEmpleadosService {
	...
	protected void ubicarEmpleado(UbicacionEmpleadoDTO ubicacionEmpleadoDTO) {
		...
		if (puesto == null) {
			throw new ReglaNegocioException("El puesto con identificador "
					+ ubicacionEmpleadoDTO.getIdInventarioVacante() + " no se encuentra registrado",
					ReglaNegocioCodigoError.SIN_REGISTRO);
		}

		if (puesto.getDisponible().equals("SI")) {
			throw new ReglaNegocioException("El puesto no tiene asignado un empleado.",
			...
			if (puesto.getSubadscripcion() != null) {
				if (!puesto.getSubadscripcion().getIdSubadscripcion()
					...
					SubadscripcionEntity areaAdscripcionEntity = areaAdscripcionRepository
							.obtenerPorId(ubicacionEmpleadoDTO.getIdSubadscripcion());
				...
				if (datoLaboral.getCentroResponsabilidad() != null) {
					if (!datoLaboral.getCentroResponsabilidad().getIdCentroResponsabilidad()
						...
						lccAnterior = lccAnterior + " CENTRO RESPONSABILIDAD: "
								+ puesto.getConfiguracion().getCentroResponsabilidad().getDescripcion();
	...
	public void modificacionPrograma(Integer idPuesto, Integer idPrograma) {
		...
		if (puesto == null) {
			throw new SistemaException("No se encontró el puesto con identificador " + idPuesto,
		...
		if (programa == null) {
			throw new SistemaException("No se encontró el programa con identificador " + idPrograma,
					SistemaCodigoError.BUSQUEDA_SIN_RESULTADOS);
		}
