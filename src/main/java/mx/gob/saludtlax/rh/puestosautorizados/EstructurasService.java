...
public class EstructurasService {
	...
	protected void actualizarEstructuraNomina(EstructuraNominaDTO dto) {
		...
		if (puesto == null) {
			throw new SistemaException("El puesto con identificador " + dto.getIdPuesto() + " no existe.",
					SistemaCodigoError.BUSQUEDA_SIN_RESULTADOS);
		}

		if (!puesto.getEstatus().isPuestoActivo()) {
			throw new ReglaNegocioException("No se puede modificar la estructura de un puesto que no esté activo.",
	...
	protected EstructuraNominaDTO obtenerEstructuraNominaPuesto(Integer idPuesto) {
		...
		if (puesto == null) {
			throw new SistemaException("El puesto con identificador " + idPuesto + " no existe.",
					SistemaCodigoError.BUSQUEDA_SIN_RESULTADOS);
		}

		if (puesto.getTipoContratacion().getAreaResponsable() != 1) {
			throw new ReglaNegocioException("Para obtener la estructura nómina el puesto debe ser federal.",
