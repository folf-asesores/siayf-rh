		...
		try {
			...
			if (ValidacionUtil.esNumeroPositivo(tipoContratacion)) {
				view.setSalario(tabulador.obtenerSueldoPorPuestoTipoTabulador(
						view.getSolicitud().getDatosLaborales().getIdPuesto(), tipoContratacion));
		...
		if (tipoContratacion != 0) {
			...
			if (tipoContratacion == EnumTipoContratacion.FORMALIZADOS
				...
				List<SelectItem> listaTiposNombramiento = SelectItemsUtil
						.listaCatalogos(catalogo.consultarNombramientosPorContratacion(
								view.getSolicitud().getDatosLaborales().getTipoContratacion()));
	...
	public void seleccionarPuestoDisponibleInterinato() {
		if (ValidacionUtil.esNumeroPositivo(view.getSolicitud().getIdPuestoDisponible())) {
			...
			DatoLaboralDTO datosLaboralesPuestoDisponible = configuracionPresupuestal
					.obtenerDatosLaboralesPuesto(view.getSolicitud().getIdPuestoDisponible());
			...
			view.getSolicitud().getDatosLaborales()
					.setIdUnidadResponsable(datosLaboralesPuestoDisponible.getIdUnidadResponsable());
			view.getSolicitud().getDatosLaborales()
					.setIdFuenteFinanciamiento(datosLaboralesPuestoDisponible.getIdFuenteFinanciamiento());
			obtenerSubfuentesFinanciamiento();
			view.getSolicitud().getDatosLaborales()
					.setIdSubfuenteFinanciamiento(datosLaboralesPuestoDisponible.getIdSubfuenteFinanciamiento());
	...
	public void obtenerDetallesPrograma() {
		...
		if (view.getSolicitud().getDatosLaborales().getIdPrograma() != 0) {
			view.setListaDetallesPrograma(SelectItemsUtil.listaCatalogos(
					catalogo.consultarListaDetallesPrograma(view.getSolicitud().getDatosLaborales().getIdPrograma())));
	...
	public void obtenerUnidadesResponsables() {
		...
		if (view.getSolicitud().getDatosLaborales().getIdDependencia() != 0) {
			view.setListaUnidadesResponsables(
					SelectItemsUtil.listaCatalogos(catalogo.listaUnidadesResponsablesPorDependencia(
							view.getSolicitud().getDatosLaborales().getIdDependencia())));
		}
		...
		if (view.getSolicitud().getDatosLaborales().getIdFuenteFinanciamiento() != 0) {
			view.setListaSubfuentesFinanciamiento(
					SelectItemsUtil.listaCatalogos(catalogo.listaSubfuentesFinanciamientosPorFinanciamiento(
							view.getSolicitud().getDatosLaborales().getIdFuenteFinanciamiento())));
		}
	}
