...
public class ProgramaService {
    ...
    private void validarPrograma(ProgramaDTO programa) {
        if (programa == null) {
            throw new ValidacionException(
                    "Los datos del programa son requeridos.",
        ...
        if (!ValidacionUtil.esNumeroPositivo(programa.getIdProyecto())) {
            throw new ValidacionException("El proyecto es requerido.",
                    ValidacionCodigoError.VALOR_REQUERIDO);
        }
        if (!ValidacionUtil.esNumeroPositivo(programa.getIdDependencia())) {
            throw new ValidacionException("La dependencia es requerida.",
                    ValidacionCodigoError.VALOR_REQUERIDO);
        }
        if (!ValidacionUtil
                .esNumeroPositivo(programa.getIdUnidadResponsable())) {
            throw new ValidacionException("La unidad responsable es requerida.",
                    ValidacionCodigoError.VALOR_REQUERIDO);
        }
        if (!ValidacionUtil
                .esNumeroPositivo(programa.getIdFuenteFinanciamiento())) {
            throw new ValidacionException(
                    "La fuente de financiamiento es requerida.",
                    ValidacionCodigoError.VALOR_REQUERIDO);
        }
        if (!ValidacionUtil
                .esNumeroPositivo(programa.getIdSubfuentefinanciamiento())) {
            throw new ValidacionException(
                    "La subfuente de financiamiento es requerida.",
                    ValidacionCodigoError.VALOR_REQUERIDO);
        }

        if (!ValidacionUtil.esNumeroPositivo(programa.getIdTipoRecurso())) {
            throw new ValidacionException("El tipo de recurso es requerido.",
                    ValidacionCodigoError.VALOR_REQUERIDO);
        }
    }
    ...
    protected void crearDetallePrograma(DetalleProgramaDTO detalleProgramaDTO) {
        ...
        if (detalleProgramaDTO.getIdTipoDetalle()
            ...
            dto.setIdAccion(
                    EnumTiposAccionesAutorizacion.APERTURA_VACANTE_PROGRAMA_FEDERAL_POR_DETALLE);
    ...
    protected ProgramaDTO obtenerProgramaPorId(Integer idPrograma) {
        ...
        if (p.getUnidadResponsable() != null) {
            dto.setIdUnidadResponsable(
                    p.getUnidadResponsable().getIdUnidadResponsable());
        }
        dto.setIdFuenteFinanciamiento(
                p.getFuenteFinanciamiento().getIdFuenteFinanciamiento());
        dto.setIdSubfuentefinanciamiento(
                p.getSubfuenteFinanciamiento().getIdSubfuenteFinanciamiento());
    ...
    public ConfiguracionDetalleProgramaDTO obtenerDetallePrograma(
        ...
        if (detallePrograma == null) {
            throw new ValidacionException(
                    "El detalle con identificador " + idDetallePrograma
                            + " no se ha encontrado",
                    ValidacionCodigoError.REGISTRO_NO_ENCONTRADO);
        }
        ...
        configuracionDetallePrograma
                .setNombrePrograma(programa.getPrograma().toLowerCase());
        if (detallePrograma.getTipoDetallePrograma()
                .equals(EnumTipoDetallePrograma.PARTIDA)) {
            configuracionDetallePrograma
                    .setDependencia(programa.getDependencia().getDescripcion());
            configuracionDetallePrograma.setFuenteFinanciamiento(
                    programa.getFuenteFinanciamiento().getDescripcion());
            configuracionDetallePrograma
                    .setProyecto(programa.getProyecto().getDescripcion());
            configuracionDetallePrograma.setSubFuenteFinanciamiento(
                    programa.getSubfuenteFinanciamiento().getDescripcion());
            configuracionDetallePrograma
                    .setTipoRecurso(programa.getTipoRecurso().getDescripcion());
            configuracionDetallePrograma.setUnidadResponsable(
                    programa.getUnidadResponsable().getDescripcion());
        }
    ...
    protected void editarPrograma(ProgramaDTO edicionPrograma) {
        ...
        ProgramaEntity entity = programaRepository
                .obtenerPorId(edicionPrograma.getIdPrograma());
        if (entity == null) {
    ...
    protected ProgramaDTO obtenerDetalleProgramaPorId(Integer idPrograma) {
        ...
        if (p.getUnidadResponsable() != null) {
            programa.setIdUnidadResponsable(
                    p.getUnidadResponsable().getIdUnidadResponsable());
        }

        if (p.getFuenteFinanciamiento() != null) {
            programa.setIdFuenteFinanciamiento(
                    p.getFuenteFinanciamiento().getIdFuenteFinanciamiento());
        }

        if (p.getSubfuenteFinanciamiento() != null) {
            programa.setIdSubfuentefinanciamiento(p.getSubfuenteFinanciamiento()
