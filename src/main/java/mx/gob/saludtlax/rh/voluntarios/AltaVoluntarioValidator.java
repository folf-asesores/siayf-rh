...
public class AltaVoluntarioValidator {
	...
	public Object validate(InvocationContext context) throws Exception {
		...
		if (dto == null) {
			throw new ValidacionException("La información del voluntario es requerida.",
		...
		if (!ValidacionUtil.esNumeroPositivo(dto.getSueldoMensual())) {
			throw new ValidacionException("El sueldo del empleado debe ser mayor a 0",
					ValidacionCodigoError.VALOR_REQUERIDO);
		}
		...
		if (!ValidacionUtil.tieneCadenaFormatoNumero(dto.getNumeroCuenta())) {
			throw new ValidacionException("Formato invalido de cuenta, verique que solo contenga números.",
					ValidacionCodigoError.FORMATO_INVALIDO);
		}

		if (dto.getFechaInicial() == null || dto.getFechaFinal() == null) {
			throw new ValidacionException("Es requerido especificar el periodo de contratación.",
					ValidacionCodigoError.VALOR_REQUERIDO);
		}

		if (FechaUtil.esFechaInicialMayorQueFinal(dto.getFechaInicial(), dto.getFechaFinal())) {
			throw new ValidacionException("La fecha de inicio no puede ser mayor a la final",
