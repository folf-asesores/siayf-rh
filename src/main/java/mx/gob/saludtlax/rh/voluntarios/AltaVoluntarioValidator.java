    ...
    public Object validate(InvocationContext context) throws Exception {
        ...
        if (dto == null) {
            throw new ValidacionException(
                    "La información del voluntario es requerida.",
                    ValidacionCodigoError.VALOR_REQUERIDO);
        }

        if (!ValidacionUtil.esNumeroPositivo(dto.getIdAspirante())) {
            throw new ValidacionException("El aspirante es requerido.",
                    ValidacionCodigoError.VALOR_REQUERIDO);
        }
        ...
        if (ValidacionUtil.esCadenaVacia(dto.getNumeroCuenta())) {
            throw new ValidacionException("El numero de cuenta es requerido.",
                    ValidacionCodigoError.VALOR_REQUERIDO);
        }

        if (!ValidacionUtil.tieneCadenaFormatoNumero(dto.getNumeroCuenta())) {
            throw new ValidacionException(
                    "Formato invalido de cuenta, verique que solo contenga números.",
                    ValidacionCodigoError.FORMATO_INVALIDO);
        }

        if (dto.getFechaInicial() == null || dto.getFechaFinal() == null) {
            throw new ValidacionException(
                    "Es requerido especificar el periodo de contratación.",
                    ValidacionCodigoError.VALOR_REQUERIDO);
        }

        if (FechaUtil.esFechaInicialMayorQueFinal(dto.getFechaInicial(),
                dto.getFechaFinal())) {
            throw new ValidacionException(
        ...
        if (dto.getFechaIngreso() == null) {
            throw new ValidacionException("La fecha de ingreso es requerida.",
                    ValidacionCodigoError.VALOR_REQUERIDO);
        }
        ...
        if (FechaUtil.esFechaInicialMayorQueFinal(dto.getFechaInicial(),
                dto.getFechaIngreso())) {
            throw new ValidacionException(
                    "La fecha de ingreso no puede ser menor que la fecha inicial de contratación.",
                    ValidacionCodigoError.FECHA_INCORRECTA);
        }
