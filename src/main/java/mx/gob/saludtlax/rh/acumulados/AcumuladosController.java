...
public class AcumuladosController implements Serializable {
	...
	public void inicio() {
		...
		for (TipoNombramientoDTO tipoNombramiento : tiposNom) {
			tiposNombramientosItems
					.add(new SelectItem(tipoNombramiento.getIdTipoNombramiento(), (tipoNombramiento.getClave().isEmpty()
							? tipoNombramiento.getDescripcion() : tipoNombramiento.getClave())));
		}
	...
	public void realizarBusqueda() {
		...
		try {
			if (nombramientoSeleccionado > 0) {
				...
				claveNombramientoSeleccionado = nombramientoEJB.obtenerNombramiento(nombramientoSeleccionado)
						.getClave();
			}
			dts = consultaNominaService.listaConsultaNominaPorNombramiento(nombramientoSeleccionado, quincenaInicial,
					quincenaFinal, anio);
	...
	public void descargarExcel() {
		try {
			...
			String[] parametros = { "ID_USUARIO", String.valueOf(usuario.getIdUsuario()), "REPORTE_NOMBRE",
					...
					String.valueOf(this.nombramientoSeleccionado), "QUINCENA_INICIAL",
					String.valueOf(this.quincenaInicial), "QUINCENA_FINAL", String.valueOf(this.quincenaFinal),
					"ANIO_REAL", String.valueOf(this.anio), };
			...
			this.setBytes(admintradorReportes.obtenerReporte(referencia));

			if (this.getBytes() != null) {
				JSFUtils.descargarArchivo(this.getBytes(), "acumulados", TipoArchivo.getMIMEType("xlsx"));
