...
public class ModificacionLaboralValidator {
    ...
    public Object validate(InvocationContext context) throws Exception {
        ...
        if (!ValidacionUtil
                .esNumeroPositivo(dto.getIdSubfuenteFinanciamiento())) {
            throw new ValidacionException(
        ...
        if (dto.getTipoContratacion() != EnumTipoContratacion.CONTRATO_FEDERAL
                && dto.getTipoContratacion() != EnumTipoContratacion.CONTRATO_ESTATAL) {
            if (!ValidacionUtil.esNumeroPositivo(dto.getIdTabulador())) {
                throw new ValidacionException("El tabulador es requerido.",
                        ValidacionCodigoError.VALOR_REQUERIDO);
            }
        }
        ...
        if (dto.getTipoContratacion() == EnumTipoContratacion.CONTRATO_ESTATAL) {
            ...
            if (!ValidacionUtil.esNumeroPositivo(dto.getSueldo01())) {
                throw new ValidacionException(
                        "El sueldo 01 debe ser un número mayor a cero.",
                        ValidacionCodigoError.VALOR_REQUERIDO);
            }
        ...
        if (dto.getFechaInicioLabores() == null) {
            throw new ValidacionException(
                    "La fecha de inicio de labores es requerida.",
                    ValidacionCodigoError.VALOR_REQUERIDO);
        }
