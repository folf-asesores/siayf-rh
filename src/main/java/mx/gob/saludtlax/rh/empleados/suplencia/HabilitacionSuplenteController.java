...
public class HabilitacionSuplenteController implements Serializable {
	...
	public void obtenerUnidadesResponsables() {
		...
		if (view.getRegistroSuplente().getIdDependencia() != 0) {
			view.setListaUnidadesResponsables(SelectItemsUtil.listaCatalogos(
					catalogos.listaUnidadesResponsablesPorDependencia(view.getRegistroSuplente().getIdDependencia())));
		}
	...
	public void consultarMunicipioPorIdEstado() {
		if (view.getRegistroSuplente().getSuplente().getDireccion().getIdEstado() != 0) {
			view.setListaMunicipios(SelectItemsUtil.listaCatalogos(catalogos.consultarMunicipiosPorEstado(
					view.getRegistroSuplente().getSuplente().getDireccion().getIdEstado())));
		}
	}

	public void consultarAsentamientosPorIdMunicipio() {
		if (view.getRegistroSuplente().getSuplente().getDireccion().getIdMunicipio() != 0) {
			view.setListaAsentamientos(SelectItemsUtil.listaCatalogos(catalogos.consultarAsantamientosPorMunicipios(
					view.getRegistroSuplente().getSuplente().getDireccion().getIdMunicipio())));
	...
	public void consultarSuplentes() {
		...
		if (view.getSuplentes().isEmpty()) {
			JSFUtils.warningMessage("",
					"No se encontrarón suplentes con el criterio " + view.getFiltro().getCriterio());
		}
