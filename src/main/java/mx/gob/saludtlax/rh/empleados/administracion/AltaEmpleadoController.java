...
public class AltaEmpleadoController implements Serializable {
	...
	public void inicio() {
		...
		view.setListaTiposJornadas(SelectItemsUtil.listaCatalogos(catalogo
				.listaTiposJornadas()));
		view.setListaBancos(SelectItemsUtil.listaCatalogos(catalogo
				.listaBancos()));
		view.setListaMetodosPago(SelectItemsUtil.listaCatalogos(catalogo
	...
	public void vacantesDisponiblesPorContratacion() {
		try {
			...
			filtroVacanteDTO
					.setTipoBusqueda(EnumFiltroConsultaVacante.DISPONIBLES_POSTULADAS);

			view.setVacantesDisponibles(vacantes
		...
		try {
			...
			if (candidato.getIdTipoCandidato() == EnumTipoCandidato.EMPLEADO) {
				ValidacionEmpleadoDTO validacion = empleado
						.validarDatosObligatoriosEmpleado(candidato
								.getIdContexto());
			...
			view.getAltaEmpleado()
					.getNombramiento()
					.setClavePresupuestal(
