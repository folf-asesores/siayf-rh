...
public class AltaEmpleadoService {
	...
	public void altaEmpleado(AltaEmpleadoDTO dto) {
		...
		if (vacante == null) {
			throw new ValidacionException(
					"No se encontró registro de la vacante con identificador " + dto.getIdVacante(),
					ValidacionCodigoError.REGISTRO_NO_ENCONTRADO);
		}

		if (vacante.getDisponible().equals("NO")) {
			throw new ReglaNegocioException("La vacante no se encuentra disponible",
					ReglaNegocioCodigoError.VACANTE_NO_DISPONIBLE);
		}

		if (vacante.getIdCandidatoPostulado() == null) {
			throw new ReglaNegocioException("La vacante no tiene asignada un candidato",
					ReglaNegocioCodigoError.SIN_REGISTRO);
		}

		if (configuracionPresupuestalRepository.numeroLaboralEnUso(dto.getNumeroLaboral())) {
			throw new ReglaNegocioException("El número laboral se encuentra en uso, ingrese uno nuevo.",
		...
		if (candidato == null) {
			throw new ReglaNegocioException(
					"No se encontro el candidato con identificador " + vacante.getIdCandidatoPostulado(),
					ReglaNegocioCodigoError.SIN_REGISTRO);
		}
		...
		if (candidato.getTipoCandidato() == EnumTipoCandidato.ASPIRANTE) {
			...
			empleado = reingresarEmpleado(candidato, tipoContratacion, dto.getIdUsuario(), dto.getIdBanco(),
		...
		if (tipoContratacion == EnumTipoContratacion.BASE || tipoContratacion == EnumTipoContratacion.CONFIANZA
			...
			nombramientoService.crearNombramiento(dto.getNombramiento(),
					vacante.getConfiguracion().getNombramiento().getIdTipoNombramiento(), dto.getIdTipoJornada());
	...
	private EmpleadoEntity reingresarEmpleado(CandidatoVacanteEntity candidato, Integer tipoContratacion,
			Integer idUsuario, Integer idBanco, String numeroCuenta, Integer idMetodoPago) {
		...
		if (!ValidacionUtil.esNumeroPositivo(movimientoReingreso.getIdMovimiento())) {
			movimientoReingreso
					.setObservaciones("ALTA EMPLEADO DE " + EnumTipoContratacion.obtenerDescripcion(tipoContratacion));
	...
	private EmpleadoEntity registrarAspirante(InventarioVacanteEntity vacante, CandidatoVacanteEntity candidato,
			AltaEmpleadoDTO dto) {

		AspiranteEntity aspiranteEntity = aspiranteRepository.obtenerPorId(candidato.getIdContexto());
		if (aspiranteEntity == null) {
			throw new SistemaException("No se encontró aspirante con identificador " + candidato.getIdContexto(),
