...
public class AltaEmpleadoService {
	...
	public void altaEmpleado(AltaEmpleadoDTO dto) {
		...
		if (configuracionPresupuestalRepository.numeroLaboralEnUso(dto
				.getNumeroLaboral())) {
			throw new ReglaNegocioException(
		...
		if (candidato.getTipoCandidato() == EnumTipoCandidato.ASPIRANTE) {
			...
			empleado = reingresarEmpleado(candidato, tipoContratacion,
					dto.getIdUsuario(), dto.getIdBanco(),
					dto.getNumeroCuenta(), dto.getIdMetodoPago());
		}
		...
		if (tipoContratacion == EnumTipoContratacion.BASE
			...
			dto.getNombramiento().setSueldo(
					vacante.getConfiguracion().getSueldo());
			dto.getNombramiento().setIdClasificacionNombramiento(
					idClasificacionNombramiento);

			nombramientoService.crearNombramiento(dto.getNombramiento(),
					vacante.getConfiguracion().getNombramiento()
							.getIdTipoNombramiento(), dto.getIdTipoJornada());
			...
			nuevoContrato.setSueldoMensual(vacante.getConfiguracion()
					.getSueldo());
		...
		configuracionPresupuestal.setFechaInicioLabores(dto
	...
	private EmpleadoEntity reingresarEmpleado(CandidatoVacanteEntity candidato,
		...
		EmpleadoEntity empleado = empleadoRepository.obtenerPorId(candidato
				.getIdContexto());
		if (empleado == null) {
			throw new ValidacionException(
		...
		if (idTipoBaja != null) {
			...
			if (tipoContratacion == EnumTipoContratacion.BASE
				...
				if (baja.isConAntecedente()) {
					...
					movimientoReingreso
							.setIdMovimiento(EnumTipoMovimiento.REINGRESO_BASE_SIN_ANTECEDENTES);
				}
			} else if (tipoContratacion == EnumTipoContratacion.CONFIANZA) {
				if (baja.isConAntecedente()) {
					movimientoReingreso
							.setIdMovimiento(EnumTipoMovimiento.REINGRESO_CONFIANZA);
				} else {
					movimientoReingreso
							.setIdMovimiento(EnumTipoMovimiento.REINGRESO_CONFIANZA_SIN_ANTECEDENTES);
				}
			} else if (tipoContratacion == EnumTipoContratacion.CONTRATO_ESTATAL
					|| tipoContratacion == EnumTipoContratacion.CONTRATO_FEDERAL) {
				...
				if (baja.isConAntecedente()) {
					movimientoReingreso
							.setIdMovimiento(EnumTipoMovimiento.REINGRESO_EVENTUAL);
				} else {
					movimientoReingreso
							.setIdMovimiento(EnumTipoMovimiento.REINGRESO_EVENTUAL_SIN_ANTECEDENTES);
				}
			}
		...
		// movimiento por alta
		if (!ValidacionUtil.esNumeroPositivo(movimientoReingreso
				.getIdMovimiento())) {
			movimientoReingreso
					.setObservaciones("ALTA EMPLEADO DE "
	...
	private EmpleadoEntity registrarAspirante(InventarioVacanteEntity vacante,
		...
		if (empleadoRepository.existeEmpleadoConNumeroPersonal(dto
				.getNumeroEmpleado())) {
			throw new ReglaNegocioException(
					"El número laboral ya se encuentra asignado a un empleado, ingrese uno nuevo.",
					ReglaNegocioCodigoError.NUMERO_EMPLEADO);
		}
		...
		List<ExperienciaLaboralEntity> experienciasLaborales = experienciaLaboralRepository
				.consultarExperienciasLaboralesAspirante(aspiranteEntity
						.getIdAspirante());
		if (!experienciasLaborales.isEmpty()) {
			for (ExperienciaLaboralEntity entity : experienciasLaborales) {
				...
				experienciaLaboralRepository.actualizar(entity);
			}

		}

		List<HistorialAcademicoEntity> historialesAcademicos = historialAcademicoRepository
				.consultarHistorialAcademicoAspirante(aspiranteEntity
						.getIdAspirante());
		...
		for (HistorialAcademicoEntity entity : historialesAcademicos) {
			entity.setIdEmpleado(nuevoEmpleado.getIdEmpleado());
			historialAcademicoRepository.actualizar(entity);
		}
		...
		if (tipoContratacion == EnumTipoContratacion.BASE) {
			...
			movimientoAlta
					.setIdMovimiento(EnumTipoMovimiento.ALTA_PERSONAL_CONFIANZA);
		} else if (tipoContratacion == EnumTipoContratacion.CONTRATO_ESTATAL
				|| tipoContratacion == EnumTipoContratacion.CONTRATO_FEDERAL) {
		...
		if (!ValidacionUtil.esNumeroPositivo(movimientoAlta.getIdMovimiento())) {
			...
			movimientosEmpleados.crearMovimientoEmpleado(movimientoAlta);
		}
