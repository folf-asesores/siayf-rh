...
public class CierreSuplenciaController implements Serializable {
	...
	public void inicio() {
		...
		view.setListaSubfuentesFinanciamiento(
				SelectItemsUtil.listaCatalogos(catalogo.listaSubfuentesFinanciamientos()));
	}

	public void consultarDetallesQuincena() {
		view.setMostrarColumnasCierre(false);
		view.getListaDetalles().clear();
		view.setListaDetalles(suplencia.consultarQuincenasSuplente(view.getNumeroQuincena(),
	...
	public void obtenerUnidadesResponsables() {
		...
		if (view.getCierre().getIdDependencia() != 0) {
			view.setListaUnidadesResponsables(SelectItemsUtil.listaCatalogos(
					catalogo.listaUnidadesResponsablesPorDependencia(view.getCierre().getIdDependencia())));
		}
