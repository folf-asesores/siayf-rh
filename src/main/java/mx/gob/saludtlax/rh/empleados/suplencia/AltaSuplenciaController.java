...
public class AltaSuplenciaController implements Serializable {
	...
	public void inicio() {
		...
		view.setListaDocumentos(SelectItemsUtil.listaCatalogos(catalogo
				.consultarDocumentosExpedientesClasificacion(EnumClasificacionExpediente.DOCUMENTOS_SUPLENCIAS)));
	...
	public void seleccionarSuplente(SuplenteDTO suplente) {
		Integer idQuincena = suplencia.obtenerIdQuincenaSuplente(view.getNumeroQuincenaActual(),
				view.getEjercicioActivo(), suplente.getIdSuplente());
	...
	public void calcular() {
		...
		if (!ValidacionUtil.esNumeroPositivo(view.getAltaSuplencia().getIdTabulador())) {
			...
			if (FechaUtil.esFechaInicialMayorQueFinal(view.getAltaSuplencia().getFechaInicio(),
				...
				BigDecimal sueldoDiario = tabulador
						.obtenerSueldoDiarioPorIdTabulador(view.getAltaSuplencia().getIdTabulador());
				view.getAltaSuplencia().setCantidadDiaria(sueldoDiario);
				int dias = FechaUtil.calcularDias(view.getAltaSuplencia().getFechaInicio(),
						view.getAltaSuplencia().getFechaFin());
	...
	public void calcularTotalesEditados() {
		...
		if (!ValidacionUtil.esNumeroPositivo(view.getEdicion().getIdTabulador())) {
			...
			if (fechaInicio.compareTo(fechaFin) == 1) {
				...
				BigDecimal sueldoDiario = tabulador
						.obtenerSueldoDiarioPorIdTabulador(view.getEdicion().getIdTabulador());
	...
	public void mostrarAltaSuplenciaRango() {
		String estatusQuincena = suplencia.obtenerEstatusQuincenaSuplencia(view.getNumeroQuincenaActual(),
				FechaUtil.ejercicioActual(), view.getSuplenteSeleccionado().getIdSuplente());
		if (estatusQuincena == null) {
			view.setMostrarAltaSuplenciaRango(true);
		} else if (!estatusQuincena.equals(EnumEstatusSuplencia.CAPTURA)) {
			JSFUtils.errorMessage("", "La quincena " + view.getNumeroQuincenaActual() + " del suplente "
