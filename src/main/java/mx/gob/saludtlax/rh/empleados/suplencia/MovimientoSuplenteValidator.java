...
public class MovimientoSuplenteValidator {
    ...
    public Object validate(InvocationContext context) throws Exception {
        MovimientoSuplenteDTO dto = (MovimientoSuplenteDTO) context
                .getParameters()[0];
        ...
        if (dto.getFechaFin() == null) {
            throw new ValidacionException("La fecha fin es requerida.",
                    ValidacionCodigoError.VALOR_REQUERIDO);
        }

        if (dto.getMovimiento().equals(EnumTipoMovimientoSuplente.VACACIONES)) {
            if (!ValidacionUtil
                    .esNumeroPositivo(dto.getEjercicioFiscalPeriodo())) {
                throw new ValidacionException(
                        "Es requerido especificar el ejercicio al que pertenece el periodo",
                        ValidacionCodigoError.VALOR_REQUERIDO);
            }
        }
