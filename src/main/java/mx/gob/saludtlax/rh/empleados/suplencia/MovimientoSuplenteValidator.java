...
public class MovimientoSuplenteValidator {
    ...
    public Object validate(InvocationContext context) throws Exception {
        ...
        if (dto == null) {
            throw new ValidacionException("Los datos del movimiento son requeridos.", ValidacionCodigoError.VALOR_REQUERIDO);
        }
        ...
        if (dto.getMovimiento().equals(EnumTipoMovimientoSuplente.VACACIONES)) {
            if (!ValidacionUtil.esNumeroPositivo(dto.getEjercicioFiscalPeriodo())) {
                throw new ValidacionException("Es requerido especificar el ejercicio al que pertenece el periodo", ValidacionCodigoError.VALOR_REQUERIDO);
            }
        }
