...
public class MovimientoEmpleadoReporteService {
	...
	public List<ConsentradoAltaBajaExcelDTO> listaConsultaConsentradoAltaBajaPorRangoFecha(Integer idTipoContratacion,
			Date fechaInicial, Date fechaFinal) {
		try {
			Session session = em.unwrap(Session.class);
			Query query = session
					.createSQLQuery("CALL usp_concentrado_altas_bajas(:idTipoContratacion, :fechaInicial, :fechaFinal)")
					.setParameter("idTipoContratacion", idTipoContratacion).setParameter("fechaInicial", fechaInicial)
					.setParameter("fechaFinal", fechaFinal);
			query.setResultTransformer(Transformers.aliasToBean(ConsentradoAltaBajaExcelDTO.class));
			@SuppressWarnings("unchecked")
			List<ConsentradoAltaBajaExcelDTO> result = (List<ConsentradoAltaBajaExcelDTO>) query.list();
