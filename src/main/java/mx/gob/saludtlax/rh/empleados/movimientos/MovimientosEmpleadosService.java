...
public class MovimientosEmpleadosService {
	...
	protected void crearMovimiento(RegistroMovimientoDTO dto) {
		...
		UsuarioEntity usuarioEntity = usuarioRepository.obtenerPorId(dto
				.getIdUsuario());
		...
		if (movimiento.isAutorizacion()) {
			boolean tieneConfiguradoUsuariosAprobacion = configuracionAprobacionRepository
					.tieneProcesoAprobacionMovimientoPersonal(movimiento
							.getIdPadre());
			...
			String mensaje = "el movimiento "
					+ movimiento.getMovimiento().toLowerCase()
					+ " al empleado " + empleado.getNombreCompleto();
			// Iniciar proceso de autorizacion
			NuevaAutorizacionDTO autorizacion = new NuevaAutorizacionDTO();
			autorizacion
					.setIdAccion(EnumTiposAccionesAutorizacion.MOVIMIENTOS_PERSONAL);
			autorizacion.setIdEntidadContexto(movimientoEmpleado
	...
	public String autorizarMovimiento(Integer idMovimiento) {
		...
		if (movimiento.getIdPadre() == EnumTipoMovimiento.BAJAS_DEFINITIVAS) {
			...
			String comentarios = movimiento.getMovimiento()
					+ "EN EL PERIODO "
					+ FechaUtil.formatoFecha(movimientoEmpleado
							.getFechaInicioPermiso())
					+ "-"
					+ FechaUtil.formatoFecha(movimientoEmpleado
							.getFechaFinPermiso());
