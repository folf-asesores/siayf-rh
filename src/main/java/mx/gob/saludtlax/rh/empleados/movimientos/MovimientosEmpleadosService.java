...
public class MovimientosEmpleadosService {
    ...
    protected void crearMovimiento(RegistroMovimientoDTO dto) {
        ...
        if (puesto == null) {
            throw new ReglaNegocioException(
                    "No se encontró el puesto del empleado con identificador "
                            + dto.getIdVacante(),
        ...
        TipoMovimientoEmpleadoEntity movimiento = tipoMovimientoEmpleadoRepository
                .obtenerPorId(dto.getIdMovimiento());

        if (puesto.getDisponible().equals("SI")) {
            throw new ReglaNegocioException(
                    "La vacante seleccionada no tiene asignado un empleado activo, se encuentra disponible",
        ...
        if (movimiento.isAutorizacion()) {
            boolean tieneConfiguradoUsuariosAprobacion = configuracionAprobacionRepository
                    .tieneProcesoAprobacionMovimientoPersonal(
                            movimiento.getIdPadre());
            if (!tieneConfiguradoUsuariosAprobacion) {
                throw new ReglaNegocioException(
                        "El movimiento requiere de autorización pero no tiene configurados usuarios.",
                        ReglaNegocioCodigoError.CONFIGURACION_NO_EXISTE);
            }
            movimientoEmpleado
                    .setEstatusMovimiento(EnumEstatusMovimiento.PENDIENTE);
            String mensaje = "el movimiento "
                    + movimiento.getMovimiento().toLowerCase() + " al empleado "
                    + empleado.getNombreCompleto();
            // Iniciar proceso de autorizacion
            NuevaAutorizacionDTO autorizacion = new NuevaAutorizacionDTO();
            autorizacion.setIdAccion(
                    EnumTiposAccionesAutorizacion.MOVIMIENTOS_PERSONAL);
            autorizacion.setIdEntidadContexto(
                    movimientoEmpleado.getIdMovimientoEmpleado());
            ...
            registrarBitacora(movimientoEmpleado.getEmpleado().getIdEmpleado(),
                    movimientoEmpleado.getUsuarioEntity().getIdUsuario(), "-",
                    "-", EnumTipoModificacionEmpleado.SOLICITUD_MOVIMIENTO,
    ...
    private void registrarBitacora(Integer idEmpleado, Integer idUsuario,
            String lccNueva, String lccActual, Integer idMovimientoBitacora,
            String comentarios) {
    ...
    public String autorizarMovimiento(Integer idMovimiento) {
        MovimientoEmpleadoEntity movimientoEmpleado = bitacoraMovimientoAutorizadoRepository
        ...
        if (movimiento.getIdPadre() == EnumTipoMovimiento.BAJAS_DEFINITIVAS) {
            if (movimiento.getClave() != 1113) {
                ...
                empleadoMovimiento
                        .setTipoBaja(movimiento.getIdTipoMovimiento());
            ...
            String comentarios = movimiento.getMovimiento() + "EN EL PERIODO "
                    + FechaUtil.formatoFecha(
                            movimientoEmpleado.getFechaInicioPermiso())
                    + "-" + FechaUtil.formatoFecha(
                            movimientoEmpleado.getFechaFinPermiso());
            ...
            PuestoGeneralEntity puestoPromocion = puestogeneralRepository
                    .obtenerPorId(movimientoEmpleado.getIdPuestoGeneral());
            String lccNueva = "PUESTO NUEVO: " + puestoPromocion.getCodigo()
                    + " " + puestoPromocion.getPuesto();
            ConfiguracionPresupuestoEntity datoLaboral = movimientoEmpleado
                    .getInventarioVacante().getConfiguracion();

            String lccActual = "PUESTO ANTERIOR: "
                    + datoLaboral.getPuesto().getCodigo() + " "
                    + datoLaboral.getPuesto().getPuesto();
    ...
    protected void validarMovimiento(Integer idPuesto, Integer tipoMovimiento) {
        int tipoContratacion = inventarioVacanteRepository
                .obtenerIdTipoContratacionPuesto(idPuesto);
        ...
        if (tipoMovimiento == EnumTipoMovimiento.PROMOCION_PUESTO_AUMENTO_PERCEPCIONES) {
            if (tipoContratacion != EnumTipoContratacion.BASE
                ...
                throw new ReglaNegocioException(
                        "El tipo de contratación no tiene permitido el movimiento seleccionado.",
    ...
    protected Integer obtenerPadreMovimiento(Integer idMovimiento) {
        if (idMovimiento == null) {
            throw new ValidacionException("El movimiento es requerido.",
                    ValidacionCodigoError.VALOR_REQUERIDO);
        }
        return tipoMovimientoEmpleadoRepository
                .obtenerMovimientoPadre(idMovimiento);
    }
