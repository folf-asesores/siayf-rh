...
public class AltaSuplenteValidator {
	...
	public Object validate(InvocationContext context) throws Exception {
		...
		if (dto == null) {
			throw new ValidacionException("Los datos del registro del suplente son requeridos.",
		...
		if (!ValidacionUtil.esNumeroPositivo(dto.getIdTipoCandidato())) {
			throw new ValidacionException("Es requerido seleccionar el tipo de suplente que desea registrar.",
					ValidacionCodigoError.VALOR_REQUERIDO);
		}
		...
		if (dto.getIdTipoCandidato().equals(EnumTipoCandidato.EMPLEADO)) {
			...
			if (suplente == null) {
				throw new ValidacionException("La información del suplente es requerida",
						ValidacionCodigoError.VALOR_REQUERIDO);
			}
			...
			if (ValidacionUtil.esCadenaVacia(suplente.getCurp())) {
				throw new ValidacionException("La curp del suplente es requerida.",
						ValidacionCodigoError.VALOR_REQUERIDO);
			}

			if (ValidacionUtil.esCadenaVacia(suplente.getNombre())) {
				throw new ValidacionException("El nombre del suplente es requerido.",
						ValidacionCodigoError.VALOR_REQUERIDO);
			}

			if (ValidacionUtil.esCadenaVacia(suplente.getApellidoMaterno())) {
				throw new ValidacionException("El apellido materno es requerido.",
						ValidacionCodigoError.VALOR_REQUERIDO);
			}

			if (suplente.getFechaNacimiento() == null) {
				throw new ValidacionException("La fecha de nacimiento es incorrecta.",
						ValidacionCodigoError.VALOR_REQUERIDO);
			}

			if (ValidacionUtil.esCadenaVacia(suplente.getSexo())) {
				throw new ValidacionException("El sexo del suplente es requerido.",
						ValidacionCodigoError.VALOR_REQUERIDO);
			}

			if (!ValidacionUtil.esNumeroPositivo(suplente.getNumeroPersonal())) {
				throw new ValidacionException("El número personal del suplente es requerido.",
						ValidacionCodigoError.VALOR_REQUERIDO);
			}

			DireccionDTO direccion = dto.getSuplente().getDireccion();
			if (direccion == null) {
				throw new ValidacionException("Los datos de la dirección son requeridos.",
						ValidacionCodigoError.VALOR_REQUERIDO);
			}
			...
			if (ValidacionUtil.esCadenaVacia(direccion.getNumeroExterior())) {
				throw new ValidacionException("El número exterior es requerido.",
						ValidacionCodigoError.VALOR_REQUERIDO);
			}
		...
		if (!ValidacionUtil.tieneCadenaFormatoNumero(dto.getNumeroCuenta())) {
			throw new ValidacionException("El número de cuenta solo admite números.",
