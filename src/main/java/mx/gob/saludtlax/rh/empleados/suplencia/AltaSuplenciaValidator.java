    ...
    public Object validate(InvocationContext context) throws Exception {
        ...
        if (dto == null) {
            throw new ValidacionException(
                    "Los datos del alta de la suplencia son requeridos.",
        ...
        if (!ValidacionUtil.esNumeroPositivo(dto.getEjercicioFiscal())) {
            throw new ValidacionException(
                    "El ejercicio fiscal de la quincena es requerido.",
                    ValidacionCodigoError.VALOR_REQUERIDO);
        }

        if (!ValidacionUtil.esNumeroPositivo(dto.getIdCausaSuplencia())) {
            throw new ValidacionException(
                    "El motivo de la suplencia es requerido.",
                    ValidacionCodigoError.VALOR_REQUERIDO);
        }
        ...
        if (dto.getFechaInicio() == null) {
            throw new ValidacionException("La fecha de inicio es requerida..",
                    ValidacionCodigoError.VALOR_REQUERIDO);
        }

        if (dto.getFechaFin() == null) {
            throw new ValidacionException("La fecha final es requerida..",
                    ValidacionCodigoError.VALOR_REQUERIDO);
        }
