...
public class ConfiguracionPresupuestalService {
    ...
    protected Integer crearDatoLaboral(SolicitudNuevoPuestoDTO dto) {
        ...
        EstatusConfiguracionesEntity estatus = this.estatusConfiguracionesRepository.obtenerPorId(EnumEstatusConfiguracion.EN_ESPERA_AUTORIZACION);
        ...
        if (iDTipoContratacion == EnumTipoContratacion.INTERINATO) {
            InventarioVacanteEntity puestoDisponible = this.inventarioVacanteRepository.obtenerPorId(dto.getIdPuestoDisponible());
            ...
            TipoContratacionEntity tipoContratacion = this.tipoContratacionRepository.obtenerPorId(EnumTipoContratacion.INTERINATO);
            ...
            DependenciaTempEntity dependencia = this.dependenciasRepository.obtenerPorId(dto.getDatosLaborales().getIdDependencia());
            FuenteFinanciamientoEntity fuenteFinanciamiento = this.fuenteFinanciamientoRepository
            ...
            if (idNombramiento == null) {
                TiposNombramientosEntity nombramiento = this.nombramientoRepository.nombramientoPorId(dto.getDatosLaborales().getIdNombramiento());
                nuevoDatoLaboral.setNombramiento(nombramiento);
            } else {
                TiposNombramientosEntity nombramiento = this.nombramientoRepository.nombramientoPorId(idNombramiento);
                nuevoDatoLaboral.setNombramiento(nombramiento);
            }

            ProyectoTempEntity proyecto = this.proyectoRepository.obtenerPorId(dto.getDatosLaborales().getIdProyecto());
            PuestoGeneralEntity puesto = this.puestoGeneralRepository.obtenerPorId(dto.getDatosLaborales().getIdPuesto());
            SubFuenteFinanciamientoTempEntity subfuenteFinanciamiento = this.subfuentefinanciamientoRepository
                    .obtenerPorId(dto.getDatosLaborales().getIdSubfuenteFinanciamiento());
            TipoRecursoTempEntity tipoRecursoTempEntity = this.tipoRecursoTempRepository.obtenerPorId(dto.getDatosLaborales().getIdTipoRecurso());
            UnidadResponsableEntity unidadResponsable = this.unidadResponsableRepository.obtenerPorId(dto.getDatosLaborales().getIdUnidadResponsable());

            TabuladorEntity tabulador = this.tabuladorRepository.obtenerPorId(dto.getDatosLaborales().getIdTabulador());
            TipoContratacionEntity tipoContratacion = this.tipoContratacionRepository.obtenerPorId(idTipoContratacion);
        ...
        this.configuracionPresupuestoRepository.crear(nuevoDatoLaboral);
    ...
    protected void modificarDatoLaboral(DatoLaboralDTO datoLaboral, Integer idPuesto, Integer idUsuario) {
        ...
        InventarioVacanteEntity puesto = this.inventarioVacanteRepository.obtenerPorId(idPuesto);
        if (puesto == null) {
        ...
        ConfiguracionPresupuestoEntity datoLaboralEmpleado = this.configuracionPresupuestoRepository.obtenerPorId(puesto.getConfiguracion().getId());
        ...
        ProyectoTempEntity proyecto = this.proyectoRepository.obtenerPorId(datoLaboral.getIdProyecto());
        DependenciaTempEntity dependencia = this.dependenciasRepository.obtenerPorId(datoLaboral.getIdDependencia());
        UnidadResponsableEntity unidadResponsable = this.unidadResponsableRepository.obtenerPorId(datoLaboral.getIdUnidadResponsable());
        FuenteFinanciamientoEntity fuenteFinanciamiento = this.fuenteFinanciamientoRepository.obtenerPorId(datoLaboral.getIdFuenteFinanciamiento());
        SubFuenteFinanciamientoTempEntity subfuenteFinanciamiento = this.subfuentefinanciamientoRepository
                .obtenerPorId(datoLaboral.getIdSubfuenteFinanciamiento());
        TipoRecursoTempEntity tipoRecursoTempEntity = this.tipoRecursoTempRepository.obtenerPorId(datoLaboral.getIdTipoRecurso());

        PuestoGeneralEntity puestoGeneral = this.puestoGeneralRepository.obtenerPorId(datoLaboral.getIdPuesto());
        ...
        datoLaboralEmpleado.setFechaInicioLabores(datoLaboral.getFechaInicioLabores());
        ...
        if (datoLaboral.getIdTabulador() != null) {
            tabulador = this.tabuladorRepository.obtenerPorId(datoLaboral.getIdTabulador());
        }
        ...
        this.configuracionPresupuestoRepository.actualizar(datoLaboralEmpleado);
        ...
        if (!lccActual.equals(lccNueva)) {
            ...
            Integer idBitacora = this.bitacoraModificacionService.registrarBitacora(bitacora);
    ...
    protected DatoLaboralDTO obtenerConfiguracionPorId(Integer idConfiguracion) {
        ...
        ConfiguracionPresupuestoEntity datoLaboral = this.configuracionPresupuestoRepository.obtenerPorId(idConfiguracion);
        if (datoLaboral == null) {
    ...
    protected DatoLaboralDTO obtenerDatosLaboralesPuesto(Integer idPuesto) {
        ...
        Integer idConfiguracion = this.inventarioVacanteRepository.obtenerIdDatoLaboralPorPuesto(idPuesto);
        if (!ValidacionUtil.esNumeroPositivo(idConfiguracion)) {
            throw new ValidacionException("El puesto no tiene dato laboral asignado.", ValidacionCodigoError.REGISTRO_NO_ENCONTRADO);
        }
    ...
    protected DetalleConfiguracionPresupuestoDTO obtenerDetallePorIdConfiguracion(Integer idConfiguracionPresupuesto) {
        ...
        ConfiguracionPresupuestoEntity configuracion = this.configuracionPresupuestoRepository.obtenerPorId(idConfiguracionPresupuesto);
    ...
    protected List<InfoConfiguracionDTO> consultarConfiguracionesPorCriterio(FiltroConsultaFinanciamientosDTO filtro) {
        ...
        lista = this.configuracionPresupuestoRepository.configuracionesPorTipoContratacion(filtro.getTipoContratacion());
    ...
    protected List<InfoConfiguracionDTO> consultarConfiguracionesPorEmpleado(Integer idEmpleado) {
        List<InfoConfiguracionDTO> lista = new ArrayList<>();
        try {
            lista = this.configuracionPresupuestoRepository.configuracionesPorIdEmpleado(idEmpleado);
            return lista;
    ...
    protected void procesarPadron(int padron) {
        if (padron == EnumTipoContratacion.CONTRATO_ESTATAL) {
            List<TemporalConfEmpleadoEntity> temporales = this.temporalConfEmpleadoRepository.temporalesPorNombramientoSinClasificar("V");
            if (!temporales.isEmpty()) {
                int contador = 1;
                for (TemporalConfEmpleadoEntity t : temporales) {
                    System.out.println("procesados" + contador);
                    TempContratosEstatalesEntity contratoEstatal = this.tempContratosEstatalesRepository.contratoEstatalPorRfc(t.getRfc());
                    if (contratoEstatal != null) {
                        ...
                        this.temporalConfEmpleadoRepository.actualizar(t);
                        contratoEstatal.setIdTemporalConfiguracionEmpleado(t.getId());
                        this.tempContratosEstatalesRepository.actualizar(contratoEstatal);
                    }
            ...
            if (!temporales.isEmpty()) {
                ...
                for (TemporalConfEmpleadoEntity t : temporales) {
                    System.out.println("procesados" + contador);
                    TempContratosFederalesEntity contratosFederales = this.tempContratosFederalesRepository.obtenerContratoFederalPorRfc(t.getRfc());

                    if (contratosFederales != null) {
                        ProgramaEntity programa = this.programaRepository.programaPorDescripcion(contratosFederales.getPrograma());
                        if (programa != null) {
                            ...
                            this.tempContratosFederalesRepository.actualizar(contratosFederales);
                            contador++;
    ...
    protected InfoDatosLaboralesDTO obtenerDatosLaboralesIdPuesto(Integer idPuesto) {
        ...
        ConfiguracionPresupuestoEntity datosLaborales = this.configuracionPresupuestoRepository.obtenerPorId(idConfiguracion);
        ...
        dto.setSueldo(this.tabuladorService.obtenerInfoSueldoIdTabulador(datosLaborales.getTabulador()));
