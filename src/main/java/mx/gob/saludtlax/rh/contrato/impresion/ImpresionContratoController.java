...
public class ImpresionContratoController implements Serializable {
	...
	@PostConstruct
	public void init() {
		this.view = new ImpresionContratoView();

		obtenerListaContratoEmpleado();

		this.view.setListaTipoContrato(SelectItemsUtil
				.listaTiposContratosEmpleados());
	}
	...
	public void obtenerListaContratoEmpleado() {
		try {
			...
			if (!listaContrato.isEmpty()) {
				this.view.setListaContrato(listaContrato);
			}
	...
	public void obtenerListaContratoEmpleadoPorTipo() {
		try {
			...
			List<ContratoDTO> listaContrato = contrato
					.obtenerListaContratoPorTipo(this.view.getTipoContrato());

			if (!listaContrato.isEmpty()) {
				this.view.setListaContrato(listaContrato);
			} else {
				this.view.setListaContrato(new ArrayList<ContratoDTO>());
			}
	...
	public void descargarContrato() {
		try {
			...
			contrato.actualizarContratoPorImpresion(
					this.view.getIdContratoSeleccionado(),
					this.view.getNumeroContrato());
			...
			String[] parametros = { "ID_USUARIO",
					...
					"ID_CONTRATO",
					String.valueOf(this.view.getIdContratoSeleccionado()) };
	...
	public void mostrarConfirmacionDescarga(Integer idContratoSeleccionado,
		...
		this.view.setIdContratoSeleccionado(idContratoSeleccionado);
	...
	public void validatorTipoContrato(FacesContext context,
		...
		case "tipoContrato":
			...
			this.obtenerListaContratoEmpleado();
