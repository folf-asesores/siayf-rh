...
public class ContratoService implements Serializable {
	...
	protected ContratoDTO obtenerContratoEmpleadoPorIdContrato(
		...
		if (idContrato == null) {
			throw new ValidacionException(contexto
					+ "El identificador del contrato es requerido",
		...
		dto.setNombrePuestoGeneral(inventarioVacanteEntity.getConfiguracion()
				.getPuesto() == null ? "" : inventarioVacanteEntity
				.getConfiguracion().getPuesto().getPuesto());
		...
		dto.setDomicilioServicio(inventarioVacanteEntity.getAdscripcion() == null ? ""
	...
	protected List<ContratoDTO> listaContrato() {
		...
		if (!listaContratoEmpleadoEntities.isEmpty()) {
			for (ContratoEmpleadoEntity contratoEmpleadoEntity : listaContratoEmpleadoEntities) {
				...
				dto.setNombreCompletoEmpleado(empleadoEntity
						.getNombreCompleto() == null ? "" : empleadoEntity
						.getNombreCompleto());
				dto.setNumeroContrato(contratoEmpleadoEntity
						.getNumeroContrato());
	...
	protected List<ContratoDTO> obtenerListaContratoPorTipo(Integer tipoContrato) {
		...
		if (tipoContrato == null | tipoContrato == 0) {
			throw new ValidacionException(contexto
					+ "El tipo contrato es requerido",
					ValidacionCodigoError.VALOR_REQUERIDO);
		}

		List<ContratoDTO> listaContratoDTO = new ArrayList<ContratoDTO>();
