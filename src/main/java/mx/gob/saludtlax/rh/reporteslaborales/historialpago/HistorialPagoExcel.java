...
public class HistorialPagoExcel implements Serializable {
    ...
    private final InputStream is = ContratoExcel.class
            .getResourceAsStream("/plantillas/nomina/Historial_Pago.xlsx");
    ...
    private void llenarDetalles(List<HistorialPagoDetalleDTO> estructura) {
        ...
        for (HistorialPagoDetalleDTO detalle : estructura) {
            ...
            celdaInicioPeriodo.setCellValue(detalle.getInicioPeriodo() == null
                    ? "" : simpleDateFormat.format(detalle.getInicioPeriodo()));

            Cell celdaFinPeriodo = filaDetalle.createCell(FIN_PERIODO);
            celdaFinPeriodo.setCellValue(detalle.getFinPeriodo() == null ? ""
                    : simpleDateFormat.format(detalle.getFinPeriodo()));
