 ...
 * 
 */
package mx.gob.saludtlax.rh.reporteslaborales.historialpago;
 ...
 *
 */
public class HistorialPagoExcel implements Serializable {

	/**
	 * 
	 */
	private static final long serialVersionUID = 6378773614199473187L;

	private final InputStream is = ContratoExcel.class.getResourceAsStream("/plantillas/nomina/Historial_Pago.xlsx");

	/**
	 * El nombre de la hoja donde se encuentra el detalle
	 */
	private static final String NOMBRE_HOJA = "Historial_Pago";
	 ...
	 */
	public byte[] generar(List<HistorialPagoDetalleDTO> detalles) {
		try {
	...
	private void llenarDetalles(List<HistorialPagoDetalleDTO> estructura) {
		...
		for (HistorialPagoDetalleDTO detalle : estructura) {
			...
			Cell celdaHonorariosAsimilables = filaDetalle.createCell(HONORARIOS_ASIMILABLES);
			...
			celdaDiasEconomicos.setCellValue(
					(double) (detalle.getDiasEconomicos() == null ? 0 : detalle.getDiasEconomicos().doubleValue()));
			...
			celdaBono.setCellValue((double) (detalle.getBono() == null ? 0 : detalle.getBono().doubleValue()));
			...
			celdaAguinaldo
					.setCellValue((double) (detalle.getAguinaldo() == null ? 0 : detalle.getAguinaldo().doubleValue()));
			...
			celdaSubsidio
					.setCellValue((double) (detalle.getSubsidio() == null ? 0 : detalle.getSubsidio().doubleValue()));
			...
			celdaPrimaVacacional.setCellValue(
					(double) (detalle.getPrimaVacacional() == null ? 0 : detalle.getPrimaVacacional().doubleValue()));
			...
			celdaRetroactivo.setCellValue(
					(double) (detalle.getRetroactivo() == null ? 0 : detalle.getRetroactivo().doubleValue()));
			...
			celdaOtros.setCellValue((double) (detalle.getOtros() == null ? 0 : detalle.getOtros().doubleValue()));
			...
			celdaFaltaRetardo.setCellValue(
					(double) (detalle.getFaltasRetardos() == null ? 0 : detalle.getFaltasRetardos().doubleValue()));
			...
			celdaResponsabilidad.setCellValue(
					detalle.getResponsabilidades() == null ? 0 : detalle.getResponsabilidades().doubleValue());
			...
			celdaPrestamo
					.setCellValue((double) (detalle.getPrestamo() == null ? 0 : detalle.getPrestamo().doubleValue()));
			...
			celdaJuicioMercantil.setCellValue(
					(double) (detalle.getJuicioMercantil() == null ? 0 : detalle.getJuicioMercantil().doubleValue()));
			...
			celdaCuotaSindical.setCellValue(
					(double) (detalle.getCuotaSindical() == null ? 0 : detalle.getCuotaSindical().doubleValue()));
