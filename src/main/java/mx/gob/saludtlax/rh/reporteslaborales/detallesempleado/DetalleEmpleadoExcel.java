	...
	private void llenarDetalles(List<DetalleEmpleadoDTO> estructura) {
		...
		for (DetalleEmpleadoDTO detalle : estructura) {
			...
			celdaEnero.setCellValue(
					detalle.getFechaNacimiento() == null ? "" : format.format(detalle.getFechaNacimiento()));
			...
			celdaNumeroPadres
					.setCellValue(detalle.getNumeroPadre() == null ? "0" : detalle.getNumeroPadre().toString());
			...
			celdaOtroParentesco.setCellValue(
					detalle.getNumeroOtroParentesco() == null ? "0" : detalle.getNumeroOtroParentesco().toString());
			...
			celdaSueldo01.setCellValue(detalle.getSueldo01() == null ? "0"
					: "$ " + new DecimalFormat(pattern).format(detalle.getSueldo01()));

			Cell celdaSueldo14 = filaDetalle.createCell(SUELDO_14);
			celdaSueldo14.setCellValue(detalle.getSueldo14() == null ? "0"
					: "$ " + new DecimalFormat(pattern).format(detalle.getSueldo14()));
