...
public class AdjuntoAspiranteEJB implements AdjuntoAspirante {
    ...
    public int crear(InformacionAdjuntoDTO informacionDelAdjunto, byte[] adjunto) {
        ...
        if (informacionDelAdjunto == null) {
            throw new ValidacionException("No se puede guadar un adjunto que es nulo", ValidacionCodigoError.VALOR_REQUERIDO);
        }

        if (informacionDelAdjunto.getIdAspirante() == null || informacionDelAdjunto.getIdAspirante() < 1) {

            throw new ValidacionException("Debe incluir el ID del aspirante.", ValidacionCodigoError.VALOR_REQUERIDO);
        }

        if (informacionDelAdjunto.getEntidadContexto() == null) {
            throw new ValidacionException("Debe incluir el tipo documento", ValidacionCodigoError.VALOR_REQUERIDO);
        }
    ...
    public List<InformacionAdjuntoDTO> consultarInformacionAdjuntosPorEntidadContextoIdEntidadContexto(EntidadContexto entidadContexto, int idEntidadContexto) {
        ...
        return adjuntoService.consultarInformacionAdjuntosPorTipoDocumentoIdTipoDocumento(entidadContexto, idEntidadContexto);
    }
    ...
    public List<InformacionAdjuntoDTO> consultarInformacionAdjuntosPorEntidadContextoIdEntidadContextoIdDocumentoAdjuntable(EntidadContexto entidadContexto,
        ...
        if (idDocumentoAdjuntable < 1) {
            throw new ValidacionException("El idDocumentoAdjuntable no puede ser cero o negativo.", ValidacionCodigoError.NUMERO_NEGATIVO);
        }

        return adjuntoService.obtenerInformacionAduntosPorTipoDocumentoIdTipoDocumentoIdDocumentoAdjuntable(entidadContexto, idEntidadContexto,
                idDocumentoAdjuntable);
    }

    @Override
    public List<InformacionAdjuntoDTO> consultarInformacionAdjuntosPorIdAspirante(int idAspirante) {
        if (idAspirante < 1) {
            throw new ValidacionException("El ID del aspirante no puede ser cero o negativo", ValidacionCodigoError.NUMERO_NEGATIVO);
        }

        return adjuntoService.consultarInformacionAdjuntosPorIdAspirante(idAspirante);
    }

    @Override
    public void actualizar(Map<String, Object> parametros) {
        if (parametros == null) {
            throw new ValidacionException("No se puede actualizar el adjunto porque los parametros están vacios.", ValidacionCodigoError.VALOR_REQUERIDO);
        }
