...
public class ExpedienteEmpleadoService {
	...
	protected void crearExpediente(ExpedienteDTO dto) {
		...
		if (expedienteEmpleadoRepository.existeExpedienteAsignadoEmpleado(dto
				.getIdEmpleado())) {
			throw new BusinessException(contexto
					+ "El empleado ya tiene aperturado un expediente.");
		}

		if (expedienteEmpleadoRepository.existeNumeroExpediente(dto
				.getNumeroExpediente())) {
			throw new BusinessException(
					contexto
		...
		expediente.setNumeroExpediente(dto.getNumeroExpediente().trim()
				.toUpperCase());
