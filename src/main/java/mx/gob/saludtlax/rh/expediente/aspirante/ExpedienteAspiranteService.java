...
public class ExpedienteAspiranteService implements Serializable {
    ...
    protected void crearExpediente(ExpedienteAspiranteDTO dto) {
        ...
        if (!aspiranteRepository.existeIdAspirante(dto.getIdAspirante())) {
            throw new BusinessException(contexto
                    + "El identificador enviado no corresponde a un aspirante, seleccione otra opcion.");
        }

        if (expedienteAspiranteRepository
                .existeExpedienteAsignadoAspirante(dto.getIdAspirante())) {
            throw new BusinessException(contexto
                    + "El aspirante ya tiene aperturado un expediente.");
        }
    ...
    protected boolean tieneExpedienteAperturado(Integer idAspirante) {
        return expedienteAspiranteRepository
                .existeExpedienteAsignadoAspirante(idAspirante);
    }
    ...
    protected boolean validarNumeroExpedienteAspirante(
