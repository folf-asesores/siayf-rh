...
public class AdjuntoEmpleadoEJB implements AdjuntoEmpleado {
    ...
    public int crear(InformacionAdjuntoDTO informacionDelAdjunto,
        ...
        if (informacionDelAdjunto == null) {
            throw new NullPointerException(
                    "No se puede guardar un adjunto cuando la información es nula");
        }

        if (adjunto == null) {
            throw new NullPointerException(
                    "No se puede guardar un adjunto que es nulo");
        }

        if (informacionDelAdjunto.getIdEmpleado() == null
                || informacionDelAdjunto.getIdEmpleado() < 1) {
            throw new ValidacionException("Debe incluir el ID del empleado",
                    ValidacionCodigoError.VALOR_REQUERIDO);
        }

        if (informacionDelAdjunto.getEntidadContexto() == null) {
            throw new ValidacionException("Debe incluir la entidad contexto.",
                    ValidacionCodigoError.VALOR_REQUERIDO);
        }
    ...
    public List<InformacionAdjuntoDTO> consultarInformacionAdjuntosPorEntidadContextoIdEntidadContexto(
        ...
        if (entidadContexto == null) {
            throw new ValidacionException(
                    "El tipo de documento adjunto es requerido.",
                    ValidacionCodigoError.VALOR_REQUERIDO);
        }
    ...
    public List<InformacionAdjuntoDTO> consultarInformacionAdjuntosPorEntidadContextoIdEntidadContextoIdDocumentoAdjuntable(
        ...
        return adjuntoService
                .obtenerInformacionAduntosPorContextoEntidadIdContextoEntidadIdDocumentoAdjuntable(
                        entidadContexto, idEntidadContexto,
                        idDocumentoAdjuntable);
    }

    @Override
    public List<InformacionAdjuntoDTO> consultarInformacionAdjuntosPorIdEmpleado(
            int idEmpleado) {
        if (idEmpleado < 1) {
            throw new ValidacionException(
                    "El ID del empleado no puede ser cero o negativo",
                    ValidacionCodigoError.NUMERO_NEGATIVO);
        }
    ...
    @Override
    public List<String> consultarDocumentosAdjuntosPorEntidadContextoIdEntidadContexto(
        ...
        return adjuntoService.documentosAdjuntosPorEntidadContexto(
                entidadContexto, idEntidadContexto);
    }
