...
public class AdjuntoEmpleadoEJB implements AdjuntoEmpleado {
    ...
    @Inject
    private AdjuntoEmpleadoService adjuntoService;
    ...
    @Override
    public List<InformacionAdjuntoDTO> consultarInformacionAdjuntosPorEntidadContextoIdEntidadContexto(EntidadContexto entidadContexto, int idEntidadContexto) {
    ...
    @Override
    public List<InformacionAdjuntoDTO> consultarInformacionAdjuntosPorEntidadContextoIdEntidadContextoIdDocumentoAdjuntable(EntidadContexto entidadContexto,
        ...
        if (idDocumentoAdjuntable < 1) {
            throw new NullPointerException("El ID del documento adjuntable no puede ser cero o negativo");
        }

        return adjuntoService.obtenerInformacionAduntosPorContextoEntidadIdContextoEntidadIdDocumentoAdjuntable(entidadContexto, idEntidadContexto,
    ...
    public List<InformacionAdjuntoDTO> consultarInformacionAdjuntosPorIdEmpleado(int idEmpleado) {
        if (idEmpleado < 1) {
            throw new ValidacionException("El ID del empleado no puede ser cero o negativo", ValidacionCodigoError.NUMERO_NEGATIVO);
        }

        return adjuntoService.consultarInformacionAdjuntosPorIdEmpleado(idEmpleado);
    }

    @Override
    public List<String> consultarDocumentosAdjuntosPorEntidadContextoIdEntidadContexto(EntidadContexto entidadContexto, Integer idEntidadContexto) {

        if (entidadContexto == null) {
            throw new ValidacionException("La entidad contexto no puede ser nulo.", ValidacionCodigoError.VALOR_REQUERIDO);
        }

        if (idEntidadContexto == null || idEntidadContexto < 1) {
            throw new ValidacionException("El ID de la entidad contexto no puede ser nulo o menor que 1.", ValidacionCodigoError.VALOR_REQUERIDO);
        }
    ...
    public void actualizar(Map<String, Object> parametros) {
        if (parametros == null) {
            throw new NullPointerException("No se puede actualizar el adjunto porque los parametros están vacios.");
        }
    ...
    @Override
    public void importarExpedienteAspirante(Integer idAspirantente, Integer idEmpleado) {
        if (idAspirantente < 1) {
            throw new ValidacionException("El ID del aspirante no debe ser negativo.", ValidacionCodigoError.NUMERO_NEGATIVO);
        }

        if (idEmpleado < 1) {
            throw new ValidacionException("El ID del empleado no debe ser negativo.", ValidacionCodigoError.NUMERO_NEGATIVO);
        }
    ...
    public void elimnar(int idAdjunto) {
        if (idAdjunto < 1) {
            throw new ValidacionException("El ID del adjunto no puede ser cero o negativo", ValidacionCodigoError.VALOR_REQUERIDO);
        }
