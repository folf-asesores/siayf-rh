...
public class PostulacionVacanteService implements Serializable {
	...
	protected List<InfoCandidatoDTO> consultarCandidatosPostulacion(
		...
		List<CandidatoVacanteEntity> candidatosPostulados = candidatoVacanteRepository
				.listaCandidatosVacantesNoSeleccionadosPorIdPostulado(idPostulacion);
			...
			for (CandidatoVacanteEntity c : candidatosPostulados) {
				...
				if (c.getTipoCandidato() == EnumTipoCandidato.ASPIRANTE) {
					...
					if (aspiranteEntity.getFechaNacimiento() != null) {
						dto.setEdad(FechaUtil.calcularEdad(aspiranteEntity
								.getFechaNacimiento()));
					}
					...
					if (ValidacionUtil.esCadenaVacia(aspiranteEntity
							.getPerfilAcademico())) {
					...
					if (empleadoEntity.getFechaNacimiento() != null) {
						dto.setEdad(FechaUtil.calcularEdad(empleadoEntity
								.getFechaNacimiento()));
					}
					...
					if (ValidacionUtil.esCadenaVacia(empleadoEntity
							.getPerfilAcademico())) {
						dto.setEstudios("SIN HISTORIAL ACADEMICO REGISTRADO");
					} else {
	...
	protected List<InfoPostulacionDTO> consultarPostulacionesDisponibles() {
		...
		if (!resultadotaPostuacionesActivas.isEmpty()) {
			for (PostulacionVacanteEntity p : resultadotaPostuacionesActivas) {
				...
				postulacionDisponible.setFechaPostulacion(p
						.getFechaPostulacion());
				postulacionDisponible.setIdPostulacion(p
				...
				postulacionDisponible.setTipoContratacion(p
						.getInventarioVacante().getTipoContratacion()
						.getTipoContratacion());
				postulacionDisponible.setTipoPostulacion(p.getTipoPostulacion()
						.getTipoPostulacion());
				if (p.getInventarioVacante().getUltimoEmpleado() != null) {
					postulacionDisponible.setUltimoEmpleado(p
							.getInventarioVacante().getUltimoEmpleado()
							.getNombreCompleto());
				}
	...
	protected Integer postularCandidato(PostulacionDTO dto) {
		if (!ValidacionUtil.esNumeroPositivo(dto.getIdUsuario())) {
			throw new ValidacionException(
					"No se ha detectado usuario logeado.",
					ValidacionCodigoError.VALOR_REQUERIDO);
		}
		...
		if (puestoVacante == null) {
			throw new ValidacionException(
					"El puesto vacante  con identificador "
							+ dto.getIdVacante() + " no existe.",
		...
		UsuarioEntity usuario = usuarioRepository.obtenerPorId(dto
				.getIdUsuario());
		// Validar que no tenga una postulación activa.
		if (candidatoVacanteRepository.tienePostulacionActivaCandidato(
				idCandidato, tipoCandidato)) {
			throw new ReglaNegocioException(
	...
	protected void aprobarCandidatoPostulacion(Integer idPostulacion,
		...
		if (candidatoVacanteEntity.getTipoCandidato() == EnumTipoCandidato.EMPLEADO) {
			...
			String tipoContratacion = inventarioVacanteRepository
					.obtenerTipoContratacionEmpleadoActivo(candidatoVacanteEntity
							.getIdContexto());
		...
		vacanteSeleccionada.setIdCandidatoPostulado(candidatoVacanteEntity
				.getIdCandidatoVacante());
		inventarioVacanteRepository.actualizar(vacanteSeleccionada);

	}

	protected List<PuestoDisponibleDTO> consultarPuestosDisponibles() {
		List<PuestoDisponibleDTO> puestosDisponibles = new ArrayList<PuestoDisponibleDTO>();
