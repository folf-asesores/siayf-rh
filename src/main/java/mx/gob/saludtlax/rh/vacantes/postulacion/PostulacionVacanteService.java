...
public class PostulacionVacanteService implements Serializable {
    ...
    protected List<InfoCandidatoDTO> consultarCandidatosPostulacion(Integer idPostulacion) {

        if (!ValidacionUtil.esNumeroPositivo(idPostulacion)) {
            throw new ValidacionException("La postulación es requerida para obtener a los candidatos", ValidacionCodigoError.VALOR_REQUERIDO);
        }

        List<CandidatoVacanteEntity> candidatosPostulados = candidatoVacanteRepository.listaCandidatosVacantesNoSeleccionadosPorIdPostulado(idPostulacion);
        ...
        if (!candidatosPostulados.isEmpty()) {
            for (CandidatoVacanteEntity c : candidatosPostulados) {
                ...
                if (c.getTipoCandidato() == EnumTipoCandidato.ASPIRANTE) {
                    AspiranteEntity aspiranteEntity = aspiranteRepository.obtenerPorId(dto.getIdContexto());
                    ...
                    if (ValidacionUtil.esCadenaVacia(aspiranteEntity.getPerfilAcademico())) {
                    ...
                    EmpleadoEntity empleadoEntity = empleadoRepository.obtenerPorId(dto.getIdContexto());
                    ...
                    if (empleadoEntity.getFechaNacimiento() != null) {
                        dto.setEdad(FechaUtil.calcularEdad(empleadoEntity.getFechaNacimiento()));
                    }
                    ...
                    if (ValidacionUtil.esCadenaVacia(empleadoEntity.getPerfilAcademico())) {
                        dto.setEstudios("SIN HISTORIAL ACADEMICO REGISTRADO");
                    } else {
    ...
    protected List<InfoPostulacionDTO> consultarPostulacionesDisponibles() {
        ...
        List<PostulacionVacanteEntity> resultadotaPostuacionesActivas = postulacionVacanteRepository.obtenerListaPostuladoVacanteDisponibles();
        ...
        if (!resultadotaPostuacionesActivas.isEmpty()) {
            for (PostulacionVacanteEntity p : resultadotaPostuacionesActivas) {
                ...
                postulacionDisponible.setFechaPostulacion(p.getFechaPostulacion());
                postulacionDisponible.setIdPostulacion(p.getIdPostuladoVacante());
                postulacionDisponible.setPuesto(p.getInventarioVacante().getConfiguracion().getPuesto().getCodigo() + "-"
                        + p.getInventarioVacante().getConfiguracion().getPuesto().getPuesto());
                postulacionDisponible.setSueldo(p.getInventarioVacante().getConfiguracion().getSueldo());
                postulacionDisponible.setTipoContratacion(p.getInventarioVacante().getTipoContratacion().getTipoContratacion());
                postulacionDisponible.setTipoPostulacion(p.getTipoPostulacion().getTipoPostulacion());
                if (p.getInventarioVacante().getUltimoEmpleado() != null) {
                    postulacionDisponible.setUltimoEmpleado(p.getInventarioVacante().getUltimoEmpleado().getNombreCompleto());
                }
    ...
    protected Integer postularCandidato(PostulacionDTO dto) {
        if (!ValidacionUtil.esNumeroPositivo(dto.getIdUsuario())) {
            throw new ValidacionException("No se ha detectado usuario logeado.", ValidacionCodigoError.VALOR_REQUERIDO);
        }

        if (!ValidacionUtil.esNumeroPositivo(dto.getIdVacante())) {
            throw new ValidacionException("Es requerido indicar la vacante que quiere asignar.", ValidacionCodigoError.VALOR_REQUERIDO);
        }

        if (!ValidacionUtil.esNumeroPositivo(dto.getIdAspirante()) && !ValidacionUtil.esNumeroPositivo(dto.getIdEmpleado())) {
            throw new ValidacionException("Es requerido seleccionar un candidato.", ValidacionCodigoError.VALOR_REQUERIDO);
        }
        ...
        InventarioVacanteEntity puestoVacante = inventarioVacanteRepository.obtenerPorId(dto.getIdVacante());

        if (puestoVacante == null) {
            throw new ValidacionException("El puesto vacante  con identificador " + dto.getIdVacante() + " no existe.",
                    ValidacionCodigoError.REGISTRO_NO_ENCONTRADO);
        }

        if (puestoVacante.getDisponible().equals("NO")) {
            throw new ReglaNegocioException("La vacante seleccionada ya no se encuentra disponible.", ReglaNegocioCodigoError.VACANTE_NO_DISPONIBLE);
        }

        // Aplica para puestos que se crean por lotes y pueden tener cualquier
        // tipo de puesto, como los contratos federales.
        if (puestoVacante.getConfiguracion().getPuesto().getPuesto().equals("SIN PUESTO")) {

            if (!ValidacionUtil.esNumeroPositivo(dto.getIdPuesto())) {
                throw new ValidacionException("Es requerido indicar el puesto del trabajador.", ValidacionCodigoError.VALOR_REQUERIDO);
            }
            PuestoGeneralEntity puesto = puestoGeneralRepository.obtenerPorId(dto.getIdPuesto());
            ConfiguracionPresupuestoEntity datosLaborales = puestoVacante.getConfiguracion();
            datosLaborales.setPuesto(puesto);
            datosLaboralesRepository.actualizar(datosLaborales);
        }

        PostulacionVacanteEntity postulacion = postulacionVacanteRepository.tienePuestoPostulacionActiva(dto.getIdVacante());

        UsuarioEntity usuario = usuarioRepository.obtenerPorId(dto.getIdUsuario());
        // Validar que no tenga una postulación activa.
        if (candidatoVacanteRepository.tienePostulacionActivaCandidato(idCandidato, tipoCandidato)) {
            throw new ReglaNegocioException("El candidato tiene una postulación activa.", ReglaNegocioCodigoError.POSTULACION_ACTIVA);
        }

        int tipoPostulacion = 0;
        if (puestoVacante.getEstatus().getIdEstatus().equals(EnumEstatusPuesto.EMPLEADO_ACTIVO)) {
            tipoPostulacion = EnumTipoPostulacion.NUEVA_CREACION;
        } else {
            tipoPostulacion = EnumTipoPostulacion.VACANTE_LIBERADA;
        }

        TipoPostulacionEntity tipoPostulacionVacante = tipoPostulacionRepository.obtenerPorId(tipoPostulacion);
        ...
        Integer idPostulacion = candidato.getPostuladoVacante().getIdPostuladoVacante();
    ...
    protected InfoCandidatoDTO obtenerInformacionCandidato(Integer idVacante) {
        ...
        Integer idCandidatoPostulado = inventarioVacanteRepository.obtenerIdCandidatoPostulado(idVacante);
        ...
        CandidatoVacanteEntity candidato = candidatoVacanteRepository.obtenerPorId(idCandidatoPostulado);
        if (candidato == null) {
            throw new ReglaNegocioException("No se encontro el candidato con identificador " + idCandidatoPostulado, ReglaNegocioCodigoError.SIN_REGISTRO);
        }
        InfoCandidatoDTO infoCandidatoDTO = new InfoCandidatoDTO();
        if (candidato.getTipoCandidato() == EnumTipoCandidato.ASPIRANTE) {
            infoCandidatoDTO = consultaAspiranteService.obtenerInformacionCandidatoAspirante(candidato.getIdContexto());

        } else if (candidato.getTipoCandidato() == EnumTipoCandidato.EMPLEADO) {
            infoCandidatoDTO = busquedaEmpleadoService.obtenerInformacionCandidatoEmpleado(candidato.getIdContexto());
        }
    ...
    protected Integer obtenerIdPostulacionActiva(Integer idPuesto) {
        if (!ValidacionUtil.esNumeroPositivo(idPuesto)) {
            throw new ValidacionException("Es requerido el puesto para obtener la postulación.", ValidacionCodigoError.VALOR_REQUERIDO);
        }

        return postulacionVacanteRepository.obtenerIdPostulacionActiva(idPuesto);
    }

    protected void aprobarCandidatoPostulacion(Integer idPostulacion, Integer idCandidato) {

        if (!ValidacionUtil.esNumeroPositivo(idPostulacion)) {
            throw new ValidacionException("La postulación es requerido.", ValidacionCodigoError.VALOR_REQUERIDO);
        }

        if (!ValidacionUtil.esNumeroPositivo(idCandidato)) {
            throw new ValidacionException("Para realizar la aprobación es requerido seleccionar un candidato.", ValidacionCodigoError.VALOR_REQUERIDO);
        }

        PostulacionVacanteEntity postulacionVacante = postulacionVacanteRepository.obtenerPorId(idPostulacion);

        if (postulacionVacante == null) {
            throw new ValidacionException("No se encontró postulación con el identificador " + idPostulacion, ValidacionCodigoError.REGISTRO_NO_ENCONTRADO);
        }
        ...
        CandidatoVacanteEntity candidatoVacanteEntity = candidatoVacanteRepository.obtenerPorId(idCandidato);

        if (candidatoVacanteEntity.getTipoCandidato() == EnumTipoCandidato.EMPLEADO) {

            String tipoContratacion = inventarioVacanteRepository.obtenerTipoContratacionEmpleadoActivo(candidatoVacanteEntity.getIdContexto());
            ...
            if (tipoContratacion != null) {
                throw new ReglaNegocioException("El empleado cuenta con un puesto activo en " + tipoContratacion.toLowerCase()
                        + " es requerido darle de baja para poder aprobarlo en el nuevo puesto.", ReglaNegocioCodigoError.EMPLEADO_CON_PUESTO_ACTIVO);
            }
        ...
        InventarioVacanteEntity vacanteSeleccionada = postulacionVacante.getInventarioVacante();

        if (vacanteSeleccionada.getIdCandidatoPostulado() != null) {
            throw new ReglaNegocioException("La vacante ya ha sido ocupada", ReglaNegocioCodigoError.YA_AUTORIZADO);
        }
        vacanteSeleccionada.setIdCandidatoPostulado(candidatoVacanteEntity.getIdCandidatoVacante());
    ...
    protected List<PuestoDisponibleDTO> consultarPuestosDisponibles() {
        ...
        List<InventarioVacanteEntity> resultadoPuestosDisponibles = inventarioVacanteRepository.consultarPuestosDisponibles();
        if (!resultadoPuestosDisponibles.isEmpty()) {
            for (InventarioVacanteEntity i : resultadoPuestosDisponibles) {
                ...
                if (i.getUltimoEmpleado() != null) {
                    dto.setEmpleadoAnterior(i.getUltimoEmpleado().getNombreCompleto());
                }
                dto.setFolioVacante(i.getFolioVacante());
                dto.setIdPuesto(i.getIdVacante());
                dto.setSueldo(i.getConfiguracion().getSueldo());
                dto.setTipoContratacion(i.getTipoContratacion().getTipoContratacion());
                dto.setTipoNombramiento(i.getConfiguracion().getNombramiento().getDescripcion());
                dto.setPuesto(i.getConfiguracion().getPuesto().getCodigo() + "-" + i.getConfiguracion().getPuesto().getPuesto());

                if (i.getEstatus().getIdEstatus() == EnumEstatusPuesto.APERTURA_DESIGNACION) {
                    dto.setTipoVacante("APERTURA DESIGNACIÓN");
                } else if (i.getEstatus().getIdEstatus() == EnumEstatusPuesto.LIBERADA) {
