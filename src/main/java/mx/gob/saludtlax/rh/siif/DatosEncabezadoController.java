...
public class DatosEncabezadoController implements Serializable {
	...
	public String irPrincipal() {
		System.out.println("irPrincipal: this.view.getIdSiifEncabezado():: " + this.view.getIdSiifEncabezado());
		
		Integer CB =datoEncabezado.obtenerCuentaBancaria(this.view.getIdSiifEncabezado());
		System.out.println("Clave Bancaria):: " + CB);
		
		if(CB!=null && CB==4){
			
			view.setListDatosPersonalesLista(datoEncabezado.obtenerListaDatosEncabezadoProspera(this.view.getIdSiifEncabezado()));
		}else{
			if(CB==7){
				view.setListDatosPersonalesLista(datoEncabezado.obtenerListaDatosEncabezadoProspera(this.view.getIdSiifEncabezado()));
			}else{
				...
				if(CB==2){
					view.setListDatosPersonalesLista(datoEncabezado.obtenerListaDatosEncabezadoContrato(this.view.getIdSiifEncabezado()));
				}else{
					view.setListDatosPersonalesLista(datoEncabezado.obtenerListaDatosEncabezado(this.view.getIdSiifEncabezado()));
				}
