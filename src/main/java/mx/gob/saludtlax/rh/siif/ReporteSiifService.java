...
public class ReporteSiifService {
    ...
    public List<SiifBitacoraDTO> obtenerReporteSiifPorPeriodo(String periodo, Integer anio) {
        ...
        Query query = session
                .createSQLQuery(
                        "SELECT id_siif_bitacoras AS idSiifBitacora, " + " id_nombramiento AS idNombramiento, " + " nombramiento_descripcion AS nombramiento, "
                                + " total_nomina AS totalNomina, " + " status, " + " anio, " + " fecha_importado AS fechaImportado, "
                                + " fecha_envio AS fechaEnvio, " + " periodo AS periodo, " + " total_percepciones AS totalPercepciones, "
                                + " total_deducciones AS totalDeducciones, " + " total_neto AS totalNeto, " + " id_tipo_nomina AS idTipoNomina, "
                                + " id_cuenta_bancaria AS idCuentaBancaria, " + " cuenta_bancaria AS cuentaBancaria, " + " tipo_nomina AS tipoNomina,"
                                + " periodo_reporte AS periodoAfectacion," + " anio_reporte AS anioAfectacion," + " tipo_archivo AS tipoArchivo " + " FROM "
    ...
    public SIIFEncabezadoDTO obtenerEncabezadoPorId(int idSiifEncabezado) {
        ...
        Session session = entityManager.unwrap(Session.class);
        Query query = session
                .createSQLQuery("SELECT	" + "en.id_siif_encabezado as idSIIFEncabezado," + "en.id_nomina as idNomina," + " en.id_poder as idPoder,"
                        + " en.id_tipo_nomina as idTipoNomina," + " en.fecha_fin_quincena as fechaFinQuincena,"
                        + " en.id_tipo_emision_nomina as idTipoEmisionNomina," + " en.id_cuenta_bancaria as idCuentaBancaria,"
                        + " en.percepciones AS percepciones," + " en.deducciones AS deducciones," + " en.neto AS neto,"
                        + " en.id_estado_nomina as idEstadoNomina" + " FROM siif_encabezados AS en" + " WHERE en.id_siif_encabezado = (:id_siif_encabezado)")
    ...
    public SIIFEncabezadoDTO actualizarEncabezado(SIIFEncabezadoDTO encabezadoDTO) {
        Session session = entityManager.unwrap(Session.class);
        Query query = session
                .createSQLQuery("UPDATE " + " siif_encabezados AS se " + " SET " + "se.id_nomina = (:id_nomina) , " + " se.id_tipo_nomina = (:id_tipo_nomina) ,"
                        + " se.id_tipo_emision_nomina = (:id_tipo_emision_nomina) ," + " se.id_cuenta_bancaria = (:id_cuenta_bancaria)" + " WHERE "
                        + " se.id_siif_encabezado = (:id_siif_encabezado)")
                .setParameter("id_nomina", encabezadoDTO.getIdNomina()).setParameter("id_tipo_nomina", encabezadoDTO.getIdTipoNomina())
    ...
    public SIIFEncabezadoDTO actualizarCheques(Integer id, Integer qna) {
        Session session = entityManager.unwrap(Session.class);
        Query query = session.createSQLQuery("CALL usp_actualizar_cheques_siif(:idSiifEncabezdo,:qna)").setParameter("idSiifEncabezdo", id).setParameter("qna",
                qna);
        query.executeUpdate();
        return null;
    }
    ...
    public String obtenerTipoEmisionNomina(EstructuraNominaDatEntity estructuraNominaEntity) {
        ...
        Query query = session
                .createSQLQuery(
                        "SELECT num_cheq " + "FROM estructuras_nominas_trailers AS t " + "WHERE t.rfc = :rfc AND " + "id_siif_bitacoras = :idSiifBitacora ")
                .setParameter("rfc", estructuraNominaEntity.getRfc()).setParameter("idSiifBitacora", estructuraNominaEntity.getIdSiifBitacora());
    ...
    public EstructuraNominaDatEntity obtenerEstructuraNominaDatEntity(String rfc, Integer idReporteSiif) {
        EstructuraNominaDatEntity estructuraNominaDatEntity = (EstructuraNominaDatEntity) entityManager
                .createQuery("SELECT n FROM EstructuraNominaDatEntity c WHERE c.rfc = :rfc AND  idReporteSiif = :idReporteSiif").setParameter("rfc", rfc)
    ...
    public void actualizarSiifEncabezado(SIIFEncabezadoDTO siifEncabezado) {
        ...
        CuentasBancariasEntity cuentaBancaria = entityManager.find(CuentasBancariasEntity.class, bitacora.getIdCuentaBancaria());
    ...
    public SiifBitacoraDTO obtenerSiiifBitacoraById(Integer idSiifBitacora) {
        ...
        if (query.list().size() > 0) {
            ...
            if (result.getIdCuentaBancaria() != 7 || result.getIdCuentaBancaria() != 4) {
                query = session.createSQLQuery("CALL usp_encabezados_inconsistencia(:idSiifBitacora)").setParameter("idSiifBitacora", idSiifBitacora);
                query.setResultTransformer(Transformers.aliasToBean(SIIFEncabezadoDTO.class));
            } else {
    ...
    public SiifBitacoraDTO importarNominaTheosToSIIF(UploadedFile dat, UploadedFile tra, SiifBitacoraDTO bitacora) {
        ...
        try {
            ...
            while (stTra.hasMoreTokens()) {
                ...
                estructuraNominaTraEntity.setConceptoSiif(estructuraNominaTraEntity.getConcep() + estructuraNominaTraEntity.getPtaAnt());
    ...
    @TransactionAttribute(TransactionAttributeType.REQUIRES_NEW)
    public SiifBitacoraDTO importarNominaContSIIF(UploadedFile cont, SiifBitacoraDTO bitacora, PaqueteEntradaFederalDTO paqueteEntrada) {
        int i = 0;
    ...
    public SiifBitacoraDTO clasificarEncabezados(SiifBitacoraDTO bitacora) {
        Session session = entityManager.unwrap(Session.class);
        Query query = session.createSQLQuery("CALL sp_clasificar_nomina_estructura(:idSiifBitacora)").setParameter("idSiifBitacora",
        ...
        for (SIIFEncabezadoDTO dto : result) {
            ...
            if (Arrays.asList(idPensiones).contains(dto.getIdTipoNomina())) {
                ...
                if (dto.getIdNombramiento().intValue() == 4) {
                    ...
                    query = session.createSQLQuery("CALL sp_clasificar_reg_seguro_popular(:idSiifBitacora, :idNombramiento)")
                            .setParameter("idSiifBitacora", bitacora.getIdSiifBitacora()).setParameter("idNombramiento", dto.getIdNombramiento());
                    query.executeUpdate();

                    query = session.createSQLQuery("CALL usp_obtener_encabezados_seguro_popular(:idSiifBitacora)").setParameter("idSiifBitacora",
                            bitacora.getIdSiifBitacora());
                    query.setResultTransformer(Transformers.aliasToBean(SIIFEncabezadoDTO.class));
                    @SuppressWarnings("unchecked")
    ...
    public void clasificarEncabezados2(SiifBitacoraDTO bitacora) {
        ...
        Session session = entityManager.unwrap(Session.class);
        Query query = session.createSQLQuery("CALL usp_clasificar_nomina_estructura_siif(:idSiifBitacora)").setParameter("idSiifBitacora",
                bitacora.getIdSiifBitacora());
        query.executeUpdate();
    ...
    public void clasificarEncabezados3(SiifBitacoraDTO bitacora) {
        ...
        Session session = entityManager.unwrap(Session.class);
        Query query = session.createSQLQuery("CALL usp_clasificar_nomina_estructura_siif_2(:idSiifBitacora)").setParameter("idSiifBitacora",
                bitacora.getIdSiifBitacora());
        query.executeUpdate();
    ...
    public SiifBitacoraDTO clasificarRegularizadosSiif(List<SIIFEncabezadoDTO> result, SiifBitacoraDTO bitacora, PaqueteEntradaFederalDTO entrada) {
        ...
        for (SIIFEncabezadoDTO dto : result) {
            ...
            if (Arrays.asList(idPensiones).contains(dto.getIdTipoNomina())) {
                ...
                if (dto.getIdNombramiento() != null && dto.getIdNombramiento().intValue() == 4 && regProcesado == 0) {
                    ...
                    query = session
                            .createSQLQuery("INSERT INTO siif_encabezados( " + "fecha_fin_quincena, " + "id_tipo_emision_nomina, " + "percepciones, "
                                    + "deducciones, " + "neto, " + "id_siif_bitacora, " + "id_nombramiento, " + "sub_programa ) "
                                    + "SELECT	cast(n.p_pago_F as date) AS fechaFinQuincena, " + "n.tipo_emision_nomina AS idTipoEmisionNomina, "
                                    + "SUM(n.per)	AS percepciones, " + "SUM(n.ded) AS deducciones, " + "SUM(n.neto) AS neto, " + ":idSiifBitacora, "
                                    + "n.id_nombramiento, " + "'Seguro Popular Federal' " + "FROM estructuras_nominas AS n "
                                    + "LEFT OUTER JOIN siif_seguro_popular_federal AS s " + "ON(	s.rfc = n.rfc AND s.rfc IS NOT NULL) "
                                    + "WHERE n.id_siif_bitacoras =:idSiifBitacora AND n.id_nombramiento =:idNombramiento "
                                    + "AND s.quincena =:quincena AND s.periodo =:anio AND s.id_tipo_nomina =:tipoNomina " + "GROUP BY n.tipo_emision_nomina ")
                            .setParameter("idSiifBitacora", bitacora.getIdSiifBitacora()).setParameter("idNombramiento", dto.getIdNombramiento())
                            .setParameter("quincena", Integer.parseInt(entrada.getPeriodoAfectacion()))
                            .setParameter("anio", "A" + entrada.getAnioAfectacion().toString()).setParameter("tipoNomina", entrada.getIdTipoNomina());
                    query.executeUpdate();
                    System.out.println("Crea encabezados SPF::::: ");
                    
                    query = session.createSQLQuery("SELECT e.id_siif_encabezado AS idSIIFEncabezado, " + "e.id_nomina AS idNomina, "
                    ...
                    for (SIIFEncabezadoDTO dtoSPF : resultSegPopFed) {
                        ...
                        System.out.print("::: " + dtoSPF.getIdNombramiento() + "::: " + dtoSPF.getSubPrograma() + ":::\n");
                    }
                    
                    query = session.createSQLQuery("SELECT e.id_siif_encabezado AS idSIIFEncabezado, " + "e.id_nomina AS idNomina,  e.id_poder AS idPoder, "
                            + "e.id_tipo_nomina AS idTipoNomina,  e.fecha_fin_quincena AS fechaFinQuincena, "
                            + "e.id_tipo_emision_nomina AS idTipoEmisionNomina, "
                            + "e.id_cuenta_bancaria AS idCuentaBancaria,  e.percepciones AS percepciones, " + "e.deducciones AS deducciones,  e.neto AS neto, "
                            + "e.id_estado_nomina AS idEstadoNomina,  e.id_siif_bitacora AS idSIIFBitacora, "
                            + "e.id_nombramiento AS idNombramiento,  e.sub_programa AS subPrograma " + "FROM siif_encabezados AS e "
                            + "WHERE e.id_siif_bitacora =:idSiifBitacora " + "AND e.sub_programa = 'Seguro Popular Federal' ")
                            .setParameter("idSiifBitacora", bitacora.getIdSiifBitacora());
                    query.setResultTransformer(Transformers.aliasToBean(SIIFEncabezadoDTO.class));
                    @SuppressWarnings("unchecked")
                        ...
                        for (SIIFEncabezadoDTO idSegPopFed : resultEncabezadoSPF) {
                            ...
                            query = session
                                    .createSQLQuery("SELECT spf.rfc FROM siif_seguro_popular_federal AS spf "
                                            + "WHERE spf.quincena =:quincena AND spf.id_tipo_nomina =:tipoNomina " + "AND spf.periodo =:periodo ")
                                    .setParameter("quincena", Integer.parseInt(entrada.getPeriodoAfectacion()))
                            ...
                            for (String rfcSPF : resultRfcSPF) {
                                query = session
                                        .createSQLQuery("UPDATE estructuras_nominas AS n " + "SET n.id_siif_encabezados =:@id_siif_encabezadoSPF, "
                                                + "n.id_subfuente_financiamiento = 245 " + "WHERE n.id_siif_bitacoras =:idSiifBitacora "
                                                + "AND	n.id_nombramiento =:idNombramiento " + "AND	n.rfc =:rfc " + "AND n.tipo_emision_nomina =:tipoEmision ")
                                        .setParameter("@id_siif_encabezadoSPF", id_siif_encabezadoSPF)
                                        .setParameter("idSiifBitacora", bitacora.getIdSiifBitacora()).setParameter("idNombramiento", dto.getIdNombramiento())
                                        .setParameter("rfc", rfcSPF).setParameter("tipoEmision", tipoEmision);
                                query.executeUpdate();
                            }
                    ...
                    query = session
                            .createSQLQuery("INSERT INTO siif_encabezados( " + "fecha_fin_quincena, " + "id_tipo_emision_nomina, " + "percepciones, "
                                    ...
                                    + "SELECT	cast(n.p_pago_F as date) AS fechaFinQuincena, " + "n.tipo_emision_nomina AS idTipoEmisionNomina, "
                                    + "SUM(n.per)	AS percepciones, " + "SUM(n.ded) AS deducciones, " + "SUM(n.neto) AS neto, " + ":idSiifBitacora, "
                                    + "n.id_nombramiento, " + "'Federales' " + "FROM estructuras_nominas AS n " + "WHERE n.id_siif_bitacoras =:idSiifBitacora "
                    ...
                    query = session.createSQLQuery("SELECT e.id_siif_encabezado AS idSIIFEncabezado, " + "e.id_nomina AS idNomina,  e.id_poder AS idPoder, "
                            ...
                            + "e.id_estado_nomina AS idEstadoNomina,  e.id_siif_bitacora AS idSIIFBitacora, "
                            + "e.id_nombramiento AS idNombramiento,  e.sub_programa AS subPrograma " + "FROM siif_encabezados AS e "
                            + "WHERE e.id_siif_bitacora =:idSiifBitacora " + "AND e.sub_programa = 'Federales' ")
                            .setParameter("idSiifBitacora", bitacora.getIdSiifBitacora());
                    query.setResultTransformer(Transformers.aliasToBean(SIIFEncabezadoDTO.class));
                    @SuppressWarnings("unchecked")
                    ...
                    if (resultEncabezado != null) {
                        for (SIIFEncabezadoDTO idFederal : resultEncabezado) {
                            ...
                            query = session
                                    .createSQLQuery("UPDATE estructuras_nominas AS n " + "SET n.id_siif_encabezados =:@id_siif_encabezadoF "
                                            + "WHERE n.id_siif_bitacoras =:idSiifBitacora " + "AND n.id_nombramiento =:idNombramiento "
                                            + "AND n.puesto IN ('M02002','M02048','M02059','M03002','M03005','M03011','M03024','M03025') "
                                            + "AND n.tipo_emision_nomina =:tipoEmision ")
                                    .setParameter("@id_siif_encabezadoF", id_siif_encabezadoF).setParameter("idSiifBitacora", bitacora.getIdSiifBitacora())
                                    .setParameter("idNombramiento", dto.getIdNombramiento()).setParameter("tipoEmision", tipoEmision);
                            query.executeUpdate();
                    ...
                    ///>>>
                    query = session.createSQLQuery("INSERT INTO siif_encabezados( " + "fecha_fin_quincena, " + "id_tipo_emision_nomina, " + "percepciones, "
                            + "deducciones, " + "neto, " + "id_siif_bitacora, " + "id_nombramiento, " + "sub_programa ) "
                            + "SELECT	cast(n.p_pago_F as date) AS fechaFinQuincena, " + "n.tipo_emision_nomina AS idTipoEmisionNomina, "
                            + "SUM(n.per)	AS percepciones, " + "SUM(n.ded) AS deducciones, " + "SUM(n.neto) AS neto, " + ":idSiifBitacora, "
                            + "n.id_nombramiento, " + "'Seguro Popular' " + "FROM estructuras_nominas AS n " + "WHERE n.id_siif_bitacoras =:idSiifBitacora "
                            + "AND n.id_nombramiento =:idNombramiento " + "AND n.id_siif_encabezados IS NULL")
                            .setParameter("idSiifBitacora", bitacora.getIdSiifBitacora()).setParameter("idNombramiento", dto.getIdNombramiento());
                    query.executeUpdate();
                    ...
                    query = session.createSQLQuery("SELECT e.id_siif_encabezado AS idSIIFEncabezado, " + "e.id_nomina AS idNomina,  e.id_poder AS idPoder, "
                            ...
                            + "e.id_estado_nomina AS idEstadoNomina,  e.id_siif_bitacora AS idSIIFBitacora, "
                            + "e.id_nombramiento AS idNombramiento,  e.sub_programa AS subPrograma " + "FROM siif_encabezados AS e "
                            + "WHERE e.id_siif_bitacora =:idSiifBitacora " + "AND e.sub_programa = 'Seguro Popular' AND e.neto IS NOT NULL ")
                            .setParameter("idSiifBitacora", bitacora.getIdSiifBitacora());
                    query.setResultTransformer(Transformers.aliasToBean(SIIFEncabezadoDTO.class));
                    @SuppressWarnings("unchecked")
                    ...
                    if (resultEncabezadoSP != null && resultEncabezadoSP.size() > 0) {
                        for (SIIFEncabezadoDTO idSegPop : resultEncabezadoSPF) {
                            ...
                            query = session
                                    .createSQLQuery("UPDATE estructuras_nominas AS n " + "SET n.id_siif_encabezados =:@id_siif_encabezadoSP, "
                                            + "n.id_subfuente_financiamiento = 245 " + "WHERE n.id_siif_bitacoras =:idSiifBitacora "
                                            + "AND n.id_nombramiento =:idNombramiento " + "AND n.tipo_emision_nomina =:tipoEmision "
                                            + "AND n.id_siif_encabezados IS NULL ")
                                    .setParameter("@id_siif_encabezadoSP", id_siif_encabezadoSP).setParameter("idSiifBitacora", bitacora.getIdSiifBitacora())
                                    .setParameter("idNombramiento", dto.getIdNombramiento()).setParameter("tipoEmision", tipoEmision);
                            query.executeUpdate();
                        ...
                        query = session.createSQLQuery("DELETE FROM siif_encabezados " + "WHERE id_siif_bitacora =:idSiifBitacora " + "AND neto IS NULL ")
                                .setParameter("idSiifBitacora", bitacora.getIdSiifBitacora());
                        query.executeUpdate();
                    ...
                    query = session
                            .createSQLQuery("SELECT e.id_siif_encabezado AS idSIIFEncabezado, " + "e.id_nomina AS idNomina, " + "e.id_poder	AS idPoder, "
                                    ...
                                    + "e.id_estado_nomina	AS idEstadoNomina, " + "e.id_siif_bitacora	AS idSIIFBitacora, "
                                    + "e.id_nombramiento AS idNombramiento, " + "e.sub_programa AS subPrograma " + "FROM siif_encabezados AS e "
                                    + "WHERE  e.id_siif_bitacora =:idSiifBitacora " + "AND e.sub_programa IS NOT NULL")
                            .setParameter("idSiifBitacora", bitacora.getIdSiifBitacora());
                    query.setResultTransformer(Transformers.aliasToBean(SIIFEncabezadoDTO.class));
                    @SuppressWarnings("unchecked")
                    List<SIIFEncabezadoDTO> resultReg = query.list();
    ...
    public SiifBitacoraDTO clasificarEncabezadosContratos(SiifBitacoraDTO bitacora) {
        ...
        Query query = session.createSQLQuery("CALL usp_clasificar_nomina_contrato(:idSiifBitacora)").setParameter("idSiifBitacora",
                bitacora.getIdSiifBitacora());
        query.setResultTransformer(Transformers.aliasToBean(SIIFEncabezadoDTO.class));
        @SuppressWarnings("unchecked")
    ...
    public SiifBitacoraDTO cambiarClaveConceptosTra(SiifBitacoraDTO bitacora) {
        ...
        Session session = entityManager.unwrap(Session.class);
        Query query = session.createSQLQuery("CALL sp_cambiar_clave_concepto(:idSiifBitacora)").setParameter("idSiifBitacora", bitacora.getIdSiifBitacora());
        query.executeUpdate();
        return bitacora;
    }
    ...
    public SiifBitacoraDTO cambiarClaveConceptosSiif(SiifBitacoraDTO bitacora) throws RemoteException, RollbackException {
    ...
    public SiifBitacoraDTO asignarEncabezadosTrailers(SiifBitacoraDTO bitacora) {
        ...
        Session session = entityManager.unwrap(Session.class);
        Query query = session.createSQLQuery("CALL usp_asignar_encabezados_a_trailers(:id_siif_bitacoras) ").setParameter("id_siif_bitacoras",
                bitacora.getIdSiifBitacora());
        query.executeUpdate();
    ...
    public SiifBitacoraDTO asignarEncabezadosTrailersSiif(SiifBitacoraDTO bitacora) {
        System.out.println("CALL usp_asignar_encabezados_a_trailers(:id_siif_bitacoras)");
        Session session = entityManager.unwrap(Session.class);
        Query query = session.createSQLQuery(" UPDATE estructuras_nominas_trailers AS nt " + "INNER JOIN estructuras_nominas AS en "
                + "ON (en.rfc = nt.rfc AND en.id_siif_bitacoras = nt.id_siif_bitacoras) " + "SET nt.id_estructuras_nominas = en.id_estructuras_nominas, "
                + "nt.id_siif_encabezados = en.id_siif_encabezados " + "WHERE en.id_siif_encabezados = nt.id_siif_encabezados "
    ...
    public SiifBitacoraDTO calcularEncabezados(SiifBitacoraDTO bitacora) {
        Session session = entityManager.unwrap(Session.class);
        Query query = session.createSQLQuery("CALL usp_calcular_encabezados(:id_siif_bitacoras) ").setParameter("id_siif_bitacoras",
                bitacora.getIdSiifBitacora());
        query.executeUpdate();
    ...
    public SiifBitacoraDTO calcularTotalesEncabezadosContratos(SiifBitacoraDTO bitacora) {
        Session session = entityManager.unwrap(Session.class);
        Query query = session.createSQLQuery("CALL sp_calcular_totales_encabezados_contratos(:id_siif_bitacoras) ").setParameter("id_siif_bitacoras",
                bitacora.getIdSiifBitacora());
        query.setResultTransformer(Transformers.aliasToBean(SIIFEncabezadoDTO.class));
        @SuppressWarnings("unchecked")
        List<SIIFEncabezadoDTO> result = query.list();
    ...
    public SiifBitacoraDTO calcularTotalesEncabezadosContrato(SiifBitacoraDTO bitacora) {
        Session session = entityManager.unwrap(Session.class);
        Query query = session.createSQLQuery("CALL usp_calcular_totales_encabezados_contrato(:id_siif_bitacoras) ").setParameter("id_siif_bitacoras",
                bitacora.getIdSiifBitacora());
        query.setResultTransformer(Transformers.aliasToBean(SIIFEncabezadoDTO.class));
        @SuppressWarnings("unchecked")
        List<SIIFEncabezadoDTO> result = query.list();
    ...
    public SiifBitacoraDTO verificarDatos(SiifBitacoraDTO bitacora) {
        ...
        Session session = entityManager.unwrap(Session.class);
        Query query = session.createSQLQuery("CALL sp_calcular_totales_encabezados(:id_siif_bitacoras) ").setParameter("id_siif_bitacoras",
                bitacora.getIdSiifBitacora());
        query.setResultTransformer(Transformers.aliasToBean(SIIFEncabezadoDTO.class));
        @SuppressWarnings("unchecked")
        List<SIIFEncabezadoDTO> result = query.list();
    ...
    public void dispersionToEntity(SiifDeudoresDiversosDTO d, Integer id) {
        for (EstructuraNominaTrailersDTO DTO : nominaTrailersPorDeudores(id, d.getRfc(), d.getImporte())) {
            EstructuraNominaTrailersEntity entity = nominaTrailersDAO.obtenerPorId(DTO.getIdEstructurasNominasTrailers());
            entity.setConcep("29");
            entity.setPtaAnt("DD");
    ...
    public List<SiifDeudoresDiversosDTO> deudoresPorIdPeriodo(String quincena) {
        Session session = entityManager.unwrap(Session.class);
        Query query = session.createSQLQuery("" + "SELECT id_deudores_diversos AS idDeudoresDiversos, " + "rfc AS rfc, " + "quincena AS quincena, "
                + "importe AS importe " + "FROM siif_deudores_diversos WHERE quincena =:quincena").setParameter("quincena", quincena);
    ...
    public void eliminaBitacoraPorId(Integer idSiifBitacora) {
        ...
        Query query = session.createSQLQuery("CALL usp_elimina_siif_bitacora(:idSiifBitacora)").setParameter("idSiifBitacora", idSiifBitacora);
        query.executeUpdate();
    ...
    public void eliminaEncabezadoPorId(Integer idSiifBitacora) {
        ...
        Query query = session.createSQLQuery("CALL usp_elimina_siif_encabezados(:idSiifBitacora)").setParameter("idSiifBitacora", idSiifBitacora);
        query.executeUpdate();
    ...
    public List<SIIFEncabezadoDTO> obtenerListaEncabezadosSiif(SiifBitacoraDTO bitacora) {
        Session session = entityManager.unwrap(Session.class);
        Query query = session.createSQLQuery("CALL usp_clasificar_nomina_estructura_siif_3(:idSiifBitacora)").setParameter("idSiifBitacora",
                bitacora.getIdSiifBitacora());
        query.setResultTransformer(Transformers.aliasToBean(SIIFEncabezadoDTO.class));
        @SuppressWarnings("unchecked")
        List<SIIFEncabezadoDTO> result = query.list();
    ...
    public List<EstructuraNominaTrailersDTO> obtenerNominaTrailersSiif(Integer idSiifBitacora) {
        Session session = entityManager.unwrap(Session.class);
        Query query = session
                .createSQLQuery("" + "SELECT id_concepto AS idConcepto " + "FROM estructuras_nominas_trailers WHERE id_siif_bitacoras =:idSiifBitacora")
    ...
    @TransactionAttribute(TransactionAttributeType.REQUIRES_NEW)
    public List<EstructuraNominaTrailersDTO> nominaTrailersPorDeudores(Integer idSiifBitacoras, String rfc, BigDecimal importe) {
        Session session = entityManager.unwrap(Session.class);
        Query query = session
                .createSQLQuery("" + "SELECT id_estructuras_nominas_trailers AS idEstructurasNominasTrailers, " + "rfc AS rfc, " + "concep AS concep, "
                        + "importe AS importe "
                        + "FROM estructuras_nominas_trailers WHERE id_siif_bitacoras =:idSiifBitacoras AND rfc =:rfc AND importe =:importe")
                .setParameter("idSiifBitacoras", idSiifBitacoras).setParameter("rfc", rfc).setParameter("importe", importe);
        query.setResultTransformer(Transformers.aliasToBean(EstructuraNominaTrailersDTO.class));
        @SuppressWarnings("unchecked")
        List<EstructuraNominaTrailersDTO> result = query.list();
        return result;
    ...
    @TransactionAttribute(TransactionAttributeType.REQUIRES_NEW)
    public List<EstructuraNominaTrailersDTO> nominaTrailersPorDispersion(Integer idSiifBitacoras, String rfc, BigDecimal importe) {
        Session session = entityManager.unwrap(Session.class);
        Query query = session
                .createSQLQuery("" + "SELECT id_estructuras_nominas_trailers AS idEstructurasNominasTrailers, " + "rfc AS rfc, " + "concep AS concep, "
                        + "importe AS importe "
    ...
    public List<EstructuraNominaTrailersDTO> listaDeudores(Integer idSiifBitacoras) {
        ...
        Query query = session
                .createSQLQuery("" + "SELECT id_estructuras_nominas_trailers AS idEstructurasNominasTrailers, " + "rfc AS rfc, " + "concep AS concep, "
                        + "num_emp AS numEmp, " + "importe AS importe "
    ...
    public List<EstructuraNominaTrailersDTO> listaDispersion(Integer idSiifBitacoras) {
        ...
        Session session = entityManager.unwrap(Session.class);
        Query query = session
                .createSQLQuery("" + "SELECT id_estructuras_nominas_trailers AS idEstructurasNominasTrailers, " + "rfc AS rfc, " + "num_cheq AS numCheq, "
                        + "importe AS importe " + "FROM estructuras_nominas_trailers WHERE id_siif_bitacoras =:idSiifBitacoras")
