...
/***		< < < < < Inserciones > > > > >		***/
	...
	private Integer i;public Integer getI(){return i;}public void setI(Integer i){this.i = i;}//variable de conteo
...
/***		< < < < < Metodos > > > > >		***/
	...
	public List<SiifBitacoraDTO> obtenerReporteSiifPorPeriodo(String periodo, Integer anio) {
		...
		Query query = session.createSQLQuery(
				"SELECT id_siif_bitacoras AS idSiifBitacora, "
						+ " id_nombramiento AS idNombramiento, "
						+ " nombramiento_descripcion AS nombramiento, "
						+ " total_nomina AS totalNomina, "
						+ " status, "
						+ " anio, "
						+ " fecha_importado AS fechaImportado, "
	...
	public List<TipoNominaDTO> obtenerTipoNominaList() {
		Session session = entityManager.unwrap(Session.class);
		Query query = session.createSQLQuery(
				"SELECT id_tipo_nomina AS idTipoNomina, "
	...
	***/
	...
	public SiifBitacoraDTO cambiarClaveConceptosTraContratos(SiifBitacoraDTO bitacora) {
		Session session = entityManager.unwrap(Session.class);
		Query query = session.createSQLQuery("CALL sp_cambiar_clave_concepto_prospera(:idSiifBitacora)")
	...
	public SiifBitacoraDTO verificarDatosContratos(SiifBitacoraDTO bitacora) {
		Session session = entityManager.unwrap(Session.class);
		Query query = session.createSQLQuery("CALL sp_calcular_totales_encabezados_prospera(:id_siif_bitacoras) ")
	...
	private SiifBitacoraDTO settingDataDAT(List<EstructuraDTO> data, SiifBitacoraDTO bitacora) {
		...
		try {
			while (arrayIterator.hasNext()) {
				...
				estructuraDTO.setIdSubfuentaFinanciamiento(genericoDTO.getDato(77, Integer.class));
	...
	public void registrarListaEstructuraDat(List<EstructuraContratosDatDTO> listaEstructura) {
		try {
			...
			for (EstructuraContratosDatDTO trailers : listaEstructura) {
				...
				estructuraEntity.setIdSubfuentaFinanciamiento(trailers.getIdSubfuentaFinanciamiento());
...
//		< < < < < Clasificar > > > > >
	...
	public SiifBitacoraDTO clasificarEncabezadosContratos(SiifBitacoraDTO bitacora) {
		...
		Session session = entityManager.unwrap(Session.class);
		Query query = session.createSQLQuery("CALL sp_clasificar_prospera_estructura(:idSiifBitacora)")
				.setParameter("idSiifBitacora", bitacora.getIdSiifBitacora());
		query.executeUpdate();
		return bitacora;
	}
