...
public class RegistroAspiranteController implements Serializable {
	...
	@PostConstruct
	public void inicio() {
		this.view = new RegistroAspiranteView();
	...
	public void menu(int panel) {
		...
		this.view.setMostrarAdjuntarDocumentoHistorial(false);
		view.setMostrarExperienciaLaboral(false);
		view.setMostrarEscuestaPersonal(false);
		this.view.setMostrarExpediente(false);
		...
		try {
			...
			case HISTORIAL_ACADEMICO:
				...
				this.view.setAltaHistorialAcademico(new HistorialAcademicoDTO());
				this.view.setHistorialAcademicoSeleccionado(new HistorialAcademicoDTO());

				this.view.setMostrarHistorialAcademico(true);
				this.view.setMostrarHistorialAcademicoRegistro(true);
				this.view.setMostrarAdjuntarDocumentoHistorial(false);

				this.view.setListaEscolaridades(SelectItemsUtil.listaEscolaridad(catalogosEJB.listaEscolaridad()));
				this.view.setListaComprobantesEstudio(
						SelectItemsUtil.listaCatalogos(catalogosEJB.listaComprobantesEstudios()));

				this.view.setListaHistorialAcademicoAspirante(bolsaTrabajo.obtenerListaHistorialAcademico(idAspirante));
			...
			case PROFESION:
				...
				List<ProfesionDTO> profesiones = profesion.obtenerListaProfesionPorIdAspirante(this.idAspirante);
			...
			case ESPECIALIDAD:
				...
				List<EspecialidadDTO> especialidades = especialidad
						.obtenerListaEspecialidadPorIdAspirante(this.idAspirante);
			...
			case EXPEDIENTE:
				...
				List<CatalogoDTO> lista = catalogosEJB
						.consultarDocumentosExpedientesClasificacion(EnumClasificacionExpediente.DOCUMENTOS_PERSONALES);
				this.view.setListaTiposDocumentosExpediente(SelectItemsUtil.listaCatalogos(lista));

				this.view.setMostrarExpediente(true);

				List<InformacionAdjuntoDTO> documentosExpedientes = adjuntoAspirante
						.consultarInformacionAdjuntosPorIdAspirante(this.idAspirante);

				this.view.setDocumentosExpedientes(documentosExpedientes);

				if (expedienteAspirante.tieneExpedienteAperturado(this.idAspirante)) {
					System.out.println("Tiene aperturado un expediente");
					String numeroExpediente = expedienteAspirante.numeroExpedienteAspirante(this.idAspirante);
					Integer idExpediente = expedienteAspirante.obtenerIdExpedienteAspirante(this.idAspirante);
	...
	public void municipiosPorEstado() {
		...
		this.view.getAltaAspirante().getDireccionDTO().setIdMunicipio(0);
		this.view.getAltaAspirante().getDireccionDTO().setIdAsentamiento(0);
		Integer idEstado = this.view.getAltaAspirante().getDireccionDTO().getIdEstado();
		if (idEstado != 0) {
			List<CatalogoDTO> municipios = catalogosEJB.consultarMunicipiosPorEstado(idEstado);

			this.view.setListaMunicipios(SelectItemsUtil.listaCatalogos(municipios));

		}
	}

	public void asentamientosPorMunicipio() {
		Integer idMunicipio = this.view.getAltaAspirante().getDireccionDTO().getIdMunicipio();
		if (idMunicipio != 0) {
			List<CatalogoDTO> asentamientos = catalogosEJB.consultarAsantamientosPorMunicipios(idMunicipio);
			this.view.getListaAsentamientos().clear();
			this.view.setListaAsentamientos(SelectItemsUtil.listaCatalogos(asentamientos));
	...
	public void registrarDatosGenerales() {
		try {
			...
			if (this.view.getPersonasDependientes() != null) {
				for (String personaDependiente : this.view.getPersonasDependientes()) {
					if (personaDependiente.equals("CONYUGE")) {
						this.view.getAltaAspirante().setNumeroConyuges(1);
			...
			idAspirante = bolsaTrabajo.crearAspirante(this.view.getAltaAspirante());
	...
	public void subirFotoAspirate(FileUploadEvent evento) {
		...
		if (!evento.getFile().getContentType().equals(png) & !evento.getFile().getContentType().equals(jpg)
				& !evento.getFile().getContentType().equals(jpeg)) {
			...
			if (idAspirante == 0) {
				...
				try {
					...
					this.view.setMostrarSinFoto(false);
					this.view.setMostrarFoto(true);

					JSFUtils.infoMessageEspecifico("info", "Subir foto: ",
							"La foto " + nombreAdjunto + " se ha subido correctamente.");
				} catch (Exception ex) {
					JSFUtils.errorMessageEspecifico("error", "Error al adjuntar el archivo",
							"El archivo no se ha adjuntado.");
				}
			}
		}
	}
	...
	public void subirDocumentoAdjunto(FileUploadEvent evento) {
		try {
			...
			this.view.getDocumentosExpedientes().clear();
	...
	public void mostrarAdjuntarDocumentoHistorial(HistorialAcademicoDTO historialAcademicoSeleccionado) {
		...
		if (historialAcademicoSeleccionado != null) {
			...
			this.view.setHistorialAcademicoSeleccionado(historialAcademicoSeleccionado);

			this.view.setMostrarAdjuntarDocumentoHistorial(true);
			this.view.setMostrarHistorialAcademicoRegistro(false);
			...
			this.view.setListaDocumentosHistorialAcademico(SelectItemsUtil.listaCatalogos(documentosAdjuntables));

		} else {
			JSFUtils.warningMessageEspecifico("historialAcademico", "",
					"Seleccione el grado academico al que desea adjuntarle documentacion.");
		}
	}

	public void subirDocumentoAdjuntoHistorial(FileUploadEvent evento) {
		try {
			if (!ValidacionUtil.esNumeroPositivo(this.view.getAltaHistorialAcademico().getIdHistorialAcademico())) {
				throw new BusinessException(
						"Adjuntar documento: Es requerido seleccionar el tipo de documento que se está adjuntando, seleccione una opcion.");
			} else if (ValidacionUtil.esCadenaVacia(this.view.getNumeroExpediente())) {
				throw new BusinessException(
						"Adjuntar documento historial: Por favor aperture el expediente para poder adjuntarle el documento.");
			} else {

			}
			System.out.println(this.view.getIdDocumentoAdjuntable());
			UploadedFile archivo = evento.getFile();
			String nombreAdjunto = archivo.getFileName();
			byte[] adjunto = archivo.getContents();
			...
			this.view.setMostrarAdjuntarDocumentoHistorial(false);
			this.view.setIdDocumentoAdjuntable("");
			...
			JSFUtils.infoMessageEspecifico("info", "Adjuntar documento",
					"El documento " + nombreAdjunto + " se ha adjuntado correctamente.");
	...
	public void registrarProfesionAspirante() {
		try {
			profesion.crearProfesionAspirante(this.view.getIdProfesion(), this.idAspirante);
			this.view.setIdProfesion(0);
			List<ProfesionDTO> profesiones = profesion.obtenerListaProfesionPorIdAspirante(this.idAspirante);
	...
	public void registrarEspecialidadAspirante() {
		try {
			especialidad.crearEspecialidadAspirante(this.view.getIdEspecialidad(), this.idAspirante);
			List<EspecialidadDTO> especialidades = especialidad
					.obtenerListaEspecialidadPorIdAspirante(this.idAspirante);
	...
	public void registrarEncuestaPersonal() {
		try {
			...
			escuestaPersonal.setIdAspirante(this.idAspirante);
	...
	public void eliminarExperienciaLaboral() {
		try {
			bolsaTrabajo.eliminarExperienciaLaboral(
					this.view.getExperienciaLaboralAspiranteSeleccionado().getIdExperienciaLaboralAspirante());

			this.view.setMostrarDialogExperiencia(false);

			this.view.setListaExperienciaLaboralAspirante(bolsaTrabajo.obtenerListaExperienciaLaboral(idAspirante));
	...
	public void experienciaLaboralSeleccionado(ExperienciaLaboralAspiranteDTO experienciaLaboral) {
		this.view.setMostrarDialogExperiencia(true);
		this.view.setExperienciaLaboralAspiranteSeleccionado(experienciaLaboral);
	}

	public void cerrarDialogExperiencia() {
		this.view.setMostrarDialogExperiencia(false);
		this.view.setExperienciaLaboralAspiranteSeleccionado(new ExperienciaLaboralAspiranteDTO());
	}

	public void eliminarHistorialAcademico() {
		try {
			bolsaTrabajo.eliminarHistorialAcademico(
					this.view.getHistorialAcademicoSeleccionado().getIdHistorialAcademico());
			this.view.setListaHistorialAcademicoAspirante(bolsaTrabajo.obtenerListaHistorialAcademico(idAspirante));
			this.view.setMostrarDialogHistorial(false);
	...
	public void historialAcademicoSeleccionado(HistorialAcademicoDTO historialSeleccionado) {
		this.view.setHistorialAcademicoSeleccionado(historialSeleccionado);
		this.view.setMostrarDialogHistorial(true);
	}
	...
	public void aperturarExpediente() {
		try {
			...
			this.view.getExpediente().setIdAspirante(idAspirante);

			// Creando expediente del aspirante
			expedienteAspirante.crearExpediente(this.view.getExpediente());

			this.view.setMostrarAperturaExpediente(false);
			this.view.setMostrarActualizacionExpediente(true);
			this.view.setNumeroExpediente(this.view.getExpediente().getNumeroExpediente().toUpperCase());
			this.view.setImagenExpediente("expediente_aperturado.png");
	...
	public void mostrarDialogoEliminarHistorialAcademico() {
		this.view.setMostrarDialogoEliminarHistorial(true);
	}
	...
	public void mostrarPanelHistorialAcademico() {
		...
		this.view.setMostrarEscuestaPersonal(false);
		this.view.setMostrarAdjuntarDocumento(false);
		this.view.setMostrarExpediente(false);

		this.view.setListaEscolaridades(SelectItemsUtil.listaEscolaridad(catalogosEJB.listaEscolaridad()));
		this.view.setListaComprobantesEstudio(SelectItemsUtil.listaCatalogos(catalogosEJB.listaComprobantesEstudios()));
	...
	public void validatorTipoDocAdjHistorialAcademico(FacesContext context, UIComponent component, Object value)
			throws ValidatorException, BusinessException {
