...
public class RegistroHabilidadPersonalValidator {
    ...
    public Object validate(InvocationContext context) throws Exception {
        ...
        HabilidadesPersonalesAspiranteDTO dto = (HabilidadesPersonalesAspiranteDTO) context
                .getParameters()[0];
        String contexto = "Registro Habilidades Personales: ";

        if (dto == null) {
            throw new BusinessException(contexto
                    + "Los datos de la encuesta personal del aspirante son requeridos");
        }

        if (ValidacionUtil.esCadenaVacia(dto.getIdioma())) {
            throw new BusinessException(contexto
                    + "El idioma o los idiomas que habla son requeridos");
        }
        ...
        if (ValidacionUtil.esCadenaVacia(dto.getMaquinaTallerDomina())) {
            throw new BusinessException(contexto
                    + "El equipo de oficina o el taller que domine es requerido");
        }

        if (ValidacionUtil.esCadenaVacia(dto.getFuncionesOficinaDomina())) {
            throw new BusinessException(
                    contexto + "La funcion o funciones de oficna es requerido");
        }
        ...
        if (ValidacionUtil.esCadenaVacia(dto.getSoftwareDomina())) {
            throw new BusinessException(contexto
                    + "El nombre del software o software's que domine es requerido");
        }
