...
public class ConsultaAspiranteService {
    ...
    protected List<InfoAspiranteDTO> consultarAspirantePorCriterio(
            FiltroDTO filtroDTO) {
        List<InfoAspiranteDTO> aspirantes = new ArrayList<>();
        if (filtroDTO.getTipoFiltro() == EnumTipoFiltro.NOMBRE_RFC_CURP) {
            ...
            aspirantes = aspiranteRepository
                    .aspirantesDisponiblesPorCriterio(filtroDTO.getCriterio());
        }

        if (filtroDTO
                .getTipoFiltro() == EnumTipoFiltro.NOMBRE_RFC_CURP_PROFESION) {
            aspirantes = aspiranteRepository
                    .aspirantesDisponiblesPorCriterioPerfil(
    ...
    public InfoCandidatoDTO obtenerInformacionCandidatoAspirante(
            Integer idAspirante) {
        ...
        if (aspiranteEntity == null) {
            throw new ReglaNegocioException(
                    "No se ha encontrado aspirante con identificador"
                            + idAspirante,
        ...
        infoCandidatoDTO.setEdad(
                FechaUtil.calcularEdad(aspiranteEntity.getFechaNacimiento()));
    ...
    protected InfoAspiranteDTO obtenerDetalleAspirantePorId(
            Integer idAspirante) {
        ...
        if (aspiranteEntity.getIdEstatus()
            ...
            throw new ReglaNegocioException(
                    "El aspirante ha sido registrado como empleado.",
                    ReglaNegocioCodigoError.ASPIRANTE_AUTORIZADO);
        }

        if (aspiranteEntity.getIdEstatus()
                .equals(EnumEstatusAspirante.RECHAZADO)) {
            throw new ReglaNegocioException(
                    "El aspirante ha sido rechazado, actualice a disponible.",
                    ReglaNegocioCodigoError.ASPIRANTE_AUTORIZADO);
        }
    ...
    protected AspiranteDTO obtenerAspirantePorIdentificador(
            Integer idAspirante) {
        ...
        if (aspiranteEntity == null) {
            ...
            if (direccionEntity != null) {
                ...
                direccionDTO.setIdEstado(
                        direccionEntity.getEstado().getIdEstado() == null ? 0
