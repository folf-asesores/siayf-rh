...
public class HistorialAcademicoService {
     ...
     */
    protected void crearHistorialAcademicoAspirante(HistorialAcademicoDTO historialAcademico) {
        String contexto = "Registro Historial académico: ";

        AspiranteEntity aspirante = aspiranteRepository.obtenerPorId(historialAcademico.getAspirante());

        if (aspirante == null) {
            throw new ReglaNegocioException(contexto + "El identificador del aspirante no existe, registre los datos personales",
        ...
        EscolaridadEntity escolaridad = escolaridadRepository.obtenerPorId(historialAcademico.getEscolaridad());
        // Obteniendo Documento Comprobatoria por identificador unico
        ComprobanteEstudioEntity comprobanteEstudio = documentosComprobatoriosRepository.obtenerPorId(historialAcademico.getComprobanteEstudio());

        registroHistorialAcademico.setIdAspirante(historialAcademico.getAspirante());
        ...
        registroHistorialAcademico.setCursando(historialAcademico.getCursando());
        // Es falso asta que se le adjunte una dopcumentacion
        registroHistorialAcademico.setTieneDocumentacion(historialAcademico.getTieneDocumentacion());
        registroHistorialAcademico.setMaximoEstudio(historialAcademico.getEsMaximoEstudio());
        ...
        List<String> profesiones = historialAcademicoRepository.consultarProfesionesAspirante(aspirante.getIdAspirante());
        if (!profesiones.isEmpty()) {
            ...
            if (!perfilAspirante.toString().trim().isEmpty()) {
                ...
                aspirante.setPerfilAcademico(perfilAspirante.toString().substring(0, ultimaPosicion));
     ...
     * @param historialAcademico
     */
    protected void actualizarHistorialAcademicoAspirante(HistorialAcademicoDTO historialAcademico) {
        ...
        if (aspirante == null) {
            throw new ReglaNegocioException(contexto + "El identificador del aspirante no existe, registre los datos generales",
                    ReglaNegocioCodigoError.SIN_REGISTRO);
        }
        // Obteniendo Historial Academico por identificador unico
        HistorialAcademicoEntity registroHistorialAcademico = historialAcademicoRepository.obtenerPorId(historialAcademico.getIdHistorialAcademico());
    ...
    protected void eliminarHistorialAcademico(Integer idHistorialAcademico) {
        ...
        // Obteniendo Historial Academico por identificador unico
        HistorialAcademicoEntity eliminarHistorialAcademico = historialAcademicoRepository.obtenerPorId(idHistorialAcademico);
     ...
     * @return
     */
    protected List<HistorialAcademicoDTO> obtenerListaHistorialAcademicoPorIdAspirante(Integer idAspirante) {
        List<HistorialAcademicoDTO> listaDTOs = new ArrayList<>();

        List<HistorialAcademicoEntity> listaEntities = historialAcademicoRepository.consultarHistorialAcademicoAspirante(idAspirante);
        if (!listaEntities.isEmpty()) {
            for (HistorialAcademicoEntity entity : listaEntities) {
                ...
                dto.setComprobanteEstudio(entity.getComprobanteEstudio().getIdComprobanteEstudio());
                dto.setEstatusComprobanteEstudio(entity.getComprobanteEstudio().getEstatus());
                ...
                dto.setNombreComprobanteEstudio(entity.getComprobanteEstudio().getEstatus());
                dto.setNombreEscolaridad(entity.getEscolaridad().getEscolaridad());
