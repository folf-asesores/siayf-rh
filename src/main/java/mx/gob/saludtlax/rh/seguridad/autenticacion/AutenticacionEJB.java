...
public class AutenticacionEJB implements Autenticacion {
    ...
    @Override
    public String iniciarSesion(String nombreUsuario, String contrasenya,
            boolean mantenerSesion) {

        if (ValidacionUtil.esCadenaVacia(nombreUsuario)) {
            throw new SeguridadException("El nombre de usuario es requerido",
                    SeguridadCodigoError.NOMBRE_DE_USUARIO_REQUERIDO);
        }

        if (ValidacionUtil.esCadenaVacia(contrasenya)) {
            throw new SeguridadException(
                    "Se requiere que ingrese una contraseña.",
                    SeguridadCodigoError.CONTRASENYA_REQUERIDA);
        }

        return autenticacionService.iniciarSesion(nombreUsuario, contrasenya,
                mantenerSesion);
    }
    ...
    public boolean ipEstaBloqueada(String ip) {
        if (ValidacionUtil.esCadenaVacia(ip)) {
            throw new ValidacionException("La IP está vacio o es nula.",
