...
public class UsuarioService {
	...
	protected void actualizar(UsuarioDTO dto) {
		if (dto == null) {
			throw new ValidationException("No es posible actualizar un usuario nulo",
					ValidacionCodigoError.VALOR_REQUERIDO);
		}

		if (dto.getIdUsuario() < 1) {
			throw new ValidationException("No es posible actualizar un usuario con un ID negativo o cero",
	...
	private static UsuarioEntity convertirDTOAEntidad(UsuarioDTO dto, UsuarioEntity entity,
			PerfilUsuarioEntity perfil) {
		if (entity == null) {
