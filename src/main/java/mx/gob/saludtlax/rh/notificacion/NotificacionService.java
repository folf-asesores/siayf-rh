...
public class NotificacionService {
    ...
    protected NotificacionDTO obtenerPorToken(String token) throws SistemaException {
        try {
            ...
            NotificacionDTO notificacionDTO = convertirEntidadADTO(notificacion);
        ...
        try {
            ...
            if(destinatarios != null && !destinatarios.isEmpty()) {
                for (NotificacionDestinatarioEntity destinatario : destinatarios) {
                    ...
                    message.setRecipients(Message.RecipientType.TO, InternetAddress.parse(destinatario.getDestinatario().getCorreo()));
                    message.setSubject("Notificación: " + notificacion.getAsunto());

                    StringBuilder cuerpoCorreo = new StringBuilder();

                    cuerpoCorreo.append(notificacion.getCuerpo());
                    cuerpoCorreo.append("<hr /><p>Para ver mayor: información:</p>");
                    cuerpoCorreo.append("<p>Haga clic <a href=\"");
                    cuerpoCorreo.append(rootPath);
                    cuerpoCorreo.append("/notificaciones/ver-notificacion?token=");
                    cuerpoCorreo.append(destinatario.getToken());
                    cuerpoCorreo.append("\">aquí</a> </p>");
                    cuerpoCorreo.append("<p>En caso de la de no ver poder entrar con el vinculo copie y pege esta ruta en su navegador: ");
                    cuerpoCorreo.append(rootPath);
                    cuerpoCorreo.append("/notificaciones/ver-notificacion?token=");
                    cuerpoCorreo.append(destinatario.getToken());
                ...
                LOGGER.error("La notificación no se envio porque no exitia ningún destinatario (email).");
            }
