...
public class ConceptosNominaFederalesController implements Serializable {
	...
	public Boolean esMandoMedio(ConfiguracionConceptoPuestoDTO configPuesto) {
		PuestoGeneralDTO puesto = puestoGeneral.puestoPorId(configPuesto.getId_puesto_general());
		Boolean res = false;
	...
	public void obtenerConfiguraciones() {
		...
		for (ConfiguracionConceptoPuestoDTO confDto : listaConf) {
			for (int i = 0; i < listaNewPuestos.size(); i++) {
				if (listaNewPuestos.get(i).getIdPuestoGeneral().compareTo(confDto.getId_puesto_general()) == 0) {
	...
	/**
	 * Metodo usado para el componente pick List el cual fue remplazado por una tabla seleccionable
	 */
