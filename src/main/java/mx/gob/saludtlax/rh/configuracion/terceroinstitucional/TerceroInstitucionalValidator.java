...
public class TerceroInstitucionalValidator {
    ...
    public Object validate(InvocationContext context) throws Exception {
        ...
        TerceroInstitucionalDTO dto = (TerceroInstitucionalDTO) context
                .getParameters()[0];
        String contexto = "Validación campos: ";

        if (dto == null) {
            throw new ValidationException(contexto
                    + "Son requeridos los datos para el registro, dto nulo notifique a sistemas");
        }

        if (dto.getNumero().isEmpty()) {
            throw new ValidationException(
                    contexto + "El número es requerido, ingrese el número");
        }
        ...
        if (ValidacionUtil.esCadenaVacia(dto.getConceptoDeduccion())) {
            throw new ValidationException(contexto
                    + "El concepto de deducción es requerido, ingrese el concepto de deducción");
        }

        if (ValidacionUtil
                .esCadenaVacia(dto.getContrapartidaIdentificadora())) {
            throw new ValidationException(contexto
                    + "La contrapartida identificadora es requerida, ingrese la contrapartida identificadora");
        }
