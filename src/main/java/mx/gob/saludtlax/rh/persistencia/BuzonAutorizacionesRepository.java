...
public class BuzonAutorizacionesRepository extends GenericRepository<BuzonAutorizacionesEntity, Integer> {
     ...
     *            pendientes)
     */
    public List<BuzonAutorizacionesEntity> solicitudesAutorizacionesUsuarioPorEstatus(Integer idUsuario, boolean autorizado) {

        List<BuzonAutorizacionesEntity> autorizaciones = em
                .createQuery("SELECT b FROM BuzonAutorizacionesEntity AS b WHERE b.idUsuario =:idUsuario AND b.autorizado =:autorizado",
                        BuzonAutorizacionesEntity.class)
                .setParameter("autorizado", autorizado).setParameter("idUsuario", idUsuario).getResultList();
        return autorizaciones;
    }

    public Boolean esUsuarioAutorizaNomina(OperacionSistemaEntity accion, Integer idEntidadContexto, Integer idUsuario) {
        List<DetalleBuzonAutorizacionEntity> autorizaciones = em
                .createQuery(" SELECT d                                                                       "
                        ...
                        + " AND                                                                  "
                        + " d.usuarioEntity.idUsuario =:idUsuario                                ", DetalleBuzonAutorizacionEntity.class)
                .setParameter("accion", accion).setParameter("idEntidadContexto", idEntidadContexto).setParameter("idUsuario", idUsuario).getResultList();
