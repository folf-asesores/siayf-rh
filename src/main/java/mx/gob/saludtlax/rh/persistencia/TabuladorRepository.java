...
public class TabuladorRepository extends GenericRepository<TabuladorEntity, Integer> implements Serializable {
    ...
    public List<TabuladorEntity> consultarTabuladoresPorTipo(Integer idTipoTabulador) {
        return em.createQuery("SELECT t FROM TabuladorEntity as t WHERE t.tipoTabulador.idTipoTabulador =:idTipoTabulador ORDER BY t.puestoGeneral.codigo ASC",
    ...
    public BigDecimal obtenerSueldoDiarioIdTabulador(Integer idTabulador) {
        return em.createQuery("SELECT t.sueldoDiario FROM TabuladorEntity as t WHERE t.idTabulador =:idTabulador", BigDecimal.class)
                .setParameter("idTabulador", idTabulador).getSingleResult();
    }
    ...
    public TabuladorEntity obtenerTabuladorPuestoTipoTabulador(Integer idPuesto, Integer tipoTabulador) {
        try {
            return em.createQuery(
                    ...
                    TabuladorEntity.class).setParameter("idPuesto", idPuesto).setParameter("tipoTabulador", tipoTabulador).getSingleResult();
        ...
        try {
            return em
                    .createQuery(" SELECT t FROM TabuladorEntity AS t " + " WHERE t.puestoGeneral.idPuestoGeneral =:idPuesto "
                            + " AND t.tipoTabulador.idTipoTabulador =:tipoTabulador " + " ORDER BY t.ejercicioFiscal desc ", TabuladorEntity.class)
                    .setParameter("idPuesto", idPuestoGeneral).setParameter("tipoTabulador", tipoTabulador).setMaxResults(1).getSingleResult();
        } catch (NoResultException exception) {
            return null;
        }
    ...
    public TabuladorEntity obtenerTabuladorPorPuesto(Integer idPuestoGeneral, Integer tipoTabulador, Integer anioFiscal) {
        // TODO modificar el 3 por el ejercicio
        try {
            return em
                    .createQuery("SELECT t FROM TabuladorEntity AS t WHERE t.puestoGeneral.idPuestoGeneral =:idPuesto "
                            + "AND t.tipoTabulador.idTipoTabulador =:tipoTabulador AND t.ejercicioFiscal =:ejercicioActual ", TabuladorEntity.class)
                    .setParameter("idPuesto", idPuestoGeneral).setParameter("tipoTabulador", tipoTabulador).setParameter("ejercicioActual", anioFiscal)
