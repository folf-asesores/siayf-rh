 ...
 * 
 */
public class ExpedienteEmpleadoRepository extends
		GenericRepository<ExpedienteEmpleadoEntity, Integer> {
    ...
    private static final long serialVersionUID = -5862760775058476152L;
        ...
        public ExpedienteEmpleadoEntity obtenerPorIdEmpleado(Integer idEmpleado){
			...
			return em.createQuery(
					"SELECT e FROM ExpedienteEmpleadoEntity AS e WHERE e.idEmpleado =:idEmpleado",
					classType).setParameter("idEmpleado", idEmpleado)
					.getSingleResult();
		} catch (NoResultException exception) {
			return null;
		} catch (NonUniqueResultException exception) {
			throw new ReglaNegocioException(
					"Se ha encontrado más de un expediente asignado al empleado.", ReglaNegocioCodigoError.EXISTE_MAS_DE_UN_EXPEDIENTE);
		}
        }
	...
	public boolean existeExpedienteAsignadoEmpleado(Integer idEmpleado) {
		try {
			em.createQuery(
					"SELECT e.idExpedienteEmpleado FROM ExpedienteEmpleadoEntity AS e WHERE e.idEmpleado =:idEmpleado",
			...
			throw new BusinessException(
					"Se ha encontrado mas de un expediente asignado al empleado.");
		}
	}
	...
	public boolean existeNumeroExpediente(String numeroExpediente) {
		try {
			em.createQuery(
					"SELECT e.idExpedienteEmpleado FROM ExpedienteEmpleadoEntity AS e WHERE e.numeroExpediente =:numeroExpediente",
					Integer.class)
	...
	public String numeroExpedienteEmpleado(Integer idEmpleado) {
		...
		try {
			return em
					.createQuery(
							"SELECT e.numeroExpediente FROM ExpedienteEmpleadoEntity AS e WHERE e.idEmpleado =:idEmpleado",
							String.class)
					.setParameter("idEmpleado", idEmpleado).getSingleResult();

		} catch (NoResultException exception) {
			throw new BusinessException(
					"El empleado no tiene un expediente aperturado.");
		}
	}
