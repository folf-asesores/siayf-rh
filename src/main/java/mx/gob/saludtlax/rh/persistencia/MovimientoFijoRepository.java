 ...
 */
public class MovimientoFijoRepository
        extends GenericRepository<MovimientoFijoEntity, Integer> {
    ...
    public List<MovimientoNominaDTO> obtenerMovimientosFijosPorEmpleado(
        ...
        Query query = session.createSQLQuery("SELECT "
                ...
                + "anio_final AS anioFinal, " + "anio_inicial as anioInicial, "
                + "fecha_documento as fechaDocumento, "
                + "folio_documento as folio,"
                + "id_tipo_movimiento as idTipoMovimiento," + "dias as dias "
                + "FROM movimientos_empleado_nomina where id_empleado=:idEmpleado")
                .setParameter("idEmpleado", idEmpleado);
        query.setResultTransformer(
                Transformers.aliasToBean(MovimientoNominaDTO.class));
        ...
        for (MovimientoNominaDTO dto : datosDto) {
            if (dto.getIdTerceroInstitucional() != null) {
                TerceroInstitucionalEntity tI = em.createQuery(
                        "Select t From TerceroInstitucionalEntity as t where t.idTerceroInstitucional=:idTercero",
                        TerceroInstitucionalEntity.class)
                        .setParameter("idTercero",
                                dto.getIdTerceroInstitucional())
                        .getSingleResult();

                dto.setTerceroInstitucional(
                        tI.getNumero() + "-" + tI.getConceptoDeduccion() + "-"
                                + tI.getContrapartidaIdentificadora());
            }
            datosDto2.add(dto);
    ...
    public List<MovimientoNominaDTO> obtenerMovimientosFijosPorEmpleadoytipoMov(
            Integer idEmpleado, TipoMovimientoNominaDTO movDto) {
        Session session = em.unwrap(Session.class);
        Query query = session.createSQLQuery("SELECT "
                + "id_movimiento_fijo AS idMovimientoFijo, "
                + "id_tercero_institucional AS idTerceroInstitucional, "
                ...
                .setParameter("idEmpleado", idEmpleado).setParameter(
                        "idTipoMovimiento", movDto.getIdTimpoMovimiento());
        query.setResultTransformer(
                Transformers.aliasToBean(MovimientoNominaDTO.class));
        @SuppressWarnings("unchecked")
        List<MovimientoNominaDTO> datosDto = query.list();
    ...
    public List<MovimientoNominaDTO> obtenerMovimiento(Integer idEmpleado,
        ...
        try {
            ...
            for (MovimientoNominaDTO dto : datosDto) {
                if (dto.getIdTerceroInstitucional() != null) {
                    ...
                    dto.setTerceroInstitucional(tI.getNumero() + "-"
                            + tI.getConceptoDeduccion() + "-"
                            + tI.getContrapartidaIdentificadora());
                }
                datosDto2.add(dto);
    ...
    public List<MovimientoNominaDTO> obtenerMovimientosFijos() {
        ...
        Query query = session.createSQLQuery("SELECT "
                ...
                + "quincena_inicial AS quincenaInicial, "
                + "quincena_final AS quincenaFinal, "
                + "anio_final AS anioFinal, " + "folio_documento as folio,"
                + "fecha_documento, " + "anio_inicial as anioInicial, "
                + "fecha_documento as fechaDocumento, "
                + "folio_documento as folio,"
                + "id_tipo_movimiento as idTipoMovimiento," + "clave,"
                + "estatus, " + "dias as dias "
    ...
    public Integer periodoActual(Integer tipoPeriodo, Integer ejercicioFiscal,
            Date fechaActual) {
        Integer res = 0;

        try {

            EjercicioFiscalEntity ejercicioF = em.createQuery(
                    ...
                    .getSingleResult();
            PeriodoCalendariosEntity query = em.createQuery(
    ...
    public MovimientoNominaDTO obtenerMovimientoPorDatosArchivo(
        ...
        try {
            ...
            Query query = session.createSQLQuery("SELECT "
                    ...
                    + "id_tercero_institucional AS idTerceroInstitucional, "
                    + "quincena_operacion AS quincenaOperacion, "
                    + "rfc as rfc, " + "anyo_operacion AS anyoOperacion, "
                    + "importe_descontado AS importeDescontado, "
                    + "quincena_inicial AS quincenaInicial, "
                    + "quincena_final AS quincenaFinal, "
                    + "anio_final AS anioFinal, "
