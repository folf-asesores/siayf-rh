...
public class ProyectoTempRepository extends GenericRepository<ProyectoTempEntity, Integer> {
    ...
    private static final String EXISTE_PROYECTO_EN_EJERCICIO_FISCAL = "select" + "   case count(proyecto.nombre)" + "      when 0 then false"
            + "      else true" + "   end" + " from ProyectoTempEntity as proyecto" + "    where proyecto.ejercicioFiscal = :ejercicioFiscal"
    ...
    public List<ProyectoTempEntity> proyectosPorUnidadDependencia(Integer idUnidad, Integer idDependencia) {
        List<ProyectoTempEntity> proyectos = em
                .createQuery("SELECT p FROM ProyectoTempEntity AS p WHERE p.idDependencia =:idDependencia AND p.idUnidadResponsable =:idUnidadResponsable",
                        ProyectoTempEntity.class)
                .setParameter("idDependencia", idDependencia).setParameter("idUnidadResponsable", idUnidad).getResultList();
        return proyectos;
    }

    public List<ProyectoTempEntity> consultarProyectos() {
        List<ProyectoTempEntity> proyectos = em.createQuery("SELECT p FROM ProyectoTempEntity AS p", ProyectoTempEntity.class).getResultList();
        return proyectos;
    }
