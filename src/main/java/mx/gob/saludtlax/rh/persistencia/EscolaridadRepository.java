...
public class EscolaridadRepository extends GenericRepository<EscolaridadEntity, Integer> {
    ...
    public List<EscolaridadEntity> listaEscolaridades() {
        return em.createQuery("SELECT e FROM EscolaridadEntity AS e", EscolaridadEntity.class).getResultList();
    }
    ...
    public EscolaridadEntity escolaridadPorIdHistorialAcademico(Integer idHistorialAcademico) {
        return em.createQuery("SELECT h.escolaridad FROM HistorialAcademicoEntity AS h WHERE h.idHistorialAcademico = :idHistorialAcademico",
                EscolaridadEntity.class).setParameter("idHistorialAcademico", idHistorialAcademico).getSingleResult();
    }
