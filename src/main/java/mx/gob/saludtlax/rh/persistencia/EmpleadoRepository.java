 ...
 */
public class EmpleadoRepository extends
		GenericRepository<EmpleadoEntity, Integer> {
	...
	public EmpleadoEntity obtenerEmpleadoRfc(String rfc) {
		try {
			return em
					...
					.setParameter("rfc", rfc.trim()).getSingleResult();
	...
	public List<InfoVacantePostularDTO> obtenerListaEmpleadoCandidato() {
		...
		List<InfoVacantePostularDTO> resultado = em.createQuery(query,
				InfoVacantePostularDTO.class).getResultList();
