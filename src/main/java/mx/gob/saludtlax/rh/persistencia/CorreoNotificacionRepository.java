...
public class CorreoNotificacionRepository extends GenericRepository<CorreoNotificacionEntity, Integer> {
    ...
    private static final String EXISTE_CORREO = 
            "SELECT"
            + " CASE count(correo.idCorreoNotificacion)"
            + " WHEN 0 THEN false"
            + " ELSE true"
            + " END"
            + " FROM CorreoNotificacionEntity AS correo"
            + " WHERE correo.correoElectronico = :correoElectronico";
    private static final String CONSULTAR_CORREOS =
            "SELECT correo.correoElectronico" 
            + " FROM CorreoNotificacionEntity AS correo";
    ...
    public List<String> consutarCorreos() {
        List<String> consulta = 
                em.createQuery(CONSULTAR_CORREOS, String.class)
                         .getResultList();
        return consulta;
    }
