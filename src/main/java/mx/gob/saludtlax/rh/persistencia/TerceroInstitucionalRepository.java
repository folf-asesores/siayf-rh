 ...
 */
public class TerceroInstitucionalRepository
        extends GenericRepository<TerceroInstitucionalEntity, Integer> {
    ...
    public List<TerceroInstitucionalEntity> obtenerListaTerceroInstitucional() {
        return em.createQuery(OBTENER_LISTA_TERCERO_INSTITUCIONAL,
                TerceroInstitucionalEntity.class).getResultList();
    }

    public List<TerceroInstitucionalDTO> obtenerTerceroInstitucional(
            String clave, String partida) {
        Session session = em.unwrap(Session.class);
        System.out.println("¡¡¡ " + clave + " - " + partida);
        Query query = session.createSQLQuery(
                "SELECT id_tercero_institucional as idTerceroInstitucional, "
                        + " numero as numero, "
                ...
                .setParameter("clave", clave).setParameter("partida", partida);

        query.setResultTransformer(
                Transformers.aliasToBean(TerceroInstitucionalDTO.class));
        @SuppressWarnings("unchecked")
        List<TerceroInstitucionalDTO> datosDto = query.list();
