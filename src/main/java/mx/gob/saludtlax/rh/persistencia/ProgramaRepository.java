...
public class ProgramaRepository extends GenericRepository<ProgramaEntity, Integer> {
    ...
    public ProgramaEntity programaPorDescripcion(String descripcion) {
        try {
            return em.createQuery("SELECT p FROM ProgramaEntity AS p WHERE p.programa =:programa", ProgramaEntity.class).setParameter("programa", descripcion)
                    .getSingleResult();
    ...
    public List<ProgramaEntity> consultarProgramas() {
        return em.createQuery("SELECT p FROM ProgramaEntity AS p ORDER BY p.programa ASC ", ProgramaEntity.class).getResultList();
    }
