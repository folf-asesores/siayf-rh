...
public class PadronEventualEntity24Repository extends GenericRepository<PadronEventualEntity24, Integer> {
	...
	public PadronEventualEntity24 obtenerSuplentePorRfc(String rfc) {
		try {
			return em.createQuery(
					...
					PadronEventualEntity24.class).setParameter("rfc", rfc).getSingleResult();
	...
	public List<PadronEventualEntity24> consultarBajas() {
		return em
				.createQuery("SELECT p FROM PadronEventualEntity24 AS p WHERE p.tipoContratacion = 0 AND p.baja = true",
						PadronEventualEntity24.class)
				.getResultList();
	}

	public List<PadronEventualEntity24> consultarAltas() {
		return em
				.createQuery("SELECT p FROM PadronEventualEntity24 AS p WHERE p.tipoContratacion = 0 AND p.alta = true",
						PadronEventualEntity24.class)
				.getResultList();
