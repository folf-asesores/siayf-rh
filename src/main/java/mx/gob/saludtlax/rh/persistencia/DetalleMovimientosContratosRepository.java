...
public class DetalleMovimientosContratosRepository extends GenericRepository<DetalleMovimientosCotratosEntity, Integer> {
    ...
    public List<DetalleMovimientoContratoDTO> obtenerDetallesPorMovimiento(Integer idMovimiento) {
        ...
        Query query = session.createSQLQuery("Select * From DetalleMovimientosCotratosEntity where id_movimiento_contrato=:idMovimiento")
        ...
        List<DetalleMovimientoContratoDTO> result = query.list();
        return result;
    }

    public List<DetalleMovimientosCotratosEntity> obtenerDetalleMovimientosPorMovimiento(Integer idMovimientoContrato) {
        TypedQuery<DetalleMovimientosCotratosEntity> query = em.createQuery(" FROM DetalleMovimientosCotratosEntity AS d                         "
                + " WHERE                                                    " + " d.idMovimientoContrato = :idMovimientoContrato           ",
                DetalleMovimientosCotratosEntity.class).setParameter("idMovimientoContrato", idMovimientoContrato);
