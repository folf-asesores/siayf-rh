 ...
 */
public class QuincenasSuplenciasRepository
        extends GenericRepository<QuincenasSuplenciasEntity, Integer> {
    ...
    public QuincenasSuplenciasEntity obtenerQuincenaSuplente(int quincena,
            int ejercicio, int idSuplente) {
        try {
            ...
            throw new SistemaException(
                    "El suplente tiene asignada mas detalle en la misma quincena.",
        ...
        try {
            ...
            return em.createQuery(
                    "SELECT q.idQuincenaSuplencia FROM QuincenasSuplenciasEntity AS q WHERE q.numeroQuincena =:quincena "
                            + "AND q.ejercicioFiscal =:ejercicio AND q.suplente.idSuplenteAutorizado =:idSuplente",
                    Integer.class).setParameter("quincena", quincena)
                    .setParameter("ejercicio", ejercicio)
                    .setParameter("idSuplente", idSuplente).getSingleResult();
            ...
            throw new SistemaException(
                    "El suplente tiene asignada la quincena " + quincena
                            + " mas de una vez",
                    SistemaCodigoError.ERROR_MULTIPLES_RESULTADOS);
        }
    }

    public List<QuincenasSuplenciasEntity> consultarSuplenciasPorQuincena(
            int quincena, int ejercicio, String estatus) {
        return em.createQuery(
    ...
    public List<QuincenaSuplenteDTO> consultarQuincenasPorNombre(
            String criterio) {
        return em.createQuery(
                ...
                .setParameter("estatus", EnumEstatusSuplencia.REVISION)
                .setParameter("criterio", "%" + criterio + "%").getResultList();
    }

    public String obtenerEstatusQuincenaSuplente(int quincena, int ejercicio,
            int idSuplente) {
        try {
            ...
            return em.createQuery(
                    "SELECT q.estatus FROM QuincenasSuplenciasEntity AS q WHERE q.numeroQuincena =:quincena "
                            + "AND q.ejercicioFiscal =:ejercicio AND q.suplente.idSuplenteAutorizado =:idSuplente",
                    String.class).setParameter("quincena", quincena)
                    .setParameter("ejercicio", ejercicio)
                    .setParameter("idSuplente", idSuplente).getSingleResult();
    ...
    public List<InfoSuplenciaDTO> consultarDiasSuplidos(Integer idSuplente) {
        return em.createQuery(
                "SELECT NEW mx.gob.saludtlax.rh.empleados.suplencia.InfoSuplenciaDTO(q.idMes, q.ejercicioFiscal, SUM(q.totalDias)) "
                        + "FROM QuincenasSuplenciasEntity AS q "
                        + "WHERE q.suplente.idSuplenteAutorizado =:idSuplente "
                        + "AND q.estatus =:estatus GROUP BY q.ejercicioFiscal, q.idMes",
                InfoSuplenciaDTO.class)
                .setParameter("estatus", EnumEstatusSuplencia.CERRADA)
                .setParameter("idSuplente", idSuplente).getResultList();
    }

    public List<SuplenciasQuincenaDTO> consultarQuincenaSuplencias(
    ...
    public List<QuincenaSuplenteDTO> consultarQuincenasPorCentroEstatus(
            int numeroQuincena, int ejercicioFiscal,
        ...
        return em.createQuery(
                ...
                QuincenaSuplenteDTO.class).setParameter("estatus", estatus)
                .setParameter("numeroQuincena", numeroQuincena)
                .setParameter("ejercicioFiscal", ejercicioFiscal)
                .setParameter("idCentro", centroResponsabilidad)
                .getResultList();
    }

    public List<QuincenasSuplenciasEntity> consultarQuincenasIdSuplente(
            Integer idSuplenteAutorizado) {
        return em.createQuery(
                "SELECT q FROM QuincenasSuplenciasEntity AS q WHERE q.suplente.idSuplenteAutorizado =:idSuplenteAutorizado",
                QuincenasSuplenciasEntity.class)
                .setParameter("idSuplenteAutorizado", idSuplenteAutorizado)
                .getResultList();
    }

    public QuincenasSuplenciasEntity obtenerPorNominaEmpleado(
        ...
        try {
            return em.createQuery(
                    ...
                    QuincenasSuplenciasEntity.class)
                    .setParameter("nominaEmpleadoEntity",
