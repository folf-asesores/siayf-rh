 ...
 */
public class DatosLaboralesCruceRepository
        extends GenericRepository<DatosLaboralesCruceEntity, Integer> {
    ...
    public List<DatosLaboralesCruceEntity> obtenerDatoLaboralEventual(
            String rfc) {
        return em.createQuery(
                "SELECT d FROM DatosLaboralesCruceEntity AS d WHERE d.rfc =:rfc AND d.estatus =:estatus",
                DatosLaboralesCruceEntity.class).setParameter("rfc", rfc)
                .setParameter("estatus", "A").getResultList();
    ...
    public List<DatosLaboralesCruceEntity> obtenerDatoLaboralSuplente(
            String rfc) {
        return em.createQuery(
                "SELECT d FROM DatosLaboralesCruceEntity AS d WHERE d.rfc =:rfc AND d.estatus =:estatus AND d.nombramiento =:nombramiento",
                DatosLaboralesCruceEntity.class).setParameter("rfc", rfc)
                .setParameter("estatus", "A").setParameter("nombramiento", "V")
                .getResultList();
