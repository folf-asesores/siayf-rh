		...
		if (listQuin != null) {
			for(ConfiguracionQuinquenioEntity entity : listQuin){
				...
				datos2.add(new ConfiguracionQuinquenioDTO(entity.getId_configuracion_quinquenio(), entity.getId_empleado(),entity.getRfc(),
						entity.getId_concepto_nomina()!=null?entity.getId_concepto_nomina().getClave():null, entity.getFecha_alta(), entity.getFecha_actualizacion(),
						entity.getEstatus(), entity.getIdnombramiento(), entity.getIdConfiguracionPresupestal(),nombreCompleto));
		...
		if (!list.isEmpty()) {
			for (ConfiguracionQuinquenioEntity entity : list) {
				...
				ConfiguracionQuinquenioDTO dto = new ConfiguracionQuinquenioDTO(entity.getId_configuracion_quinquenio(),
						...
						entity.getIdnombramiento(), entity.getIdConfiguracionPresupestal(),nombreCompleto);
				resultado.add(dto);
			}
		}
	...
	public ConfiguracionQuinquenioDTO obtenerQuinquenioPorConfiguracionPresupuestal(Integer idConfiguracion) {
		...
		try {
			...
			if (ent != null) {
				...
				dto = new ConfiguracionQuinquenioDTO(ent.getId_configuracion_quinquenio(), ent.getId_empleado(),
						ent.getRfc(),
