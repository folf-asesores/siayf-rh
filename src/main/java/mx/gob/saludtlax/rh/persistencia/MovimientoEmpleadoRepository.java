 ...
 */
public class MovimientoEmpleadoRepository
        extends GenericRepository<MovimientoEmpleadoEntity, Integer> {
    ...
    public List<MovimientoEmpleadoEntity> consultarMovimientos(
        ...
        return em.createQuery(
                ...
                .setParameter("idTipoMovimiento", idMovimiento).getResultList();
    }

    // para filtro de busqueda de movimiento por rango de  fechas
    public List<MovimientoEmpleadoEntity> consultarMovimientosFechas(Date fi,
            Date ff) {
        return em.createQuery(
                "SELECT m FROM MovimientoEmpleadoEntity AS m WHERE m.fechaIngreso BETWEEN :fi AND :ff",
                MovimientoEmpleadoEntity.class).setParameter("fi", fi)
                .setParameter("ff", ff).getResultList();
    }

    // busca movimientos por coincidencias de rfc o nombre
    public List<MovimientoEmpleadoEntity> consultarMovimientosRFC(
            String parametro) {
        return em.createQuery(
    ...
    public List<MovimientoEmpleadoEntity> consultarMovimientosPorIdEmpleado(
            Integer idEmpleado) {
    ...
    public List<MovimientoEmpleadoEntity> consultarMovimientosPorRfc(
            String rfc) {

        return em.createQuery(
                "SELECT m FROM MovimientoEmpleadoEntity AS m WHERE m.empleado.rfc =:rfc",
                MovimientoEmpleadoEntity.class).setParameter("rfc", rfc)
                .getResultList();
    }

    public List<MovimientoEmpleadoEntity> consultarMovimientosHijos(
            Integer idMovimientoPadre) {
        return em.createQuery(
                "SELECT m FROM MovimientoEmpleadoEntity AS m WHERE m.movimiento.idPadre =:idMovimientoPadre",
                MovimientoEmpleadoEntity.class)
                .setParameter("idMovimientoPadre", idMovimientoPadre)
                .getResultList();
    }

    public MovimientoEmpleadoEntity obtenerUltimoMovimientoPorPadre(
            Integer idPadre, Integer idEmpleado) {
        try {
            return em.createQuery(
                    ...
                    .setParameter("idEmpleado", idEmpleado).setMaxResults(1)
                    .getSingleResult();
    ...
    public List<ComisionadoLicenciaExcelDTO> listaConsultaComisionadoLicenciaPorRangoFecha(
        ...
        try {
            ...
            System.out
                    .println("Entrando a la consulta de la lista comisionado");
            return result;
        } catch (NoResultException exception) {
