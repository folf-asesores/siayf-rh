...
public class MovimientoEmpleadoRepository extends GenericRepository<MovimientoEmpleadoEntity, Integer> {
    ...
    public List<MovimientoEmpleadoEntity> consultarMovimientos(Integer idMovimiento) {
        return em.createQuery("SELECT m FROM MovimientoEmpleadoEntity AS m WHERE m.movimiento.idTipoMovimiento =:idTipoMovimiento",
    ...
    public List<MovimientoEmpleadoEntity> consultarMovimientosFechas(Date fi, Date ff) {
        return em.createQuery("SELECT m FROM MovimientoEmpleadoEntity AS m WHERE m.fechaIngreso BETWEEN :fi AND :ff", MovimientoEmpleadoEntity.class)
                .setParameter("fi", fi).setParameter("ff", ff).getResultList();
    }
    ...
    public List<MovimientoEmpleadoEntity> consultarMovimientosRFC(String parametro) {
        return em.createQuery("SELECT m FROM MovimientoEmpleadoEntity AS m WHERE m.empleado.rfc LIKE :parametro  or m.empleado.nombreCompleto LIKE :parametro",
                MovimientoEmpleadoEntity.class).setParameter("parametro", "%" + parametro + "%").getResultList();
    }

    public List<MovimientoEmpleadoEntity> consultarMovimientosPorIdEmpleado(Integer idEmpleado) {

        return em.createQuery("SELECT m FROM MovimientoEmpleadoEntity AS m WHERE m.empleado.idEmpleado =:idEmpleado", MovimientoEmpleadoEntity.class)
    ...
    public List<MovimientoEmpleadoEntity> consultarMovimientosPorRfc(String rfc) {
        ...
        return em.createQuery("SELECT m FROM MovimientoEmpleadoEntity AS m WHERE m.empleado.rfc =:rfc", MovimientoEmpleadoEntity.class).setParameter("rfc", rfc)
                .getResultList();
    }

    public List<MovimientoEmpleadoEntity> consultarMovimientosHijos(Integer idMovimientoPadre) {
        return em.createQuery("SELECT m FROM MovimientoEmpleadoEntity AS m WHERE m.movimiento.idPadre =:idMovimientoPadre", MovimientoEmpleadoEntity.class)
    ...
    public MovimientoEmpleadoEntity obtenerUltimoMovimientoPorPadre(Integer idPadre, Integer idEmpleado) {
        try {
            return em.createQuery(
                    ...
                    MovimientoEmpleadoEntity.class).setParameter("idPadre", idPadre).setParameter("idEmpleado", idEmpleado).setMaxResults(1).getSingleResult();
